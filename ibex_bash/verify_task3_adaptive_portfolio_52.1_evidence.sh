...
artifact_dir="outputs/Verify_Task3_AdaptivePortfolio_52.1"
python Audit_Task3_AdaptivePortfolio.py \
  --config config/Verify_Task3_AdaptivePortfolio_52.1.yaml \
  --artifact-dir "$artifact_dir" \
  --output "$artifact_dir/independent_audit.json"

test -s "$artifact_dir/portfolio_selection.json"
test -s "$artifact_dir/independent_audit.json"

result_count="$(find "$artifact_dir/frozen_artifacts" -type f \
  -name per_run.csv | wc -l)"
...
if [[ "$result_count" -ne 40 || "$checkpoint_count" -ne 40 \
  ...
  echo "unexpected frozen portfolio counts: results=$result_count " \
    "checkpoints=$checkpoint_count total=$total_count" >&2
  exit 1
fi
