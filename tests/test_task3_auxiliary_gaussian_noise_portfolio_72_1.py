"""Contracts for preregistered Task3 Gaussian-noise portfolio 72.1."""
...
import Audit_Task3_AuxiliaryGaussianNoisePortfolio_72_1 as audit_module
...
from Select_Task3_AuxiliaryGaussianNoisePortfolio_72_1 import (
    _canonical_sha256,
...
CONFIG = Path("config/Verify_Task3_AuxiliaryGaussianNoisePortfolio_72.1.yaml")
SOURCE_70_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryNormEpsilonPortfolio_70.1.yaml"
)
SOURCE_71_CONFIG = Path("config/Verify_Task3_AuxiliaryGaussianNoise_71.1.yaml")


class AuxiliaryGaussianNoisePortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]), {"current_portfolio", "auxiliary_gaussian_noise"}
        )
        ...
        self.assertEqual(spec["selection"]["target_dataset_macro_f1_gain"], 0.214)
    ...
    def test_source_config_hashes_are_frozen(self):
        ...
        self.assertEqual(
            _canonical_sha256(SOURCE_70_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_71_CONFIG),
            config["sources"]["auxiliary_gaussian_noise"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_is_independent_of_72_selector(self):
        source = inspect.getsource(audit_module)
        self.assertNotIn("Select_Task3_AuxiliaryGaussianNoisePortfolio_72_1", source)
    ...
    def test_evidence_and_cleanup_counts(self):
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_gaussian_noise_portfolio_72.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_gaussian_noise_71.1_cleanup.sh"
        ).read_text(encoding="utf-8")
