...
- **实例覆盖与划分。** “覆盖”＝某划分在该 hairpin 实例头部区域（GT 体积内 ∧ |cos(速度,涡量)|²<0.5）有 ≥2 个采样点，构建目标每实例 10 个。实例按流场固定种子分 80% 训练覆盖组 / 20% 未见组（重叠包围盒同组）；训练覆盖每个覆盖组实例，测试覆盖全部实例（覆盖组用头部区域的偏移点；**未见组直接在该实例 GT 包围盒内均匀随机取，不加任何筛选**，标签按中心 GT 归属）；未见组实例不进训练/验证（排除区＝GT 包围盒，不外扩）；负类头区跟随最近实例分组。
