"""Paired Task1235 angle extension; derived from the frozen nTDO 2.1 runner.
...
def features(rows, name, task, device):
    ...
    for r in rows:
        ...
        for block in name.split('+'):
            if block == 'objective_fmt_nTDO_v2':
                value = objective_fmt_nTDO_v2(r['raw'], num_freq=6)
            elif block == 'fmt_objective_ntod_v2':
                value = fmt_objective_ntod_v2(r['raw'], num_freq=6)
            elif block == 'fmt_v5':
                # Reuse the exact frozen parent cache (historical scale=1,
                # weight=1), avoiding changes from float32 recentering.
                angle = center_angle_fourier(r['raw'], num_freq=6).astype(np.float32)
                value = np.concatenate((r['cached_fmt'], angle), axis=1)
            elif block == 'fmt_all':
                value = r['cached_fmt']
            else:
                value = feature_matrix(adapter, block, device)
            assert np.isfinite(value).all()
            blocks.append(value)
        parts.append(np.concatenate(blocks, axis=1))
...
def preflight(spec, config_path):
    ...
    for task in spec['tasks']:
        for dataset in spec['datasets']:
            ...
            assert len(labels(train)) > spec['task3']['auxiliary_input_width']
            for r in train:
                center_angle_inputs(r['raw'])
            del train
...
def provenance(spec, config_path, task, dataset, seed, device):
    return {'experiment':spec['experiment'], 'task':task, 'dataset':dataset, 'seed':seed,
        ...
        'encoder_hashes':{p:sha('FMT_Utils/'+p) for p in ['fmt_v5.py','fmt_objective_ntod_v2.py',
            'FMT_Angles_3D.py','DFT_FMT_3D.py','objective_fmt_nTDO_v2.py']},
...
def verify_sources(pre, task, dataset, rows):
    if pre.get('smoke_only'):
        return
    hashes = {r['path']:r['sha256'] for r in pre['certificates']
              if r['task']==task and r['dataset']==dataset}
    for row in rows:
        assert hashes[row['path']] == sha(row['path']), 'Source changed after preflight'
...
def run(spec, config_path, task, dataset, seed):
    ...
    verify_sources(pre,task,dataset,train+val)
    ...
    verify_sources(pre,task,dataset,test)
...
def audit_shard(target, expected_arms):
    ...
    if complete['task'] in ('Task3','Task5'):
        counts = {v['parameter_count'] for a,v in frozen.items() if a not in ('raw','raw_wide','fixed_task3_raw')}
        assert len(counts) == 1
        assert next(iter(counts)) < frozen['raw_wide']['parameter_count']
    return rows
...
def audit(spec):
    ...
    for task in spec['tasks']:
        ...
        for arm in spec[task.lower()]['arms']:
            ...
            summary.append(item)
        for method, baseline in [('fmt_v5','old_fmt'),('fmt_objective_ntod_v2','ntdo_v2')]:
            for dataset in spec['datasets']:
                d = [r for r in part if r['dataset'] == dataset]
                delta = [float(next(r['f1'] for r in d if r['arm']==method and int(r['seed'])==seed)) -
                         float(next(r['f1'] for r in d if r['arm']==baseline and int(r['seed'])==seed))
                         for seed in spec[task.lower()]['seeds']]
                paired.append({'task':task,'dataset':dataset,'method':method,'baseline':baseline,
                               'paired_f1_gain':float(np.mean(delta)),
                               'paired_f1_seed_std':float(np.std(delta,ddof=1))})
    ...
    write_json(root/'final_audit.json',{'status':'PASS','rows':len(rows),'shards':sum(len(spec['datasets'])*len(spec[t.lower()]['seeds']) for t in spec['tasks']),
...
def smoke(spec):
    ...
    with tempfile.TemporaryDirectory(prefix='angles_smoke_') as directory:
        ...
        write_json(Path(directory)/'preflight.json',{'status':'PASS','config_sha256':sha(config),'smoke_only':True})
        data={}
...
def main():
    ...
    parser.add_argument('--config',default='config/Verify_Task1235_AngleFeatures_1.1.json')
    parser.add_argument('--phase',choices=['preflight','smoke','Task1','Task2','Task35','audit'],required=True)
