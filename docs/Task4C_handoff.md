...
**状态：规则和代码已准备，正式数据尚未生成；下述长度指单向还是双向整条曲线仍待确认。**
...
- Runge–Kutta–Fehlberg（RK45）双向积分；剔除短线、任一轴零方差等退化线，每束保留10–27线，每线按弧长重采样32点，整束以质心平移并除以最大半径。邻居距离多尺度；Channel长度[0.08,0.12]、ds[0.001,0.005]、11档；TBL长度[3,5.5]、ds[0.01,0.05]、12档。
