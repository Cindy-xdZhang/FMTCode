...
固定参数预算的新Conv3D为原FMT参数量的94.8076%，满足用户90%–95%约束。新合并测试F1为0.789915±0.012577，相对原大Conv均值变化-0.021311；原FMT仍为0.673624±0.009154。新Conv的Channel/TBL测试均值为0.829256/0.726442。本比较只针对这一预声明缩窄结构，不选择其他宽度或按测试换种子，也不替换原4.14模型。
...
在本版相同数据上，新Conv平均测试F1比原FMT高0.116291，且三个对应种子均较高；参数已低于原FMT，故此比较不支持将原大Conv相对FMT的全部性能优势归因于更多参数。另一方面，新Conv平均训练18.432分钟仍长于原FMT2.543分钟；该结果呈现本次结构的精度与训练耗时取舍，不证明其他容量分配或所有流场上的普遍排名。
