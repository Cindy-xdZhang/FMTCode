"""Contracts for preregistered Task3 auxiliary feature-scale portfolio 74.1."""
...
import Audit_Task3_AuxiliaryFeatureScalePortfolio_74_1 as audit_module
...
from Select_Task3_AuxiliaryFeatureScalePortfolio_74_1 import (
    _canonical_sha256,
...
CONFIG = Path("config/Verify_Task3_AuxiliaryFeatureScalePortfolio_74.1.yaml")
SOURCE_72_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryGaussianNoisePortfolio_72.1.yaml"
)
SOURCE_73_CONFIG = Path("config/Verify_Task3_AuxiliaryFeatureScale_73.1.yaml")


class AuxiliaryFeatureScalePortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]), {"current_portfolio", "auxiliary_feature_scale"}
        )
        ...
        self.assertEqual(spec["selection"]["target_dataset_macro_f1_gain"], 0.215)
    ...
    def test_source_config_hashes_are_frozen(self):
        ...
        self.assertEqual(
            _canonical_sha256(SOURCE_72_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_73_CONFIG),
            config["sources"]["auxiliary_feature_scale"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_is_independent_of_74_selector(self):
        source = inspect.getsource(audit_module)
        self.assertNotIn("Select_Task3_AuxiliaryFeatureScalePortfolio_74_1", source)
    ...
    def test_evidence_and_cleanup_counts(self):
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_feature_scale_portfolio_74.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_feature_scale_73.1_cleanup.sh"
        ).read_text(encoding="utf-8")
