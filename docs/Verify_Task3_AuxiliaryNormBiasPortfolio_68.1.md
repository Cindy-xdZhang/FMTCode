...
This training-free development selector is frozen before 67.1 produces any
performance metric.  For each physical family it compares the independently
audited 66.1 portfolio winner with the independently audited 67.1 auxiliary
normalization-bias winner under the same registered metric order.
...
- A 67.1 family winner is eligible only after its zero-tolerance absolute FMT
...
- The selector freezes exactly 40 models and 40 result rows for paired seeds
  40--41 before temporary 67.1 checkpoints are deleted.

The targets are F1 gain `>= +0.212` and absolute FMT F1 `>= 0.893`.  Missing
