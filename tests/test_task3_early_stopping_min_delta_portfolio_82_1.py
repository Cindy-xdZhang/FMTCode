"""Contracts for preregistered Task3 minimum-delta portfolio 82.1."""
...
import Audit_Task3_EarlyStoppingMinDeltaPortfolio_82_1 as audit_module
...
from Select_Task3_EarlyStoppingMinDeltaPortfolio_82_1 import (
    _canonical_sha256,
...
SOURCE_80_CONFIG = Path(
    "config/Verify_Task3_EarlyStoppingPatiencePortfolio_80.1.yaml"
)
SOURCE_81_CONFIG = Path(
    "config/Verify_Task3_EarlyStoppingMinDelta_81.1.yaml"
)


class EarlyStoppingMinDeltaPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "early_stopping_min_delta"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.220
        )
    ...
    def test_source_config_hashes_are_frozen(self):
        ...
        self.assertEqual(
            _canonical_sha256(SOURCE_80_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_81_CONFIG),
            config["sources"]["early_stopping_min_delta"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_is_independent_of_82_selector(self):
        source = inspect.getsource(audit_module)
        self.assertNotIn(
            "Select_Task3_EarlyStoppingMinDeltaPortfolio_82_1", source
        )
        self.assertNotIn(
    ...
    def test_evidence_and_cleanup_counts(self):
        evidence = Path(
            "ibex_bash/verify_task3_early_stopping_min_delta_portfolio_82.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_early_stopping_min_delta_81.1_cleanup.sh"
        ).read_text(encoding="utf-8")
