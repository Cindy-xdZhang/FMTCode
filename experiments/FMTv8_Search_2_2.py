...
DEFAULT_CONFIG='config/Ablation_FMTv8_Search_2.2.json'
...
def cleanup_weights(spec,directory):
    ...
    if not relative.parts or relative.parts[0] not in ('engineering_smoke','cache','final_backbones','search','refine','final','repeatability'):
    ...
    for extension in ('*.pt','*.pth','*.ckpt'):
        for path in directory.rglob(extension):
            ...
            append_record(root/'weight_cleanup.jsonl',row,'FMTv8 Search 2.2 temporary weight removed')
    return deleted
...
def search(spec,config,index,refine=False):
    ...
    for path in (root/'control_repeats').glob('*.json'):assert json.loads(path.read_text())['status']=='PASS'
    assert len(list((root/'control_repeats').glob('*.json')))==21
    ...
    for pid,profile in candidates:
        ...
        if not refine and pid=='p00':
            audit_validation(folder);continue
...
def controls(spec,config,index):
    """Repeat the original V8 twice with the same deterministic backbone."""
    task,dataset=units(spec)[index]
    seed=spec['screen_task4_seed'] if task=='Task4C' else spec['screen_seed']
    data={role:read_cache(spec,task,dataset,role) for role in ('train','validation')}
    folders=[];rows=[]
    for phase in ('search','repeatability'):
        folder=run_directory(spec,phase,task,dataset,'p00','h0',seed);folders.append(folder)
        if task=='Task4C':row,item=task4_fit(spec,config,seed,spec['pools'][0],'h0',data,folder)
        else:row,item=residual_fit(spec,config,task,dataset,seed,spec['pools'][0],'h0',data,folder,cache_folder(spec,task,dataset)/'temporary_backbones')
        del item;rows.append(row);audit_validation(folder)
        cleanup_weights(spec,folder)
    with np.load(folders[0]/'validation_predictions.npz') as a,np.load(folders[1]/'validation_predictions.npz') as b:
        for key in a.files:assert np.array_equal(a[key],b[key]),(task,dataset,key)
    assert rows[0]['selected_epoch']==rows[1]['selected_epoch']
    write(Path(spec['output'])/'control_repeats'/f'{task}_{dataset}.json',dict(status='PASS',task=task,dataset=dataset,
        validation_probability_exact=True,selected_epoch=rows[0]['selected_epoch'],validation_f1=rows[0]['validation']['f1'],
        execution=spec['execution']))


def reconcile_control(spec):
    """Require exact new-control repeats; report, never rewrite, old results."""
    checks=[]
    for task,dataset in units(spec):
        ...
        repeat=json.loads((Path(spec['output'])/'control_repeats'/f'{task}_{dataset}.json').read_text())
        assert repeat['status']=='PASS' and repeat['validation_f1']==row['validation']['f1']
        checks.append(dict(task=task,dataset=dataset,seed=seed,historical_validation_f1=expected,
            deterministic_validation_f1=row['validation']['f1'],historical_difference=row['validation']['f1']-expected))
    write(Path(spec['output'])/'control_reproduction.json',dict(status='PASS',checks=checks,
        repeatability='New deterministic control repeats exactly; historical nondeterministic scores retained separately'))
...
def shortlist(spec,config):
    ...
    pools=[];seen=set()
    for row in ranking:
        canonical=spec['pooling_equivalence'].get(row['pool'],row['pool'])
        if canonical in seen:continue
        pools.append(row['pool']);seen.add(canonical)
...
def preflight(spec,config):
    ...
    diagnosis=json.loads(Path(spec['diagnostic_reference']).read_text())
    assert next(r for r in diagnosis['results'] if r['deterministic'] and r['repeat']==1)['state_exact']
...
def runtime(spec,config,phase,state,code=None):
    ...
    append_record(root/'runtime_events.jsonl',row,'FMTv8 Search 2.2 runtime');append_record('docs/ibex_run_registry.md',row,'FMTv8 Search 2.2 runtime')


def submit(spec,config,phase,dependency=None):
    root=Path(spec['output']);logs=root/'logs';logs.mkdir(parents=True,exist_ok=True)
    gpu=phase in ('preflight','prepare','controls','search','refine','final');count={'prepare':21,'controls':21,'search':105,'refine':63,'final':63}.get(phase)
    cmd=['sbatch','--parsable','--cpus-per-task=4','--mem=32G','--time=02:00:00','--job-name=v8s22-'+phase,
    ...
    cmd+=['ibex_bash/fmtv8_search_2p2.sh',phase,config]
    ...
    append_record(root/'submissions.jsonl',row,'FMTv8 Search 2.2 submitted');append_record('docs/ibex_run_registry.md',row,'FMTv8 Search 2.2 submitted')
    print(json.dumps(row),flush=True)


def main():
    p=argparse.ArgumentParser();p.add_argument('phase',choices=['preflight','prepare','controls','search','shortlist','refine','select','final','merge','runtime','submit'])
