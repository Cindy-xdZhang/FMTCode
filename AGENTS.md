...
2026-09-14最新用户要求`Verify_Task4C_Encoders_4.15`：复用Task4-c 4.14的27,000/3,000/10,000几何样本，比较纯fmt_v5 5.1、用户明确指定的距离+夹角fmt_objective_ntod_v2 2.2及原Conv3D，各三种子；两个新分支不加kin/IVD估计或旧72维方向频谱。保留4.14训练和逐线聚合结构，仅适配输入维数；旧结果不改。协议`docs/Task4C_encoders_protocol_4.15.md`，当前实现与预检，尚无性能结论。Task1/2/3/5上一批“原FMT”的命名更正见experiment_log的task1235-angles-baseline-correction-2026-09-14，不得再将含kin方案称为纯原FMT。
