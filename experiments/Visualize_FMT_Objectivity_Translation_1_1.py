...
def render(paths,labels,original_indices,audit,output_dir,title,medium='paper'):
    ...
    for i,(alpha,trajectories,y) in enumerate(zip(audit['levels'],center,labels)):
        ...
        ax.set_box_aspect(high-low,zoom=1.05)
        ...
        for value,color in [(False,'#3776A6'),(True,'#B63B36')]:
            collection=Line3DCollection(trajectories[y==value],colors=color,
                                        linewidths=.45 if paper else .7,alpha=.7)
    ...
    if fonts['below_minimum_count'] or collision['summary']['fail']:
...
def task1_inputs(args):
    ...
    record=load_cache_records(Path(source['confirmation_cache'])/args.dataset,4,[0])[0]
    ...
    times=t0+indices*dt
    identity={'experiment':'Other_FMTObjectivityTranslation_1.1','dataset':args.dataset,
              'task':'Task1','protocol':'mainExp_Task1_3D_4.1','seed':7080,'confirmation_ordinal':0,
...
def main():
    ...
    for medium in args.media:
        image=render(paths,labels,ids,audit,args.output_dir,
                     f'{args.dataset} | Translation objectivity check',medium)
        print(image)
