...
set -euo pipefail
cd "${TASK3107_REPO_ROOT:-/home/zhanx0o/FMT_Task3_ResidualHeadBeta1Refresh_107_1}"
mkdir -p slurm_logs
source /home/zhanx0o/anaconda3/etc/profile.d/conda.sh
conda activate deepvortex

output_root="$PWD/outputs/Verify_Task3_ResidualHeadBeta1Refresh_107.1"
...
if [[ "$per_run_count" -ne 660 ]]; then
  echo "expected 660 per-run CSV files, found $per_run_count" >&2
  exit 1
fi
...
if [[ "$checkpoint_count_before" -ne 660 ]]; then
  echo "expected 660 temporary checkpoints, found $checkpoint_count_before" >&2
  exit 1
fi
...
if [[ "$archived_per_run_count" -ne 660 ]]; then
...
if [[ "$archived_checkpoint_count" -ne 0 ]]; then
  echo "archive unexpectedly contains checkpoints" >&2
  exit 1
fi
...
python Audit_Task3_ParameterSearch.py \
...
(
  ...
  sha256sum optimization_leaderboard.csv optimization_selection.json \
    preflight_manifest.json per_run_csv.tar.gz independent_audit.json \
    > artifact_sha256.txt
)
