...
from experiments import FMTv8_Search_2_2 as run
...
def merge(spec,config):
    ...
    for task,dataset in run.units(spec):
        ...
        folder=run.run_directory(spec,'repeatability',task,dataset,'p00','h0',seed)
        repeated=verify_predictions(folder,expected_labels=run.read_cache(spec,task,dataset,'validation')['labels'])
        first=run.run_directory(spec,'search',task,dataset,'p00','h0',seed)
        with np.load(first/'validation_predictions.npz') as a,np.load(folder/'validation_predictions.npz') as b:
            assert all(np.array_equal(a[k],b[k]) for k in a.files)
        repeat_rows.append(repeated)
    for first,second in [('p31','p40'),('p29','p45')]:
        for task,dataset in run.units(spec):
            seed=spec['screen_task4_seed'] if task=='Task4C' else spec['screen_seed']
            a=run.run_directory(spec,'search',task,dataset,first,'h0',seed)
            b=run.run_directory(spec,'search',task,dataset,second,'h0',seed)
            with np.load(a/'validation_predictions.npz') as aa,np.load(b/'validation_predictions.npz') as bb:
                assert all(np.array_equal(aa[k],bb[k]) for k in aa.files)
    ...
    for directory in [root/'cache',root/'final_backbones',root/'search',root/'refine',root/'final',root/'repeatability']:
    ...
    run.write(root/'summary.json',dict(version=spec['version'],selected=lock['selected'],rows=table,composite=composite,
        ...
        repeatability_classifier_seconds=sum(r['seconds'] for r in repeat_rows),
    ...
    run.write(root/'independent_audit.json',dict(status='PASS',validation_fits=len(flat),final_runs=len(finals),
        ...
        exact_control_repeats=21,equivalent_candidate_pairs_reproduced=42))
...
def complete(root):
    ...
    counts={'prepare':21,'controls':21,'search':105,'refine':63,'final':63}
    expected=set()
