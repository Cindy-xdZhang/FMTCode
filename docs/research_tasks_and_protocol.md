...
当前 **`mainExp_Task4C_Regularized_4.2`** 完全复用4.1物理样本，原FMT拼接坐标/单位切向的有方向频谱，并对训练几何增强及余弦间隔分类作对照，见[4.2协议](Task4C_regularized_protocol_4.2.md)。每流场15,000训练开发（内部13,500拟合+1,500验证）、5,000固定测试primitive；额外训练增强不计作独立物理样本，原3.1测试每流场500不变。
