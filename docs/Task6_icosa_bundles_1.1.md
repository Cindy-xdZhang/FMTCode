...
## Independent-check rejection and replacement

A sample that passes the three construction levels may still fail the independent h/64 check. In that case the builder rejects the entire 21-line sample, draws a new center from the same candidate distribution and requires the replacement to pass both the original levels and h/64. It preserves h, thresholds, integration settings and the sample count. Replacement indices and errors are recorded in the checkpoint and frame metadata. No individual neighbor is patched or padded.

This occurred in Re640 frame60: sample8665, neighbor6 terminated numerically in the h/64 backward integration (reason3), despite its coordinate error being below the limit. One complete sample was replaced. A separate rerun checked all16 audit centers/336 lines successfully; maximum difference0.0001058951 versus the unchanged limit0.0025157231. Evidence: `outputs/Verify_Task6_IcosaBundles_1.1/final_check_failure.json` and `final_recovery_independent.json`.

## Completed publication, 2026-09-22

All101 frames have completed and the final catalog now reports `COMPLETE`, with no pending frames. Totals:1,010,000 samples and21,210,000 streamlines,65 points each. All NPY arrays occupy16,866,067,568 bytes; curve arrays account for16,543,812,928 bytes including headers. The publisher verified all derived-file checksums, source field checksums, finite arrays and current human labels, including independent continuous-segment label checks. The canonical manifest remains `6c0d28f95dc35724478041ff51eacd972a14959111c14733a05f8d4999439f43`. Final bundle catalog SHA256: `422894c5ea8c9b04614514503a93a6f63633da286ad0420aadf6d422ce3dfbac`. Local evidence: `outputs/Verify_Task6_IcosaBundles_1.1/completion.json`. Earlier building/PARTIAL paragraphs above are historical progress snapshots.
