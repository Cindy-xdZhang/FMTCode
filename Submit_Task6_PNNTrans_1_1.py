"""Submit the validation phase, then separately admit final jobs only after its gate."""
import argparse
import datetime
import json
import os
...
def runtime(config, phase, state, exit_code=None):
    ...
    import socket
    device = 'CPU'
    if phase in ('fit_check', 'search', 'final'):
        import torch
    ...
    append_locked('docs/ibex_run_registry.md',
        f'\n- {now} | {name} | {spec["experiment"]} Task6 {phase} | {state} '
        f'node={socket.gethostname()} device={device} exit={exit_code}\n')
...
def submit(mode, config):
    ...
    for phase, cpu, mem, limit, array, gpu in stages:
        ...
        command += ['ibex_bash/task6_pnn_trans_1p1.sh', phase, config]
        ...
        append_locked('docs/ibex_run_registry.md', f'\n- {row["submitted"]} | {job} | {spec["experiment"]} '
            f'Task6 {phase} | {config} SHA256={row["config_sha256"]} | {commit} | '
            f'{row["device"]} array={array} | SUBMITTED\n')
