"""Contracts for preregistered Task3 gradient-clip portfolio 78.1."""
...
import Audit_Task3_AuxiliaryGradientClipPortfolio_78_1 as audit_module
...
from Select_Task3_AuxiliaryGradientClipPortfolio_78_1 import (
    _canonical_sha256,
...
CONFIG = Path(
    "config/Verify_Task3_AuxiliaryGradientClipPortfolio_78.1.yaml"
)
SOURCE_76_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryPostNormPortfolio_76.1.yaml"
)
SOURCE_77_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryGradientClip_77.1.yaml"
)


class AuxiliaryGradientClipPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "auxiliary_gradient_clip"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.218
        )
    ...
    def test_source_config_hashes_are_frozen(self):
        ...
        self.assertEqual(
            _canonical_sha256(SOURCE_76_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_77_CONFIG),
            config["sources"]["auxiliary_gradient_clip"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_is_independent_of_78_selector(self):
        source = inspect.getsource(audit_module)
        self.assertNotIn(
            "Select_Task3_AuxiliaryGradientClipPortfolio_78_1", source
        )
        self.assertNotIn(
    ...
    def test_evidence_and_cleanup_counts(self):
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_gradient_clip_portfolio_78.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_gradient_clip_77.1_cleanup.sh"
        ).read_text(encoding="utf-8")
