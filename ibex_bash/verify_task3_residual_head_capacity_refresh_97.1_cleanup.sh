...
repo_root="${TASK397_REPO_ROOT:-/home/zhanx0o/FMT_Task3_ResidualHeadCapacityRefresh_97_1}"
portfolio_root="${TASK398_REPO_ROOT:-/home/zhanx0o/FMT_Task3_ResidualHeadCapacityRefreshPortfolio_98_1}"
cd "$repo_root"
mkdir -p slurm_logs

output_root="$repo_root/outputs/Verify_Task3_ResidualHeadCapacityRefresh_97.1"
...
portfolio="$portfolio_root/outputs/Verify_Task3_ResidualHeadCapacityRefreshPortfolio_98.1/portfolio_selection.json"
portfolio_audit="$portfolio_root/outputs/Verify_Task3_ResidualHeadCapacityRefreshPortfolio_98.1/independent_audit.json"
...
expected_output_root="$resolved_outputs/Verify_Task3_ResidualHeadCapacityRefresh_97.1"
...
if [[ "$resolved_output_root" != "$expected_output_root" ]] || \
  ...
  echo "refusing cleanup outside exact resolved 97.1 root: $resolved_candidates" >&2
  exit 1
fi
...
if evidence.get("status") != "passed" or int(
    ...
    raise SystemExit("97.1 evidence archive is incomplete")
if hashlib.sha256(archive_path.read_bytes()).hexdigest() != str(
    evidence.get("stable_archive_sha256", "")
):
    raise SystemExit("97.1 archive SHA-256 differs")
if source_audit.get("status") != "passed":
    raise SystemExit("97.1 independent audit did not pass")
if audit.get("status") != "passed" or not audit.get("all_frozen_hashes_verified"):
    raise SystemExit("98.1 independent portfolio audit did not pass")
if int(portfolio.get("frozen_model_count", -1)) != 40 or int(
    portfolio.get("frozen_artifact_file_count", -1)
) != 80:
    raise SystemExit("98.1 frozen portfolio is incomplete")
PY

checkpoint_count_before="$(find "$resolved_candidates" -type f \
  \( -name '*.pt' -o -name '*.pth' -o -name '*.ckpt' \) | wc -l)"
if [[ "$checkpoint_count_before" -ne 780 ]]; then
  echo "expected exactly 780 temporary 97.1 checkpoints, found $checkpoint_count_before" >&2
  exit 1
fi
...
if [[ "$checkpoint_count_after" -ne 0 ]]; then
  echo "97.1 checkpoint cleanup incomplete: $checkpoint_count_after remain" >&2
  exit 1
fi
...
Path(sys.argv[1]).write_text(json.dumps({
    "experiment": "Verify_Task3_ResidualHeadCapacityRefresh_97.1",
    "job_id": os.environ.get("SLURM_JOB_ID", ""),
    "checkpoint_count_before": int(sys.argv[2]),
    "checkpoint_count_after": int(sys.argv[3]),
    "reason": "98.1 frozen portfolio and independent audit complete",
    "status": "passed",
