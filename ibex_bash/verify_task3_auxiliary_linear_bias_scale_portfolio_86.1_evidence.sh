...
set -euo pipefail
cd "${TASK386_REPO_ROOT:-/home/zhanx0o/FMT_Task3_AuxiliaryLinearBiasScalePortfolio_86_1}"
mkdir -p slurm_logs
source /home/zhanx0o/anaconda3/etc/profile.d/conda.sh
conda activate deepvortex

artifact_dir="outputs/Verify_Task3_AuxiliaryLinearBiasScalePortfolio_86.1"
python Audit_Task3_AuxiliaryLinearBiasScalePortfolio_86_1.py \
  --config config/Verify_Task3_AuxiliaryLinearBiasScalePortfolio_86.1.yaml \
...
if [[ "$result_count" -ne 40 || "$checkpoint_count" -ne 40 \
  ...
  echo "unexpected 86.1 frozen counts: results=$result_count checkpoints=$checkpoint_count total=$total_count" >&2
  exit 1
fi
