...
2026-09-15最新用户确认`task4-c-v9.15_v2`长度均为单向：Channel[0.08,0.12]、TBL[3,5.5]。正式配置已解除待确认状态，保持132,000束及每实例3束head；使用指定p35/n0_k06（最大半径、训练集逐特征标准化、6频率），与32³/36³/48³小Conv比较。科学commit9769c68d已公开推送；Ibex51911059–51911066共50进程已提交，依次GPU预检、132实例pilot、实测空间检查、正式构建、全量数据核验、编码、12次训练、汇总。当前尚无正式数据/新F1，状态见docs/ibex_run_registry.md。用户已明确允许公开整个当前分支，此授权取代历史公开推送待确认条目。
