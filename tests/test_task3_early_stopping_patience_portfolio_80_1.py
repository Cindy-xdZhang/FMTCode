"""Contracts for preregistered Task3 patience portfolio 80.1."""
...
import Audit_Task3_EarlyStoppingPatiencePortfolio_80_1 as audit_module
...
from Select_Task3_EarlyStoppingPatiencePortfolio_80_1 import (
    _canonical_sha256,
...
SOURCE_78_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryGradientClipPortfolio_78.1.yaml"
)
SOURCE_79_CONFIG = Path(
    "config/Verify_Task3_EarlyStoppingPatience_79.1.yaml"
)


class EarlyStoppingPatiencePortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "early_stopping_patience"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.219
        )
    ...
    def test_source_config_hashes_are_frozen(self):
        ...
        self.assertEqual(
            _canonical_sha256(SOURCE_78_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_79_CONFIG),
            config["sources"]["early_stopping_patience"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_is_independent_of_80_selector(self):
        source = inspect.getsource(audit_module)
        self.assertNotIn(
            "Select_Task3_EarlyStoppingPatiencePortfolio_80_1", source
        )
        self.assertNotIn(
    ...
    def test_evidence_and_cleanup_counts(self):
        evidence = Path(
            "ibex_bash/verify_task3_early_stopping_patience_portfolio_80.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_early_stopping_patience_79.1_cleanup.sh"
        ).read_text(encoding="utf-8")
