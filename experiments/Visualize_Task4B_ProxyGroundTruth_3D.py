"""Re-threshold saved full-volume proxy GT and render it; never run training."""
import argparse
import hashlib
import json
import os
...
def render_volume(labels,low,high,spacing,path,name,threshold,fraction,spec):
    ...
    text(renderer,f'{name.upper()}  |  Proxy ground truth: IVD > 0.98a',.027,.951,30)
    for k in range(4):
...
def main(config, render_only=False):
    ...
    for name in ['channel','tbl']:
        ...
        a=original['flows'][name]['thresholds']['a']; threshold=spec['threshold_multiplier']*a
        ...
        assert np.all(vortex[ids>0]) and np.all(gt_ivd>threshold)
        ...
        report=dict(a=a,old_threshold=.9*a,new_threshold=threshold,old_vortex_fraction=float((ivd>.9*a).mean()),
            new_vortex_fraction=float(vortex.mean()),removed_voxel_count=int(np.sum((ivd>.9*a)&~vortex)),
        ...
        report['hairpins']=render(points,hairpin_labels,spacing,points.min(0)-spacing,points.max(0)+spacing,
            out/f'{name}_groundtruth_hairpins_3d.png',f'{name.upper()} | Proxy GT hairpins: IVD > 0.98a',
            f'All {len(points):,} hairpin voxels on the complete GT grid | no streamline-validity filter | no model prediction')
...
if __name__=='__main__':
    ...
    p.add_argument('--config',default='config/Other_Task4B_ProxyGTThreshold_4.2.json')
