"""Task4-c 4.4: fixed data and networks with hidden-representation interpolation regularization."""
...
DEFAULT_CONFIG = "config/mainExp_Task4C_ManifoldMixup_4.4.json"
...
def identity(config):
    ...
    for p in ("experiments/Task4C_Multiscale_4_1.py", "FMT_Utils/Task4C_Multiscale_4_1.py",
              "FMT_Utils/Task4C_Regularized_4_2.py", "FMT_Utils/Task4C_LinePooling_4_3.py",
              "experiments/Task4C_LinePooling_4_3.py",
...
def train(spec, config, index, phase):
    ...
    from FMT_Utils.Task4C_ManifoldMixup_4_4 import make_model, training_loss, classifier_probability
    ...
    mix_rng = np.random.default_rng(seed + 100000)
    ...
    for epoch in range(1, options["epochs"]+1):
        ...
        for start in range(0, len(order), batch_size):
            ...
            loss = training_loss(model, x, y, candidate, positive_weight, mix_rng)
...
def submit_job(spec, config, phase, array=None, previous=None):
    ...
    command = ["sbatch", "--parsable", "--nodes=1", "--ntasks=1", "--cpus-per-task=4",
               ...
               f"--job-name=t4c44-{phase}", f"--output={out}/logs/{phase}_%A_%a.out", f"--error={out}/logs/{phase}_%A_%a.err"]
    if previous:
    ...
    command += ["ibex_bash/task4c_manifold_mixup_4p4.sh", phase, config]
    ...
    append_locked("docs/ibex_run_registry.md", "\n- Task4-c 4.4 SUBMITTED "+json.dumps(row)+"\n")
    print(json.dumps(row), flush=True)
    return job


def check_predecessor(spec):
    """Run another development round only after 4.3 is complete and below target."""
    source = spec["prerequisite"]
    previous = json.loads((Path(source["output"])/"selection.json").read_text())
    if previous["identity"]["config_sha256"] != source["config_sha256"]:
        raise ValueError("Predecessor identity changed")
    if previous["validation_gate_passed"] or previous["test_loaded"]:
        raise ValueError("Review the completed predecessor before opening another experiment")
    return previous


def submit(spec, config):
    check_predecessor(spec)
