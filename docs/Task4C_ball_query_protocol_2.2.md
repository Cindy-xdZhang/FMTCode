...
- 本轮 `h_mode=local_min`，即样本所在网格单元的最小边长，配置记录原始网格哈希和逐样本 h 文件哈希；禁止直接将旧 `local_grid_scale`（三方向间距几何平均）误当最小间距。全流场最小间距也已诊断，固定1h下全部零邻居；`h_definition_equivalence.json` 逐行核实两种 h 在本轮扩张规则下选出的邻居集合完全相同、均等于最近k个点且无第k距离并列，因此这一计量选择不影响本轮训练输入。
...
输出根 `outputs/Ablation_Task4C_BallQuery_2.2/`；`source_verification.json` 记录球统计，`neighbors/*.npz` 保存全部邻居证据；`arms/*/seed96611/final/*/seed96611/` 保存选轮锁、日志和预测；`summary.json` 只有预测复算通过后才算正式结果。科学commit与实际作业号记于独立部署状态，避免提交后改动作为科学身份一部分的本文。
