...
CONFIG = 'config/Verify_FMT_SingleCenter_Task354C_1.1.json'
CODE = (
    ...
    'experiments/FMT_SingleCenter_Task354C_1_1.py', 'tests/test_fmt_single_center_1_1.py',
    'config/mainExp_Task135_FMTv8_1.1.json', 'config/Verify_Task4C_P35GTHead_1.1.json',
    'ibex_bash/fmt_single_center_task354c_1p1.sh', CONFIG,
)
...
def read4(source, role, device, batch):
    ...
    for fi, flow in enumerate(('channel', 'tbl')):
        ...
        count = len(geometry)
        assert count == expected['samples'] and geometry.shape[2:] == (32, 3)
        center = (meta['center'].astype(np.float64)-meta['centroid'].astype(np.float64))/meta['radius'].astype(np.float64)[:, None]
        centers, distance = fixed_center_ids(seeds, meta['counts'], center)
        for first in range(0, count, batch):
            sl = slice(first, first+batch)
            x = torch.tensor(np.array(geometry[sl]), device=device)
            values.append(encode(x, meta['counts'][sl], centers[sl]).cpu().numpy())
        pieces.append(dict(labels=meta['labels'].astype(np.int64), center_id=centers,
                           center_seed_distance=distance, flow_index=np.full(count, fi, dtype=np.int64),
                           row_in_split=np.arange(count), instance=meta['instance']))
        certificates.append(dict(flow=flow, role=role, files={str(folder/k): v for k, v in expected['files'].items()},
                                 samples=count, original_center_missing=int((distance >= 1e-6).sum()),
                                 center_ids_sha256=array_sha(centers), max_center_seed_distance=float(distance.max())))
...
def preflight(spec, parent, source4, config):
    ...
    for task in ('Task3', 'Task5', 'Task4c'):
        if task == 'Task4c':
            ...
            with np.load(folder/'metadata.npz') as z:
                selected = np.r_[np.flatnonzero(z['labels'] == 0)[:16], np.flatnonzero(z['labels'] == 1)[:16]]
                labels = z['labels'][selected].astype(np.int64)
                counts = z['counts'][selected]
                original = (z['center'][selected].astype(np.float64)-z['centroid'][selected])/z['radius'][selected, None]
            x = np.load(folder/'geometry.npy', mmap_mode='r')[selected]
            seed_positions = np.load(folder/'seeds.npy', mmap_mode='r')[selected]
            centers, _ = fixed_center_ids(seed_positions, counts, original)
        else:
    ...
    write(Path(spec['output'])/'preflight.json', dict(status='PASS', identity=identity(config), gpu=torch.cuda.get_device_name(),
          tests=outcome.testsRun, pilots=pilots, splits=checks, parameters=sum(p.numel() for p in model.parameters()),
                ...
                if task == 'Task4c':
                    ...
                    for fi, flow in enumerate(('channel', 'tbl')):
                        ...
                        with np.load(original/'metadata.npz') as m:
                            ...
                            centers, _ = fixed_center_ids(np.load(original/'seeds.npy'), m['counts'], (m['center'].astype(np.float64)-m['centroid'])/m['radius'][:, None])
                            assert np.array_equal(z['center_id'][sel], centers)
                else:
