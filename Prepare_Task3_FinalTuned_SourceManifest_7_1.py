"""Derive the Task3 7.1 temporal-source manifest without copying flow data."""
...
def derive(config_path: str | Path, overwrite: bool = False) -> Path:
    ...
    if parent_hash != str(section["parent_manifest_sha256"]):
        raise RuntimeError("Task3 7.1 parent source manifest changed")
    ...
    if target.exists() and not overwrite:
        ...
        if previous != payload:
            raise RuntimeError("Task3 7.1 derived source manifest changed")
        print(target)
        return target
