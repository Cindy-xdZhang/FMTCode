...
def task1_inputs(args):
    ...
    times=t0+indices*dt
    identity={'experiment':'Verify_FMTObservedPathline_2.1','dataset':args.dataset,
    ...
    classifier.predict_features=lambda feature:model.predict(feature)==vortex
    return lab_velocity,initial,times,classifier,dt,identity


def main():
    from FMT_Utils.ObservedFieldWorldline_3D import TranslatingObserverField,ReferenceFrameFromWorldline,ObservedVectorField
    from FMT_Utils.TranslationObserver_3D import integrate_pathlines
    from FMT_Utils.DFT_FMT_3D import pathline_dft_features_3d
    import torch
    p=argparse.ArgumentParser(description='Explicit worldline -> transformation -> observed field -> pathlines')
    ...
    prior=ROOT/'outputs/Other_FMTObjectivityTranslation_1.4'/args.dataset
    previous=json.loads((prior/'paper.json').read_text())
    ...
    if 'source_window_sha256' in previous:
        assert hashlib.sha256(args.source_file.read_bytes()).hexdigest()==previous['source_window_sha256']
    with np.load(prior/'observed_pathlines.npz') as z:
        old=z['pathlines'];initial=old[0,:,:,0,:].copy();old_lab=old[0].copy();del old
        times=z['sample_times'].copy()
    indices=np.asarray(previous['classification_sample_indices'])
    all_paths=[];all_labels=[];details=[];base_features=None
    def descriptor64(p):
        return pathline_dft_features_3d(torch.from_numpy(p),num_freq=6,neighbor_weight=1.,neighbor_scale=1.,neighbor_pool='sort',mode='gram',include_chirality=True)
    for alpha in np.linspace(0,1,7):
        # Step 1: integrate the camera worldline in its observer field.
        observer=TranslatingObserverField(previous['observer_velocity_times'],alpha*np.asarray(previous['observer_velocity_samples']))
        frame=ReferenceFrameFromWorldline(observer,times[0])
        # Step 2: construct observed field with inverse-location sampling.
        observed=ObservedVectorField(v,observer,frame)
        seed=frame.lab_to_observed(initial,times[0])
        # Step 3: integrate the observed vector field (not the relative field).
        paths=integrate_pathlines(observed,seed.reshape(-1,3),times,previous['max_step']).reshape(len(initial),7,len(times),3)
        assert np.isfinite(paths).all(), 'Do not alter material cohort.'
        if alpha==0:
            lab=paths.copy()
            assert np.max(np.abs(lab-old_lab))<1e-10
        displacement=np.stack([frame.camera_position(t)-frame.start for t in times])
        exact=lab-displacement[None,None]
        error=float(np.max(np.linalg.norm(paths-exact,axis=-1)))
        assert error<previous['correspondence_tolerance']
        labels,features=classifier(paths[:,:,indices])
        exact_labels,exact_features=classifier(exact[:,:,indices])
        if base_features is None:
            base_features=features.copy();base_labels=labels.copy()
            base_descriptor64=descriptor64(lab[:,:,indices])
        center_only=base_features.copy();center_only[:,:23]=exact_features[:,:23]
        other_only=base_features.copy();other_only[:,23:]=exact_features[:,23:]
        exact_d64=descriptor64(exact[:,:,indices])
        relative_geometry=(paths[:,1:]-paths[:,:1])-(lab[:,1:]-lab[:,:1])
        info={'alpha':float(alpha),'correspondence_max_error':error,
              'camera_displacement_end':displacement[-1].tolist(),
              'changed_labels':int(np.count_nonzero(labels!=base_labels)),
              'exact_transform_changed_labels':int(np.count_nonzero(exact_labels!=base_labels)),
              'integrated_vs_exact_label_disagreements':int(np.count_nonzero(labels!=exact_labels)),
              'center_block_only_changed_labels':int(np.count_nonzero(classifier.predict_features(center_only)!=base_labels)),
              'other_blocks_only_changed_labels':int(np.count_nonzero(classifier.predict_features(other_only)!=base_labels)),
              'equal_time_offset_max_error':float(np.max(np.abs(relative_geometry))),
              'exact_float64_center_fmt_max_change':float(np.max(np.abs(exact_d64[:,:23]-base_descriptor64[:,:23]))),
              'exact_float64_neighbor_fmt_max_change':float(np.max(np.abs(exact_d64[:,23:]-base_descriptor64[:,23:]))),
              'float32_center_fmt_max_change':float(np.max(np.abs(exact_features[:,:23]-base_features[:,:23]))),
              'float32_neighbor_fmt_max_change':float(np.max(np.abs(exact_features[:,23:161]-base_features[:,23:161]))),
              'float32_kin4_max_change':float(np.max(np.abs(exact_features[:,161:]-base_features[:,161:])))}
        if alpha==1:
            wrong=integrate_pathlines(lambda x,t:v(x,t)-observer(x,t),initial[:128,0],times,previous['max_step'])
            valid=np.isfinite(wrong).all(axis=(1,2))
            info['negative_control_complete_count']=int(valid.sum())
            info['wrong_relative_field_max_path_difference']=float(np.max(np.linalg.norm(wrong[valid]-paths[:128,0][valid],axis=-1))) if valid.any() else None
        print(json.dumps(info),flush=True)
        details.append(info);all_paths.append(paths);all_labels.append(labels)
    paths=np.stack(all_paths);labels=np.stack(all_labels)
    with np.load(ROOT/'outputs/Other_FMTObjectivityTranslation_1.5'/args.dataset/'observed_pathlines.npz') as z:
        previous_labels=z['predictions']
    audit={**previous,**identity,'levels':[d['alpha'] for d in details],
           'changed_labels_vs_original':[d['changed_labels'] for d in details],
           'vortex_counts':[int(y.sum()) for y in labels],
           'trajectory_correspondence_max_error':[d['correspondence_max_error'] for d in details],
           'worldline_feature_audit':details,'prior_version_label_disagreements':[int(np.count_nonzero(a!=b)) for a,b in zip(labels,previous_labels)],
           'pipeline':'Numerical camera worldline -> inverse reference transformation -> observed vector field -> RK4 pathlines',
           'observer_worldline_solver':'DOP853 rtol1e-12 atol1e-13; pure translation R=I',
           'reference_time':float(times[0]),'reference_transform':'identity at reference time',
           'bounding_box_drawn':False,'sample_times':times.tolist(),
           'job_id':os.environ.get('SLURM_JOB_ID'),'node':os.environ.get('SLURMD_NODENAME'),
           'source_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
    # Remove four-level arrays inherited only for cohort provenance.
    for key in ['feature_max_absolute_change','observer_displacement_at_end','mean_velocity_over_path_window']:
        audit.pop(key,None)
    audit['changed_label_fraction']=[float(np.mean(y!=labels[0])) for y in labels]
    audit['render_source_sha256']=audit['source_sha256']
    for key in ['display','shared_bounds','camera','medium','minimum_font_pt','figure_size_inches','view_zoom','artist_clipping','line_opacity','style_reason']:
        audit.pop(key,None)
    args.output_dir.mkdir(parents=True)
    (args.output_dir/'audit.json').write_text(json.dumps(audit,indent=2))
    np.savez_compressed(args.output_dir/'observed_pathlines.npz',pathlines=paths,predictions=labels,sample_times=times)
    for medium in ['paper','slides']:
        title=f"{dict(cylinder3d='Re160',halfcylinderRe640='Re640',halfcylinderRe6400='Re6400')[args.dataset]} 3D | Integrated observer | t0 = {times[0]:g}"
        print(render(paths,labels,np.arange(len(initial)),audit,args.output_dir/'final',title,medium,pdf_collision_audit=False),flush=True)
    print('Prior-version label disagreements:',audit['prior_version_label_disagreements'],flush=True)
