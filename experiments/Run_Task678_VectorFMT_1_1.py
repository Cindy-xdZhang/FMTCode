"""Directional Fourier-token extension on the frozen HanSampling cohort; version 1.1."""
...
from FMT_Utils.VectorFlowMapFMT_3D import features, reconstructed_data
from experiments.Run_Task678_HanSampling_1_1 import load_role as load_base_role
DEFAULT_CONFIG = 'config/Verify_Task678_VectorFMT_1.1.json'
...
def load_role(spec, dataset, role):
    assert sha256(spec['base_config']) == spec['base_config_sha256']
    base = json.loads(Path(spec['base_config']).read_text())
    base['output_root'] = spec['base_output_root']
    base['_config_sha256'] = spec['base_config_sha256']
    audit = json.loads((Path(base['output_root']) / 'data_audit.json').read_text())
    assert audit['status'] == 'PASS' and audit['config_sha256'] == spec['base_config_sha256']
    data, sources = load_base_role(base, dataset, role)
    return {**data, **features(data)}, sources
...
def run(spec, config, dataset, arm, seed, device='cuda'):
    ...
    common = {**provenance(config), 'experiment': spec['experiment'], 'dataset': dataset, 'arm': arm, 'seed': seed,
        ...
        'token_bytes': 924, 'base_config_sha256': spec['base_config_sha256'],
        'vector_encoder_sha256': sha256('FMT_Utils/VectorFlowMapFMT_3D.py'),
        'physical_input_lines': 0 if arm == 'coordinates_only' else 7}
    assert common['frozen_encoder_sha256'] == spec['frozen_encoder_sha256']
    assert common['vector_encoder_sha256'] == spec['vector_encoder_sha256']
        ...
        for role in ROLES:
            ...
            for variant in variants:
                ...
                for task in ('Task6', 'Task7', 'Task8'):
                    info = {'parameter_count': 0, 'optimizer_steps': 0, 'input_dimension': 231} if arm == 'vector_affine' else dict(models['Task6' if task == 'Task8' else task][1])
                    info.pop('learning_curve', None)
                    diagnostic = None
                    if arm == 'vector_affine':
                        prediction = interpolation(reconstructed_data(data), task)
                    else:
                    ...
                    if task == 'Task8':
                        info.update(second_query_uses='predicted_first_stage_endpoint', training_source='Task6 vector-FMT short-map model or inverse Fourier plus affine interpolation')
                        if arm == 'vector_affine':
                            steps = data['target0'].shape[2] - 1
...
def main():
    ...
    row = (spec['datasets'][args.index], 'vector_affine', 0) if args.affine else matrix(spec)[args.index]
