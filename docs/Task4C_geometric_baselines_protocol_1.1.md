...
Numerical correction before any formal training: V100 preflight 51930114 failed for a random 64-point synthetic cloud because float32 reductions amplified through the high-frequency functions (diagnostic 51930141 maximum feature difference 0.00150585 between batches). Float64 internal arithmetic produced exactly identical features across batch sizes in the 64/320/864-point diagnostic. Use float64 throughout the fixed Point-NN encoder and cast its final cache to float32. Do not loosen the preflight tolerance or change frequencies. The first dependency chain was canceled before data encoding or training; all failure records remain. Revised `execution_revision` ends in `float64_pointnn`.
