...
def render(paths,labels,original_indices,audit,output_dir,title,medium='paper',pdf_collision_audit=True):
    ...
    fig.text(.5,.006,'Labels: first 48 steps; displayed paths: 96 steps (2x duration)' ,ha='center',fontsize=font)
...
def task1_inputs(args):
    ...
    times=t0+indices*dt
    identity={'experiment':'Other_FMTObjectivityTranslation_1.4','dataset':args.dataset,
...
def main():
    ...
    if args.global_mean:
        ...
        schedule=TranslationObserver([times[0],times[0]+96*dt],[args.target_velocity]*2,times[0])
    else:
    ...
    from FMT_Utils.DenseObserverPaths_3D import dense_candidates,select_complete
    old_count=len(initial);target=2*old_count
    classification_indices=np.linspace(0,48,32).round().astype(int)
    long_times=times[0]+np.arange(97)*dt
    candidates,seeding_bounds=dense_candidates(initial,identity['primitive_offset'])
    initial,density_audit=select_complete(v,candidates,long_times,dt/args.substeps,target)
    def classify_original_window(paths):
        return classifier(paths[:,:,classification_indices])
    paths,labels,ids,audit=evaluate_observers(v,initial,long_times,classify_original_window,schedule,
                         dt/args.substeps,args.correspondence_tolerance)
    if len(ids)!=target:raise RuntimeError('Some selected primitives are incomplete in another observer; no partial output.')
    audit.update(identity);audit.update(density_audit)
    audit.update(previous_display_count=old_count,requested_display_count=target,
                 display_integration_steps=96,classification_integration_steps=48,
                 classification_sample_indices=classification_indices.tolist(),
                 display_duration=float(long_times[-1]-long_times[0]),
                 classification_duration=float(times[-1]-times[0]),
                 seeding_bounds=seeding_bounds,
                 old_seeds_retained=sum(i<old_count for i in density_audit['selected_candidate_indices']),
                 metrics_scope='Denser display cohort; no frozen benchmark F1 attached',
                 line_label_scope='Label of the original 48-step/32-sample primitive, colored along its 96-step continuation')
    audit['vortex_counts']=[int(y.sum()) for y in labels]
    audit['observer_displacement_at_end']=schedule.displacement(long_times[-1]).tolist()
    audit['mean_velocity_over_path_window']=(schedule.displacement(long_times[-1])/(long_times[-1]-long_times[0])).tolist()
    audit['job_id']=os.environ.get('SLURM_JOB_ID')
    audit['node']=os.environ.get('SLURMD_NODENAME')
    print(json.dumps({k:audit[k] for k in ['previous_display_count','requested_display_count','display_duration','classification_duration','old_seeds_retained','candidate_invalid_count','vortex_counts','changed_labels_vs_original','trajectory_correspondence_max_error','common_primitives']},indent=2),flush=True)
