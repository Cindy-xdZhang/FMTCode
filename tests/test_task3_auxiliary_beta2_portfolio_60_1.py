"""Contracts for preregistered Task3 auxiliary-beta2 portfolio 60.1."""
...
import Audit_Task3_AuxiliaryBeta2Portfolio_60_1 as audit_module
...
from Select_Task3_AuxiliaryBeta2Portfolio_60_1 import (
    _canonical_sha256,
...
CONFIG = Path("config/Verify_Task3_AuxiliaryBeta2Portfolio_60.1.yaml")
SOURCE_58_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryRegularizationPortfolio_58.1.yaml"
)
SOURCE_59_CONFIG = Path("config/Verify_Task3_AuxiliaryBeta2_59.1.yaml")


class AuxiliaryBeta2PortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]), {"current_portfolio", "auxiliary_beta2"}
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.208
        )
    ...
    def test_source_config_hashes_are_frozen(self):
        ...
        self.assertEqual(
            _canonical_sha256(SOURCE_58_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_59_CONFIG),
            config["sources"]["auxiliary_beta2"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_is_independent_of_60_selector(self):
        source = inspect.getsource(audit_module)
        self.assertNotIn("Select_Task3_AuxiliaryBeta2Portfolio_60_1", source)
    ...
    def test_evidence_and_cleanup_counts(self):
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_beta2_portfolio_60.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_beta2_59.1_cleanup.sh"
        ).read_text(encoding="utf-8")
