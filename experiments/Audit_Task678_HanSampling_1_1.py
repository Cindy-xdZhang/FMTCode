"""Replay saved predictions and independently check query/primitive/time isolation."""
import argparse
import csv
import json
import math
...
from FMT_Utils.HanFlowMapData_3D import ROLES, TEST_ROLES, measured_trajectories, support_features
...
from experiments.Run_Task678_HanSampling_1_1 import load_role, matrix, derangement_by_window
...
def audit_data(spec, config):
    ...
    root = Path(spec['output_root']); counts = []
    for dataset in spec['datasets']:
        manifest = json.loads((root / 'build' / f'{dataset}.json').read_text())
        assert manifest['status'] == 'PASS' and manifest['config_sha256'] == sha256(config)
        if 'source' in manifest:
            windows = manifest['source']['windows']
            sets = [set(range(w['start_index'], w['end_index']+1)) for w in windows]
            assert all(not sets[i] & sets[j] for i in range(len(sets)) for j in range(i))
            if dataset in spec['cylinder_datasets']:
                assert min(w['time_start'] for w in windows) >= spec['minimum_cylinder_time']
        identities = {}
        for role in ROLES:
            ...
            identities[role] = {}
            for i in range(n):
                identity = (int(data['window_ordinal'][i]), int(data['material_bundle_ids'][i]))
                identities[role][identity] = (set(data['query_ids0'][i]), set(data['query_ids1'][i]))
                if role in ('fit', 'query_validation', 'query_test'):
                    assert identity[0] in spec['splits']['train'] and identity[1] % 8 < 5
                elif role == 'primitive_validation':
                    assert identity[0] in spec['splits']['train'] and identity[1] % 8 == 5
                elif role == 'primitive_test':
                    assert identity[0] in spec['splits']['train'] and identity[1] % 8 >= 6
                else:
                    assert identity[0] in spec['splits'][role.removeprefix('time_')]
            assert len(identities[role]) == n
            expected_q = spec['sampling'][{'fit':'train_queries', 'query_validation':'validation_queries'}.get(role, 'test_queries')]
            assert data['target0'].shape[1:] == (expected_q, 63, 3)
            assert data['target1'].shape[1:] == (expected_q, 63, 3)
            assert data['target_long'].shape[1:] == (expected_q, 125, 3)
            np.testing.assert_array_equal(data['target0'], data['target_long'][:, :, :63])
            arrival = (data['target_long'][:, :, 62] - data['origin1'][:, None]) / data['radius1'][:, None, None]
            assert (np.abs(arrival).sum(-1) <= spec['sampling']['query_extent'] + 1e-4).all()
            spread = np.linalg.norm(data['support0'][:, :, -1] - data['origin1'][:, None], axis=-1).max(1)
            np.testing.assert_allclose(data['radius1'], np.maximum(data['radius0'], spec['sampling']['second_radius_factor']*spread), rtol=1e-4, atol=1e-6)
            for i in range(n):
                visible = data['context'][i, :, :, 0].reshape(-1, 3)
                gap = np.linalg.norm(visible[:, None] - data['target0'][i, :, 0][None], axis=-1).min()
                assert gap > data['radius0'][i]
            # Recompute every frozen feature from visible paths, without hidden truth.
            features = support_features(data)
            for k, v in features.items():
                np.testing.assert_array_equal(v, data[k])
            counts.append(dict(dataset=dataset, role=role, regions=n, queries=expected_q, source_records=sources))
        fit = identities['fit']
        for role in ('query_validation', 'query_test'):
            assert fit.keys() == identities[role].keys()
        for identity, queries in fit.items():
            for stage in (0, 1):
                groups = [identities[role][identity][stage] for role in ('fit', 'query_validation', 'query_test')]
                assert all(not groups[i] & groups[j] for i in range(3) for j in range(i))
        for role in ('primitive_validation', 'primitive_test', 'time_validation', 'time_test'):
            assert not fit.keys() & identities[role].keys()
        assert not identities['primitive_validation'].keys() & identities['primitive_test'].keys()
        print('DATA AUDIT PASS', dataset, flush=True)
    write_json(root / 'data_audit.json', {**provenance(config), 'status':'PASS', 'counts':counts})


def audit(spec, config):
    spec['_config_sha256'] = sha256(config)
    root = Path(spec['output_root']); rows = []
    assert json.loads((root / 'data_audit.json').read_text())['status'] == 'PASS'
    for dataset in spec['datasets']:
        arms = [(arm, seed) for seed in spec['seeds'] for arm in spec['neural_arms']] + [('affine', 0)]
        for arm, seed in arms:
            ...
            assert done['metric_records'] == (30 if arm in ('fmt_all', 'raw_positions') else 21)
            if arm != 'affine':
                for task in ('Task6', 'Task7'):
                    ...
                    assert train['input_dimension'] == (672 if arm == 'raw_positions' else 161)
                    ...
                    for task in ('Task6', 'Task7', 'Task8'):
                        ...
                        with np.load(path, allow_pickle=False) as archive:
                            ...
                            if task == 'Task8':
                                ...
                                if arm != 'affine' and variant == 'normal':
    ...
    print('HAN AUDIT PASS',len(rows),'records',flush=True)
...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--config',default=DEFAULT_CONFIG); p.add_argument('--data-only',action='store_true')
    a=p.parse_args(); spec=json.loads(Path(a.config).read_text())
    audit_data(spec,a.config) if a.data_only else audit(spec,a.config)
