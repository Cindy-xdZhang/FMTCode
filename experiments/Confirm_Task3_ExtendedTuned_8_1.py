"""Sealed Task3 confirmation of the training-free 54.1 portfolio.

The selected seed-40/41 models are frozen by content hash before the seventh
spatial primitive population is generated. Confirmation performs no training
or model, feature, threshold, residual-scale, or hyperparameter selection.
"""
...
import Confirm_Task3_AdaptiveTuned_7_2 as _base


SOURCE_EXPERIMENT = "Verify_Task3_ExtendedPortfolio_54.1"


@contextmanager
def _configured_base():
    previous_spatial = _base.spatial
    previous_source = _base.SOURCE_EXPERIMENT
    try:
        _base.spatial = spatial
        _base.SOURCE_EXPERIMENT = SOURCE_EXPERIMENT
        yield
    finally:
        _base.spatial = previous_spatial
        _base.SOURCE_EXPERIMENT = previous_source
...
def summarize(config_path: str | Path) -> Path:
    ...
    if payload.get("experiment") != spatial.EXPERIMENT:
        raise RuntimeError("Task3 8.1 summary experiment changed")
    if payload.get("source_search_experiment") != SOURCE_EXPERIMENT:
        raise RuntimeError("Task3 8.1 summary source portfolio changed")
    payload["comparison"] = (
        "frozen 54.1 extended-portfolio FMT residual minus its same-recipe, "
...
def _decode_dataset(spec: dict, index: int) -> str:
    if not 0 <= int(index) < len(spec["datasets"]):
        raise IndexError("Task3 8.1 dataset job outside [0,10)")
