"""Contracts for preregistered Task3 projection-activation portfolio 88.1."""
...
import Audit_Task3_AuxiliaryProjectionActivationPortfolio_88_1 as audit_module
...
from Select_Task3_AuxiliaryProjectionActivationPortfolio_88_1 import (
    _canonical_sha256,
...
SOURCE_86_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryLinearBiasScalePortfolio_86.1.yaml"
)
SOURCE_87_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryProjectionActivation_87.1.yaml"
)


class AuxiliaryProjectionActivationPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "auxiliary_projection_activation"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.226
        )
        self.assertEqual(spec["selection"]["target_absolute_fmt_f1"], 0.896)
        self.assertFalse(spec["confirmation_opened"])

    def test_source_config_hashes_are_frozen(self):
        config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
        self.assertEqual(
            _canonical_sha256(SOURCE_86_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_87_CONFIG),
            config["sources"]["auxiliary_projection_activation"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_and_cleanup_contract(self):
        ...
        self.assertNotIn(
            "Select_Task3_AuxiliaryProjectionActivationPortfolio_88_1",
            source,
        )
        self.assertNotIn(
        ...
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_projection_activation_portfolio_88.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_projection_activation_87.1_cleanup.sh"
        ).read_text(encoding="utf-8")
