...
def runtime(config,phase,state,exit_code=None):
    ...
    device='CPU'
    if phase in ('baseline','screen','refine','final'):
        import torch
...
def submit(mode,config):
    ...
    for phase,cpu,mem,limit,array,gpu in stages:
        command=['sbatch','--parsable',f'--job-name=t6-pnn13-{phase}','--nodes=1','--ntasks=1',
        ...
        command += ['ibex_bash/task6_pnn_data_schedule_1p3.sh',phase,config]
...
if __name__=='__main__':
    ...
    parser.add_argument('--config',default='config/Verify_Task6_PNNTrans_1.3.json')
