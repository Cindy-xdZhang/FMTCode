...
|---|---|---:|---:|
| Task1 | fmt_v5−原FMT | +0.003399±0.000256 | 5/10 |
| Task1 | 距离+夹角−距离 | +0.016920±0.003618 | 6/10 |
| Task2 | fmt_v5−原FMT | -0.043361±0.025972 | 3/10 |
| Task2 | 距离+夹角−距离 | +0.128779±0.012080 | 8/10 |
| Task3 | fmt_v5−原FMT | -0.014613±0.005538 | 1/10 |
| Task3 | 距离+夹角−距离 | +0.011787±0.004321 | 8/10 |
| Task5 | fmt_v5−原FMT | -0.008440±0.002861 | 3/10 |
...
fmt_v5相对各自父版本：平均F1提高的任务为Task1，降低的任务为Task2、Task3、Task5。
距离+夹角版相对各自父版本：平均F1提高的任务为Task1、Task2、Task3、Task5，降低的任务为无。

以上判断只针对本版15条中心夹角的六频率表示与既定训练流程，不说明全部客观特征有效或无效，也不能把当前差异归因于未检验的单独机制。原FMT扩展没有在四任务上一致提高，Task2的下降保留。Task3/5含Raw输入，不能由编码器客观性推断整个网络客观。未根据本次测试改变数据、频率、模型、阈值选择规则或训练预算。
