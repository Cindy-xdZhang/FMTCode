...
- **Task3（2D/3D）**：加入 FMT 是否提高 IVD 标签监督的涡/非涡二分类性能。当前 3D 主表为逐 physical-family 冻结的 FMT residual 与同结构、同容量 Raw-PCA residual 的 `mainExp_Task3_3D_8.1`；`mainExp_Task3_3D_7.2` 是同一改进方法在另一套未见空间 population 上的独立复现。
...
## 复核 Task3 论文主表（mainExp_Task3_3D_8.1）

在缓存、标签和冻结 checkpoint 已存在时，评估脚本不会训练、选 checkpoint、调整 residual 权重或重选阈值：

```bash
python Confirm_Task3_ExtendedTuned_8_1.py --config config/mainExp_Task3_3D_8.1.yaml --mode summary
```

最终 `per_run.csv` 与 `summary.json` 位于 `outputs/mainExp_Task3_3D_8.1/`；本地无模型证据包位于 `output/mainExp_Task3_3D_8.1_ibex/`。相对同结构、同容量 Raw-PCA residual，FMT 的 dataset-macro F1/Average Precision 增益为 `+.18005/+.19991`，10/10条目、7/7 physical family和2/2 paired seeds均为正增益。独立的第六空间population `mainExp_Task3_3D_7.2` 得到F1增益`+.17522`，因此论文结论不依赖单次空间采样。这里验证的是 IVD 涡区域二分类，不是 3D 涡类型分类。
