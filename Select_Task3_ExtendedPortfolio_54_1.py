"""Freeze the guarded Task3 winner across six development searches.

This stage trains nothing and never reads confirmation data.  It extends the
validated 52.1 portfolio with the independently preregistered 53.1 auxiliary-
dropout search, then copies the selected seed-40/41 result and checkpoint
artifacts before any source cleanup.
"""
...
def _load_spec(path: str | Path) -> dict:
    ...
    if missing:
        raise ValueError(f"missing Task3 54.1 config keys: {missing}")
    if spec["experiment"] != EXPERIMENT:
        raise ValueError("Task3 54.1 experiment identity changed")
    if spec["task"] != "Task3" or bool(spec["confirmation_opened"]):
        raise ValueError("Task3 54.1 must remain development-only")
    if set(spec["sources"]) != EXPECTED_SOURCES:
        raise ValueError("Task3 54.1 source portfolio changed")

    datasets = [str(value) for value in spec["datasets"]]
    if len(datasets) != 10 or len(set(datasets)) != 10:
        raise ValueError("Task3 54.1 requires ten unique datasets")
    if [int(value) for value in spec["source_paired_seeds"]] != [40, 41, 42]:
        raise ValueError("Task3 54.1 source seeds changed")
    if [int(value) for value in spec["frozen_confirmation_seeds"]] != [40, 41]:
        raise ValueError("Task3 54.1 confirmation seeds changed")
    ...
    if len(metrics) != len(set(metrics)):
        raise ValueError("Task3 54.1 selection metrics are duplicated")
    if not bool(selection["require_source_absolute_fmt_guard"]):
        raise ValueError("Task3 54.1 requires guarded source winners")
...
def static_preflight(config_path: str | Path) -> dict:
    ...
    if report["source_count"] != 6 or report["training_runs"] != 0:
        raise RuntimeError("Task3 54.1 static preflight contract changed")
    return report


def source_identity_preflight(config_path: str | Path) -> Path:
    """Verify six deployed source configs without opening result artifacts."""
    ...
    if target.exists():
        if json.loads(target.read_text(encoding="utf-8")) != payload:
            raise RuntimeError("Task3 54.1 source identity preflight changed")
    else:
        target.write_text(
...
def _freeze_local_copies(selection_path: Path) -> Path:
    ...
    if payload.get("experiment") != EXPERIMENT:
        raise RuntimeError("extended portfolio identity changed")
    models = list(payload.get("models", []))
    if len(models) != 40:
        raise RuntimeError("Task3 54.1 must freeze exactly 40 models")
    ...
    payload["selection_rule"] = (
        ...
        "across preregistered 44.1, 45.1, 48.1, 50.1, 51.1, and 53.1 "
        "winners; use the registered absolute-FMT guards and tie breakers"
    )
