"""Contracts for preregistered Task3 auxiliary-beta1 portfolio 62.1."""
...
import Audit_Task3_AuxiliaryBeta1Portfolio_62_1 as audit_module
...
from Select_Task3_AuxiliaryBeta1Portfolio_62_1 import (
    _canonical_sha256,
...
CONFIG = Path("config/Verify_Task3_AuxiliaryBeta1Portfolio_62.1.yaml")
SOURCE_60_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryBeta2Portfolio_60.1.yaml"
)
SOURCE_61_CONFIG = Path("config/Verify_Task3_AuxiliaryBeta1_61.1.yaml")


class AuxiliaryBeta1PortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]), {"current_portfolio", "auxiliary_beta1"}
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.209
        )
    ...
    def test_source_config_hashes_are_frozen(self):
        ...
        self.assertEqual(
            _canonical_sha256(SOURCE_60_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_61_CONFIG),
            config["sources"]["auxiliary_beta1"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_is_independent_of_62_selector(self):
        source = inspect.getsource(audit_module)
        self.assertNotIn("Select_Task3_AuxiliaryBeta1Portfolio_62_1", source)
    ...
    def test_evidence_and_cleanup_counts(self):
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_beta1_portfolio_62.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_beta1_61.1_cleanup.sh"
        ).read_text(encoding="utf-8")
