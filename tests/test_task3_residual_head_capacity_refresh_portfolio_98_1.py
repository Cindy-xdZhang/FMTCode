"""Contracts for preregistered residual-head capacity portfolio 98.1."""
...
import Audit_Task3_ResidualHeadCapacityRefreshPortfolio_98_1 as audit_module
...
from Select_Task3_ResidualHeadCapacityRefreshPortfolio_98_1 import (
    _canonical_sha256,
...
SOURCE_96_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryActivationResidualGainPortfolio_96.1.yaml"
)
SOURCE_97_CONFIG = Path(
    "config/Verify_Task3_ResidualHeadCapacityRefresh_97.1.yaml"
)


class ResidualHeadCapacityRefreshPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "residual_head_capacity_refresh"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.236
        )
        self.assertEqual(spec["selection"]["target_absolute_fmt_f1"], 0.901)
        self.assertFalse(spec["confirmation_opened"])

    def test_source_config_hashes_are_frozen(self):
        config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
        self.assertEqual(
            _canonical_sha256(SOURCE_96_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_97_CONFIG),
            config["sources"]["residual_head_capacity_refresh"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_and_cleanup_contract(self):
        ...
        self.assertNotIn(
            "Select_Task3_ResidualHeadCapacityRefreshPortfolio_98_1", source
        )
        self.assertNotIn(
        ...
        evidence = Path(
            "ibex_bash/verify_task3_residual_head_capacity_refresh_portfolio_98.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_residual_head_capacity_refresh_97.1_cleanup.sh"
        ).read_text(encoding="utf-8")
        self.assertIn('result_count" -ne 40', evidence)
        self.assertIn('total_count" -ne 80', evidence)
        self.assertIn('checkpoint_count_before" -ne 780', cleanup)
