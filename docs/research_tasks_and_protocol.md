...
## 2b. Task4-c 固定线簇数据集 v1（2026-09-18 起长期固定）

用户 2026-09-18 裁定：Task4-c 以后不再因为改变邻居数量、邻居选择方式或“中心是否固定”而重建数据集。数据集一次建好、长期只读；改变编码/网络的实验只在读取阶段选择所需的线。**规则的规范文本是 [Task4C_fixed_dataset_rules_v1.md](Task4C_fixed_dataset_rules_v1.md)**，实现与作业记录见 [Task4C_fixed_dataset_protocol_1.1.md](Task4C_fixed_dataset_protocol_1.1.md)。本节只列要点，与规范文本不一致时以规范文本为准。

- **样本＝采样点。** 中心线由采样点积分得到；3×3×3 模板（0.25h/0.5h/1h）的 26 个邻居播种点积分得到邻居线；线束是采样点的派生物，采样点与线束成对保存。
- **step1 只筛中心**（λ₂<阈值 ∧ oyf>0；候选区域内的种子还要同一连通块，为覆盖专门放进 hairpin 实例的种子还要在标注单元内且满足头部夹角）。26 个邻居只要求在计算域内。step1 只产生候选点，不决定标签。
- **标签。** 每行只有一条规则：中心点落在 hairpin 标注单元内为 1（记编号，0 有效），否则为 0。
- **step2/step3。** 沿涡量场 RK45 双向积分（Channel 单向 0.08/0.10/0.12，TBL 4/5/6），逐线清理（半线弧长 ∈[0.95L,1.002L]，32 点重采样）。保留条件：中心线通过且 ≥16 条邻居通过（≥17 线），否则丢弃重采。
- **step4 放松。** 同一来源连续 100 次失败后放松中心的 λ₂/oyf 筛选，放松样本标签按中心点 GT 归属，标 `relaxed`；再 1000 次报错。
- **实例覆盖与划分。** “覆盖”＝某划分在该 hairpin 实例头部区域（GT 体积内 ∧ |cos(速度,涡量)|²<0.5）有 ≥2 个采样点，构建目标每实例 10 个。实例按流场固定种子分 80% 训练覆盖组 / 20% 未见组（重叠包围盒同组）；训练覆盖每个覆盖组实例，测试覆盖全部实例（覆盖组用头部区域的偏移点；**未见组直接在该实例 GT 包围盒内均匀随机取，不加任何筛选**，标签按中心 GT 归属）；未见组实例不进训练/验证（排除区＝GT 包围盒，不外扩）；非 hairpin 种子不分组。
- **距离与顺序。** 评价点（验证/测试）距任意训练中心 ≥1h；覆盖组实例的 hairpin 测试点距同实例训练中心 ≤6h（“轻微偏移”，靠在测试点旁 1h–6h 处放一个训练点保证）；测试距验证 ≥0.5h；(中心,尺度) 同划分唯一。生成顺序 test→validation→train。
