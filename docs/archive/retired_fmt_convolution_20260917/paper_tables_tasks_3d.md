...
cluster 映射或阈值。Task1、Task3的论文主表（Task2已退出指标主表）分别使用一套跨全部10个数据
条目不变的任务级配置；Task5 固定一套 FMT encoder，仅按其预注册研究问题改变
pathline 尺度。Task2全部结果仅为弃用档案；Task3-8.1的逐physical-family配方保留为历史补充证据，
...
| Task2 | 当前为直接特征交互可视化，不设指标主结果 | 旧FMT＋VAE及IVD指标全部弃用；见Task2特征可视化2.1协议 |
...
| Task2 | 默认直接p35/n0_k06；比较primitive VAE、Raw＋PCA、Raw等 | 同一批primitive | 可视化探索，无指标主表；旧6.2退役 |
...
## 已弃用的Task2历史：Raw+VAE 与 FMT+同一 VAE
...
## 已弃用的Task2历史补充：逐physical-family Raw+VAE 与 FMT+同一 VAE
