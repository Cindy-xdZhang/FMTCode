"""Contracts for paired auxiliary activation-input-shift search 91.1."""
...
CONFIG = "config/Verify_Task3_AuxiliaryActivationInputShift_91.1.yaml"
SHIFTS = [None, -2.0, -1.0, -0.5, -0.25, 0.25, 0.5, 1.0, 2.0]
...
class AuxiliaryActivationInputShiftTests(unittest.TestCase):
    def test_registered_grid_targets_and_job_count(self):
        ...
        self.assertEqual(
            ...
            ["model.auxiliary_projection_activation_input_shift"],
        )
        selection = spec["optimization_selection"]
        self.assertEqual(selection["target_dataset_macro_f1_gain"], 0.230)
        self.assertEqual(selection["target_absolute_fmt_f1"], 0.898)
        self.assertFalse(selection["confirmation_opened"])

    def test_grid_changes_only_registered_input_shift(self):
        spec = _load_optimization_spec(CONFIG)
        observed = []
        ...
        self.assertEqual(observed, SHIFTS)

    def test_source_recipe_is_preserved_except_input_shift(self):
        spec = _load_optimization_spec(CONFIG)
        ...
        for rows in resolved.values():
            self.assertNotIn(
                "auxiliary_projection_activation_input_shift", rows[0]["model"]
            )
            self.assertEqual(
    ...
    def test_default_is_exact_no_op(self):
        ...
        torch.manual_seed(7068)
        explicit = _model(auxiliary_projection_activation_input_shift=0.0)
        explicit_next = torch.rand(5)
        self.assertEqual(
            explicit.auxiliary_projection_activation_input_shift_layer_count,
            0,
        )
        self.assertFalse(any(
            isinstance(child, _ActivationInputShift)
    ...
    def test_candidate_wraps_only_activations_without_rng_or_capacity_shift(self):
        ...
        torch.manual_seed(17)
        candidate = _model(auxiliary_projection_activation_input_shift=0.5)
        ...
        self.assertEqual(
            candidate.auxiliary_projection_activation_input_shift_layer_count,
            2,
        )
        self.assertEqual(len(wrappers), 2)
        self.assertTrue(all(child.shift == 0.5 for child in wrappers))
    ...
    def test_input_scaling_precedes_input_shift(self):
        model = _model(
    ...
    def test_blockwise_wraps_every_branch_activation(self):
        ...
        self.assertEqual(
            model.auxiliary_projection_activation_input_shift_layer_count, 4
        )

    def test_invalid_and_activation_free_shifts_are_rejected(self):
        for value in (float("nan"), float("inf"), float("-inf")):
            with self.assertRaisesRegex(ValueError, "must be finite"):
                _model(auxiliary_projection_activation_input_shift=value)
    ...
    def test_model_kwargs_and_evidence_contract(self):
        ...
        gpu = Path(
            "ibex_bash/verify_task3_auxiliary_activation_input_shift_91.1_gpu.sh"
        ).read_text(encoding="utf-8")
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_activation_input_shift_91.1_evidence.sh"
        ).read_text(encoding="utf-8")
