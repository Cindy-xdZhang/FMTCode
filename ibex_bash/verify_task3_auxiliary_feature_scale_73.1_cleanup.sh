...
repo_root="${TASK373_REPO_ROOT:-/home/zhanx0o/FMT_Task3_AuxiliaryFeatureScale_73_1}"
portfolio_root="${TASK374_REPO_ROOT:-/home/zhanx0o/FMT_Task3_AuxiliaryFeatureScalePortfolio_74_1}"
cd "$repo_root"
mkdir -p slurm_logs

output_root="$repo_root/outputs/Verify_Task3_AuxiliaryFeatureScale_73.1"
...
portfolio="$portfolio_root/outputs/Verify_Task3_AuxiliaryFeatureScalePortfolio_74.1/portfolio_selection.json"
portfolio_audit="$portfolio_root/outputs/Verify_Task3_AuxiliaryFeatureScalePortfolio_74.1/independent_audit.json"
...
expected_output_root="$resolved_outputs/Verify_Task3_AuxiliaryFeatureScale_73.1"
...
if [[ "$resolved_output_root" != "$expected_output_root" ]] || \
  ...
  echo "refusing cleanup outside the exact resolved 73.1 output root: $resolved_candidates" >&2
  exit 1
fi
...
if evidence.get("status") != "passed" or int(
    ...
    raise SystemExit("73.1 evidence archive is incomplete")
if hashlib.sha256(archive_path.read_bytes()).hexdigest() != str(
    evidence.get("stable_archive_sha256", "")
):
    raise SystemExit("73.1 archive SHA-256 differs")
if source_audit.get("status") != "passed":
    raise SystemExit("73.1 independent audit did not pass")
if audit.get("status") != "passed" or not audit.get("all_frozen_hashes_verified"):
    raise SystemExit("74.1 independent portfolio audit did not pass")
...
if [[ "$checkpoint_count_before" -ne 660 ]]; then
  echo "expected exactly 660 temporary 73.1 checkpoints, found $checkpoint_count_before" >&2
  exit 1
fi
...
if [[ "$checkpoint_count_after" -ne 0 ]]; then
  echo "73.1 checkpoint cleanup incomplete: $checkpoint_count_after remain" >&2
  exit 1
fi
...
Path(sys.argv[1]).write_text(json.dumps({
    "experiment": "Verify_Task3_AuxiliaryFeatureScale_73.1",
    "job_id": os.environ.get("SLURM_JOB_ID", ""),
    "checkpoint_count_before": int(sys.argv[2]),
    "checkpoint_count_after": int(sys.argv[3]),
    "reason": "74.1 frozen portfolio and independent audit complete",
    "status": "passed",
