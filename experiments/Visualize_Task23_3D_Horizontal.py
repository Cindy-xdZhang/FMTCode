...
try:
    ...
    reconstruct_ivd_reference_surface = None
...
PANEL_ZOOM = 1.12
CLOSEUP_PANEL_ZOOM = 1.20
VERTICAL_CROP_PAD_PIXELS = 24
# Synthetic points are forbidden: all three panels must use exactly the same
# evaluated seed array that was stored in the prediction artifact.
POINT_DENSITY_MULTIPLIER = 1.0
IVD_SURFACE_CACHE = ROOT / "outputs/IVD_reference_surface_cache"
REFERENCE_SURFACE_ALPHA = 0.28
CLOSEUP_POSITIVE_FRACTION = 0.035
CLOSEUP_MINIMUM_POSITIVES = 12
CLOSEUP_PADDING_FACTOR = 1.08
CLOSEUP_MINIMUM_SPAN_FRACTION = 0.025
...
def _prepare_axis(ax, bounds, view, panel_zoom=PANEL_ZOOM):
    if paper is None:
        raise RuntimeError(
            "rendering requires experiments/Visualize_Task1_3D_PaperCandidates.py"
        )
    paper._set_physical_axes(ax, bounds, view)
    span = np.maximum(np.asarray(bounds)[1] - np.asarray(bounds)[0], 1e-12)
    ax.set_box_aspect(span, zoom=float(panel_zoom))
...
def _validate_render_payload(payload, point_density_multiplier=1.0):
    """Enforce the one-seed/one-label/one-prediction rendering contract."""
    if not np.isclose(float(point_density_multiplier), 1.0, rtol=0, atol=0):
        raise ValueError(
            "synthetic point densification is forbidden; "
            "point_density_multiplier must be exactly 1"
        )
    seeds = np.asarray(payload["seeds"], dtype=np.float32)
    reference = np.asarray(payload["reference"], dtype=bool)
    predictions = [np.asarray(value, dtype=bool) for value in payload["predictions"]]
    ...
    if reference.shape != (len(seeds),):
        raise ValueError("reference must contain exactly one label per seed")
    if len(predictions) != 2:
        raise ValueError("a triptych requires exactly two prediction arrays")
    if any(value.shape != reference.shape for value in predictions):
        raise ValueError("every prediction must contain exactly one label per seed")
    if payload.get("ivd_mesh") is None:
        raise ValueError("an audited whole-field IVD-p95 isosurface is required")
    bounds = np.asarray(payload["bounds"], dtype=np.float64)
    if bounds.shape != (2, 3) or not np.all(bounds[1] > bounds[0]):
        raise ValueError("physical bounds must have shape [2,3] and positive spans")
    return seeds, reference, predictions


def _confusion_masks(reference, prediction):
    """Return a strict, disjoint TP/FP/FN/TN partition for the same seeds."""
    reference = np.asarray(reference, dtype=bool)
    prediction = np.asarray(prediction, dtype=bool)
    if reference.ndim != 1 or prediction.shape != reference.shape:
        raise ValueError("reference and prediction must be same-length 1D arrays")
    masks = {
    ...
    memberships = np.stack(list(masks.values()), axis=0).sum(axis=0)
    if not np.all(memberships == 1):
        raise RuntimeError("TP/FP/FN/TN do not form an exact seed partition")
    if np.any(masks["false_positive"] & reference):
        raise RuntimeError("a ground-truth-positive seed was marked false positive")
    if np.any(masks["false_negative"] & ~reference):
        raise RuntimeError("a ground-truth-negative seed was marked false negative")
    return masks


def _closeup_bounds(bounds, seeds, reference):
    """Choose one shared close-up using IVD labels only.

    Predictions are deliberately excluded so Raw and FMT cannot move the
    camera toward a convenient success.  The coarse whole-field artifacts are
    not tightened further here: doing so leaves only one to three regular-grid
    points.  Genuine close-ups use experiments/Visualize_Task23_Multiflow_DenseCloseup.py
    and its 20,480 newly integrated local Sobol primitives.
    """
    if _vortex_roi is None:
        raise RuntimeError(
            "close-up rendering requires experiments/Visualize_Task3_F22Anchored.py"
        )
    return _vortex_roi(
        bounds,
        seeds,
        reference,
        positive_fraction=CLOSEUP_POSITIVE_FRACTION,
        minimum_points=CLOSEUP_MINIMUM_POSITIVES,
        padding_factor=CLOSEUP_PADDING_FACTOR,
        minimum_span_fraction=CLOSEUP_MINIMUM_SPAN_FRACTION,
        exact_target_span=False,
        center_on_selected_neighbourhood=True,
    )


def _count_blue_like_pixels(rgb):
    """Count saturated blue/cyan pixels in a rendered RGB image."""
    ...
    return int(np.count_nonzero(blue))


def _visible_mask(seeds, bounds):
    if bounds is None:
        return np.ones(len(seeds), dtype=bool)
    bounds = np.asarray(bounds, dtype=np.float64)
    return np.all(
        (np.asarray(seeds) >= bounds[0])
        & (np.asarray(seeds) <= bounds[1]),
        axis=1,
    )


def _add_ivd_surface(ax, mesh, bounds=None, alpha=REFERENCE_SURFACE_ALPHA):
    """Draw the IVD surface, clipping it to the close-up cuboid when needed."""
    if bounds is None:
        paper._add_ivd_surface(ax, mesh, float(alpha))
        return
    if _clipped_mesh_triangles is None:
        raise RuntimeError(
            "close-up rendering requires experiments/Visualize_Task3_F22Anchored.py"
        )
    triangles = _clipped_mesh_triangles(mesh, np.asarray(bounds, dtype=np.float64))
    if not len(triangles):
        raise RuntimeError("close-up crop contains no IVD isosurface triangle")
...
def _draw_confusion(
    ax, seeds, reference, prediction, *, bounds=None,
    ...
    visible = _visible_mask(seeds, bounds)
    categories = [
        (masks["true_positive"], paper.COLORS["vortex"], "o", 13.0, 0.92),
        (masks["false_positive"], paper.COLORS["false_positive"], "^", 16.0, 0.94),
        (masks["false_negative"], paper.COLORS["false_negative"], "x", 22.0, 0.98),
    ]
    if show_true_negatives:
        categories.insert(
            0,
            (masks["true_negative"], paper.COLORS["non_vortex"], "o", 2.2, 0.12),
        )
...
def _draw_reference(
    ax, seeds, reference, ivd_mesh, *, bounds=None, show_background=True,
):
    if paper is None:
        raise RuntimeError(
            "rendering requires experiments/Visualize_Task1_3D_PaperCandidates.py"
    ...
    visible = _visible_mask(seeds, bounds)
    _add_ivd_surface(
        ax, ivd_mesh, bounds=bounds, alpha=REFERENCE_SURFACE_ALPHA
    )
    layers = [(reference, paper.COLORS["vortex"], 13.0, 0.92)]
    if show_background:
        layers.insert(
            0, (~reference, paper.COLORS["non_vortex"], 2.2, 0.12)
        )
    for mask, color, size, alpha in layers:
        mask = np.asarray(mask, dtype=bool) & visible
        ax.scatter(
    ...
    return {
        "ground_truth_negative": int(np.count_nonzero(~reference & visible)),
        "ground_truth_positive": int(np.count_nonzero(reference & visible)),
    }


def _render_comparison(
    dataset, task, payload, output_dir, dpi, point_density_multiplier,
    view_mode="full",
):
    spec = FLOW_SPECS[dataset]
    seeds, reference, predictions = _validate_render_payload(
        payload, point_density_multiplier
    )
    if view_mode not in {"full", "closeup"}:
        raise ValueError("view_mode must be full or closeup")
    render_bounds = (
        np.asarray(payload["bounds"], dtype=np.float64)
        if view_mode == "full" else
        _closeup_bounds(payload["bounds"], seeds, reference)
    )
    # The regular true-negative lattice obscures vortex geometry and is not a
    # necessary visual encoding: every metric still uses all negatives.  Both
    # whole-domain and close-up plates therefore render foreground/error marks
    # only.
    show_background = False
    panel_zoom = PANEL_ZOOM if view_mode == "full" else CLOSEUP_PANEL_ZOOM
    fig, axes, rectangles = _new_comparison_figure()
    counts = [
        _draw_reference(
            axes[0], seeds, reference, payload["ivd_mesh"],
            bounds=None if view_mode == "full" else render_bounds,
            show_background=show_background,
        )
    ]
    _prepare_axis(
        axes[0], render_bounds, spec["view"], panel_zoom=panel_zoom
    )
    for axis, prediction in zip(axes[1:], predictions):
        counts.append(
            _draw_confusion(
                axis, seeds, reference, prediction,
                bounds=None if view_mode == "full" else render_bounds,
                show_true_negatives=show_background,
            )
        )
        _prepare_axis(
            axis, render_bounds, spec["view"], panel_zoom=panel_zoom
        )
    suffix = "horizontal_clean" if view_mode == "full" else "closeup_clean"
    path = output_dir / f"{dataset}_{task}_{suffix}.png"
    ...
    with Image.open(path) as image:
        ...
        blue_like_pixel_count = _count_blue_like_pixels(np.asarray(rgb))
    if blue_like_pixel_count:
        raise RuntimeError(
            f"{dataset}/{task}: rendered {blue_like_pixel_count} blue-like "
            "pixels although the true-negative background layer is disabled"
        )
    sampling = {
        "method": "exact evaluated seeds; no interpolation or densification",
        "original_point_count": int(len(seeds)),
        "rendered_point_count": int(len(seeds)),
        "density_multiplier": 1.0,
        "metrics_use_original_points_only": True,
        "all_panels_use_identical_seed_coordinates": True,
        "view_mode": view_mode,
        "render_bounds": render_bounds.tolist(),
        "blue_true_negative_layer_rendered": False,
        "rendered_blue_like_pixel_count": blue_like_pixel_count,
        "closeup_focus_uses_predictions": False,
        "closeup_uses_dense_local_resampling": False,
    }
    return path, rectangles, pixel_size, counts, sampling


def _attach_ivd_reference(dataset, payload):
    if reconstruct_ivd_reference_surface is None:
        raise RuntimeError(
            "rendering requires experiments/VisualizationIVDReference_3D.py"
        )
    reconstructed = reconstruct_ivd_reference_surface(
        ROOT,
        dataset,
        payload["metadata"],
        payload["seeds"],
...
def _record(dataset, task, payload, image_info):
    ...
    return {
        ...
        "camera": {
            ...
            "physical_bounds": sampling["render_bounds"],
            "panel_zoom": (
                PANEL_ZOOM
                if sampling["view_mode"] == "full" else
                CLOSEUP_PANEL_ZOOM
            ),
            "focus_basis": (
                "whole physical domain"
                if sampling["view_mode"] == "full" else
                "IVD-positive labels only; predictions excluded"
            ),
        },
...
def run(
    ...
    for task in requested_tasks:
        ...
        metadata = {
            ...
            "layout": (
                "one compact 1x3 whole-domain comparison image per flow"
                if view_mode == "full" else
                "one compact 1x3 IVD-only close-up image per flow"
            ),
            ...
            "annotation_policy": {
                "retained": [
                    ...
                    "x/y/z axis labels",
                    "coordinate ticks",
                    "audited whole-field IVD-p95 isosurface",
                    "exact IVD seed labels",
                    "TP/FP/FN spatial marks; true-negative blue layer omitted",
                ],
                "removed": [
                    "pathline/IVD scene overview",
                    "true-negative blue point lattice",
                    "synthetic nearest-neighbor point densification",
                    "figure titles",
                    "panel titles",
                    "legends",
                    "colorbars",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--view-mode", choices=("full", "closeup"), default="full",
        help=(
            "closeup uses an IVD-label-only local crop and structurally omits "
            "the blue true-negative point layer"
        ),
    )
    parser.add_argument(
        ...
        help="Must remain 1; synthetic visualization points are forbidden.",
    )
