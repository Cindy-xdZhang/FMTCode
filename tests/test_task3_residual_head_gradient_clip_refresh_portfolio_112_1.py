"""Contracts for residual-head gradient-clipping portfolio 112.1."""
...
import Audit_Task3_ResidualHeadGradientClipRefreshPortfolio_112_1 as audit_module
...
from Select_Task3_ResidualHeadGradientClipRefreshPortfolio_112_1 import (
    _canonical_sha256,
...
CONFIG = Path(
    "config/Verify_Task3_ResidualHeadGradientClipRefreshPortfolio_112.1.yaml"
)
SOURCE_110_CONFIG = Path(
    "config/Verify_Task3_ResidualHeadEpsilonRefreshPortfolio_110.1.yaml"
)
SOURCE_111_CONFIG = Path(
    "config/Verify_Task3_ResidualHeadGradientClipRefresh_111.1.yaml"
)


class ResidualHeadGradientClipRefreshPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "residual_head_gradient_clip_refresh"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.250
        )
        self.assertEqual(spec["selection"]["target_absolute_fmt_f1"], 0.908)
        self.assertFalse(spec["confirmation_opened"])

    def test_source_config_hashes_are_frozen(self):
        config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
        self.assertEqual(
            _canonical_sha256(SOURCE_110_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_111_CONFIG),
            config["sources"]["residual_head_gradient_clip_refresh"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_and_cleanup_contract(self):
        ...
        self.assertNotIn(
            "Select_Task3_ResidualHeadGradientClipRefreshPortfolio_112_1",
            source,
        )
        self.assertNotIn(
        ...
        evidence = Path(
            "ibex_bash/verify_task3_residual_head_gradient_clip_refresh_portfolio_112.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_residual_head_gradient_clip_refresh_111.1_cleanup.sh"
        ).read_text(encoding="utf-8")
