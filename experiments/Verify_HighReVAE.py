...
from experiments.Run_Task2_Universality import _fit_cluster, _load_slices, _prepare
from experiments.Verify_HighReSampling3D import _load_common_records
...
def _train(train_x, eval_x, settings, source, seed, device, *, return_model=False):
    _seed(seed)
    model = FeatureVAE3D(
    ...
    result = (train_mu.cpu().numpy(), eval_mu.cpu().numpy(), losses)
    return (*result, model) if bool(return_model) else result
