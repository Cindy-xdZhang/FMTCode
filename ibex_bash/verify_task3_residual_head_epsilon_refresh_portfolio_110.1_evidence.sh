...
artifact_dir="outputs/Verify_Task3_ResidualHeadEpsilonRefreshPortfolio_110.1"
python Audit_Task3_ResidualHeadEpsilonRefreshPortfolio_110_1.py \
...
if [[ "$result_count" -ne 40 || "$checkpoint_count" -ne 40 \
  ...
  echo "unexpected 110.1 frozen counts: results=$result_count checkpoints=$checkpoint_count total=$total_count" >&2
  exit 1
fi
