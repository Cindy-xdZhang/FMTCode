...
科学commit `4ac7e6ab`，保持27,000训练视图、原head/scale/标签/实例计数和全部验证/测试，中心4,460→13,875。完整数据已生成且独立落盘审计通过；新旧两组固定同一4.5优化设置和4.3模型，以96111–96113三个种子比较，当前无完整分类结果。见[4.9协议](Task4C_center_diversity_protocol_4.9.md)。

2026-09-14部分结果：FMT两组的三个种子已完成；Conv3D尚未全部结束，不填入不完整均值。以下全部为验证F1，原4.6测试不变。

| 方法 | 原采样，4,460中心 | 更多中心，13,875中心 |
|---|---:|---:|
| FMT，88,514参数 | 0.415513 ± 0.015613 | 0.404945 ± 0.014071 |
| Conv3D，219,602参数 | 待完成 | 待完成 |

原采样组使用4.5同样配置、不同随机种子，不能将其0.415513与4.5的0.399091之差写成新方法增益。9份完成预测已独立复算通过，报告留在Ibex`outputs/Ablation_Task4C_CenterDiversity_4.9/independent_validation_audit_20260914.json`；完整结论待全部结束。
