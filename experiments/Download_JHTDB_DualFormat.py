...
def finish_vtk(loader,c,out,manifest):
...
def finish_flow(loader,c,out,manifest):
    ...
    finish_vtk(loader,c,out,manifest)
    manifest['status']='exporting_netcdf'
    write_json(out/'manifest.json',manifest)
    manifest['netcdf']=export_netcdf(c,out,manifest)
    manifest['status']='complete'
    manifest['finished_utc']=datetime.now(timezone.utc).isoformat()
    write_json(out/'manifest.json',manifest)
...
def run(config_path,token):
    ...
    sources={p:sha(Path(p)) for p in [__file__,'FLowUtils/flowDatasetUtils/JHTDB_VTK.py',
                                      'FLowUtils/flowDatasetUtils/JHTDB_Lodader.py','experiments/Download_JHTDB_Channel.py','experiments/JHTDB_DualFormat.py','experiments/Download_JHTDB_VTK.py']}
    ...
    for c in config['flows']:
        ...
        if path.exists():
            ...
            m={'version':config['version'],'config':c,'config_sha256':signature,'frames':[],
               ...
               'packages':{p:importlib.metadata.version(p) for p in ['vtk','givernylocal','numpy','netCDF4']},
    ...
    for c in config['flows']:
        try:
            ...
            print(f'{c["name"]}: VTK, complete NetCDF and online queries verified',flush=True)
        except Exception as exc:
            m=manifests[c['name']]
...
if __name__=='__main__':
    ...
    p.add_argument('--config',default='config/Verify_JHTDB_DualFormatDownload_1.2.json')
