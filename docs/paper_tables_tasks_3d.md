...
2026-09-14部分复核：下表只列已齐的三个种子95911–95913，均为验证F1；48³ Conv尚待两次复核。14份已完成预测在Ibex独立复算通过，报告`independent_validation_audit_20260914.json`。
...
## Task4-c 4.10：辅助监督式对比损失，进行中

科学commit `6bfc357b`。固定原采样、4.5配置和4.3分类推断函数，只增加训练投影器及监督式对比损失；权重0/0.1/0.5，温度0.1。始终复核零权重对照及选定正权重，三个种子96211–96213。工程核验及完整缓存复制通过，14次训练已提交，尚无完整科学结果。见[4.10协议](Task4C_supervised_contrastive_protocol_4.10.md)。原4.6测试保持。
