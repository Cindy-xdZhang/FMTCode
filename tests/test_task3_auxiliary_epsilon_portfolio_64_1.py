"""Contracts for preregistered Task3 auxiliary-epsilon portfolio 64.1."""
...
import Audit_Task3_AuxiliaryEpsilonPortfolio_64_1 as audit_module
...
from Select_Task3_AuxiliaryEpsilonPortfolio_64_1 import (
    _canonical_sha256,
...
CONFIG = Path("config/Verify_Task3_AuxiliaryEpsilonPortfolio_64.1.yaml")
SOURCE_62_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryBeta1Portfolio_62.1.yaml"
)
SOURCE_63_CONFIG = Path("config/Verify_Task3_AuxiliaryEpsilon_63.1.yaml")


class AuxiliaryEpsilonPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]), {"current_portfolio", "auxiliary_epsilon"}
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.210
        )
    ...
    def test_source_config_hashes_are_frozen(self):
        ...
        self.assertEqual(
            _canonical_sha256(SOURCE_62_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_63_CONFIG),
            config["sources"]["auxiliary_epsilon"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_is_independent_of_64_selector(self):
        source = inspect.getsource(audit_module)
        self.assertNotIn("Select_Task3_AuxiliaryEpsilonPortfolio_64_1", source)
    ...
    def test_evidence_and_cleanup_counts(self):
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_epsilon_portfolio_64.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_epsilon_63.1_cleanup.sh"
        ).read_text(encoding="utf-8")
