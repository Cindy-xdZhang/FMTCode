...
| mainExp_3DFMT_1.1 | 2026-08-17 | Task1-3D | 每个种子生成 7 条 pathline（center、x±、y±、z±）；中心位移与邻居相对位移先做时间差分；三维实序列逐坐标做 Fourier 变换，每个频率取 Gram 不变量并附加旋向三重积；邻居特征排序池化；StandardScaler + KMeans(k=2)；输出 3D/正交投影/Z截面/pathline 图 | `FMT_Clustering_3D.py`, `FMT_Utils/DFT_FMT_3D.py`, `FMT_Utils/FMT_3D_pipeline.py` | `config/PathlineFMTclustering3D.yaml` | 合成 smoke：180/180 valid，feature `[180,77]`，cluster `[60,120]`。真实 `halfcylinderRe160Resampled.nc`：7200/8000 valid，feature `[7200,161]`，cluster `[7000,200]` | 少数簇在全部 Z 截面稳定集中于近圆柱区域，代表性轨线明显弯曲/回旋，背景轨线基本平直；初步说明 Fourier 特征分出了近尾流回流/强旋转区。尚无 Q-criterion/IVD 参考标签，不能据此宣称等于完整涡区。只保证常数旋转/常数平移不变，不保证时变观察者下的完整客观性。 |
