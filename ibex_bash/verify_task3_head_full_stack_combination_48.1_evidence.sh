...
set -euo pipefail
cd "${TASK481_REPO_ROOT:-/home/zhanx0o/FMT_Task3_HeadFullStackCombination_48_1}"
mkdir -p slurm_logs

output_root="$PWD/outputs/Verify_Task3_HeadFullStackCombination_48.1"
...
if [[ "$per_run_count" -ne 960 ]]; then
  echo "expected 960 per-run CSV files, found $per_run_count" >&2
  exit 1
fi

checkpoint_count_before="$(find "$candidate_root" -type f \
  \( -name '*.pt' -o -name '*.pth' -o -name '*.ckpt' \) | wc -l)"
if [[ "$checkpoint_count_before" -ne 960 ]]; then
  echo "expected 960 temporary checkpoints, found $checkpoint_count_before" >&2
  exit 1
fi
...
if [[ "$archived_per_run_count" -ne 960 ]]; then
