...
Run:
    python -m experiments.VatistasFlowDatasetGenerator --config config/VatistasDataset.yaml
    python -m experiments.VatistasFlowDatasetGenerator --full     # write all 60000 (sharded, ~39GB)
    python -m experiments.VatistasFlowDatasetGenerator --refit     # redo the vastistas parameters fitting first
"""
...
def load_fit_outputs(fit_dir):
    ...
    if not (os.path.isfile(dist_path) and os.path.isfile(npz_path)):
        raise FileNotFoundError(
            f"No cached fit in '{fit_dir}'. Run `python -m experiments.FittingVatistasParam` first, "
