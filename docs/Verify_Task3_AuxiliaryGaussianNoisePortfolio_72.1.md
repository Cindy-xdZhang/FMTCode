...
This training-free development selector is frozen before 71.1 produces any
performance metric.  For each physical family it compares the independently
audited 70.1 portfolio winner with the independently audited 71.1 auxiliary
Gaussian-noise winner under the same registered metric order.
...
- A 71.1 family winner is eligible only after its zero-tolerance absolute FMT
...
- The selector freezes exactly 40 models and 40 result rows for paired seeds
  40--41 before temporary 71.1 checkpoints are deleted.

The targets are F1 gain `>= +0.214` and absolute FMT F1 `>= 0.893`.  Missing
either target is retained as a negative result and reaching both does not open
a new confirmation population.

## Status

Completed and independently audited on 2026-09-01. No 71.1 family replaces its
70.1 source under the zero-tolerance guard; the portfolio remains Raw-PCA/FMT
