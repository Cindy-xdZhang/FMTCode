"""Reconcile all V6/V7 scheduler jobs and export predictions without weights."""
import collections
import csv
import hashlib
import json
...
def main():
    root = Path('outputs/Ablation_Task4C_FMTv6v7_4.20')
    ...
    for r in submissions:
        expected.update([r['job_id']+'_'+str(i) for i in range(6)] if r['phase']=='train' else [r['job_id']])
    actual = {r['JobID']:r for r in rows if '.' not in r['JobID']}
    assert set(actual)==expected and len(actual)==8
    ...
    assert canonical(events)==canonical(durable) and len(events)==16
    ...
    for r in actual.values():
        ...
        if pair[0]['phase'] in ('preflight','train'):
            assert all('V100' in e['device'] for e in pair)
    audit = json.loads((root/'independent_audit.json').read_text())
    assert audit['status']=='PASS' and len(audit['runs'])==6
    assert audit['sample_counts']==dict(train=27000,validation=3000,test=10000)
    ...
    assert reproduction['status']=='PASS' and len(reproduction['comparisons'])==3
    summary = json.loads((root/'summary.json').read_text()); exported=[]
    for r in summary['rows']:
        item = dict(encoder=r['encoder'],parameters=r['parameters'])
        for split in ('training','validation','test'):
            for key in ('f1_mean','f1_std','average_precision_mean'):
                item[split+'_'+key]=r[split][key]
            for flow in ('channel','tbl'):
                item[split+'_'+flow+'_f1']=r[split]['per_flow_f1_mean'][flow]
    ...
    report=dict(status='PASS',scheduler_processes=rows,successful_processes=8,runtime_events=16,
    ...
    print(json.dumps(dict(status='PASS',processes=8,events=16,files=len(included),archive_sha256=manifest['archive_sha256'])))
