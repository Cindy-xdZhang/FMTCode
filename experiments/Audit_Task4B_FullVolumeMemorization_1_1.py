"""Independent strict audit for ``Verify_Task4B_FullVolumeMemorization_1.1``.
...
AUDIT_VERSION = "Audit_Task4B_FullVolumeMemorization_1.1"
...
def _validate_config(spec: dict[str, Any]) -> tuple[tuple[str, ...], tuple[int, ...]]:
    ...
    _require(
        contract.get("evaluation_set") == "exactly_the_same_all_20641_rows",
...
def validate_cache_identity(arrays: dict[str, np.ndarray]) -> dict[str, Any]:
    ...
    source_evidence = _exact_index_row_evidence(arrays["source_candidate_indices"])
    voxel_evidence = _exact_index_row_evidence(arrays["voxel_indices_xyz"])
    _require(
