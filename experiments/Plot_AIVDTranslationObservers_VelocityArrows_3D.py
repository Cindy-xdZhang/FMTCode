"""Annotation-only revision: camera velocity arrows share direction and length scale."""
...
def draw_camera_velocity(fig, baseline, index, medium, direction, velocity, time):
    """Encode projected velocity at t0 with a common tail-to-tip length scale."""
    ...
    rhs = '0' if index == 0 else ('' if index == 6 else fractions[index])+r'\vec{u}_{\rm mean}(t)'
    label = fig.text(.737,baseline,r'$\vec{u}_{\rm cam}(t)='+rhs+'$',
                     fontsize=8*scale,ha='left',va='baseline',color='black')
    label.set_gid(f'camera_velocity_{index}')
    origin = np.array([.737, baseline-11*scale/fh])
    length_pt = 60*scale*index/6
    endpoint = origin + direction*length_pt/np.array([fw,fh])
    if index:
        # No shrink: measured shaft plus head span is proportional to speed.
        arrow = FancyArrowPatch(origin,endpoint,transform=fig.transFigure,
                    arrowstyle='-|>',mutation_scale=5*scale,lw=.85*scale,
                    color='black',shrinkA=0,shrinkB=0,clip_on=False)
        arrow.set_gid(f'camera_velocity_arrow_{index}');fig.add_artist(arrow)
    else:
        dot = Ellipse(origin,2*scale/fw,2*scale/fh,transform=fig.transFigure,
                      facecolor='black',edgecolor='none',clip_on=False)
        dot.set_gid('camera_velocity_zero_0');fig.add_artist(dot)
    return {'panel':'abcdefg'[index],'velocity_fraction':index/6,
            'definition':'u_cam(t) = fraction * spatial mean flow velocity(t)',
            'arrow_time':time,'velocity_vector_at_arrow_time':(index/6*velocity).tolist(),
            'arrow_direction_screen':direction.tolist(),'arrow_length_pt':length_pt,
            'arrow_tail_figure':origin.tolist(),'arrow_tip_figure':endpoint.tolist(),
            'arrow_scale':'60pt paper / 90pt slides at 100%; zero shown as dot',
...
def render(paths,labels,original_indices,audit,output_dir,title,medium='paper',pdf_collision_audit=True):
    ...
    low-=pad;high+=pad
    velocity_times=np.asarray(audit['observer_velocity_times'],dtype=float)
    velocity_samples=np.asarray(audit['observer_velocity_samples'],dtype=float)
    arrow_time=float(audit['initial_time'])
    assert np.all(np.diff(velocity_times)>0)
    assert velocity_samples.shape==(len(velocity_times),3) and np.isfinite(velocity_samples).all()
    assert velocity_times[0]<=arrow_time<=velocity_times[-1]
    velocity=np.array([np.interp(arrow_time,velocity_times,velocity_samples[:,j]) for j in range(3)])
    ...
    for i,(alpha,trajectories,y) in enumerate(zip(audit['levels'],center,labels)):
        ...
        # Orthographic 3D projection has an equal-aspect 2D plane; translation
        # does not change this direction between the seven observer panels.
        midpoint=(low+high)/2
        projected=np.array(proj3d.proj_transform(*(np.stack([midpoint,midpoint+velocity]).T),ax.get_proj()))[:2]
        direction=projected[:,1]-projected[:,0]
        assert np.linalg.norm(direction)>0
        direction/=np.linalg.norm(direction)
        camera_annotations.append(draw_camera_velocity(fig,bottom+.191/1.675,i,medium,direction,velocity,arrow_time))
    ...
    fig.text(.5,1-.048/1.675,r'$\vec{u}_{\rm mean}(t)$: global mean flow velocity; arrows at t0, shared scale',
             ha='center',fontsize=max(font,8))
