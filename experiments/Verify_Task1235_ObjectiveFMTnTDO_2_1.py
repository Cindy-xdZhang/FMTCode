"""Paired performance experiment for the scalar-only nTDO v2 descriptor with paired v1 control.
...
def features(rows, name, task, device):
    parts = []
    for r in rows:
        if name == 'objective_fmt_nTDO_v2':
            value = objective_fmt_nTDO_v2(r['raw'], num_freq=6)
            assert value.shape == (len(r['raw']), 231)
        elif name == 'objective_fmt_nTDO':
...
def provenance(spec, config_path, task, dataset, seed, device):
    return {'experiment':spec['experiment'], 'task':task, 'dataset':dataset, 'seed':seed,
        ...
        'encoder_sha256':sha('FMT_Utils/objective_fmt_nTDO_v2.py'),
        'v1_encoder_sha256':sha('FMT_Utils/objective_fmt_nTDO.py'),
...
def audit_shard(target, expected_arms):
    ...
    if complete['task'] in ('Task3','Task5'):
        assert {frozen[a]['parameter_count'] for a in ('raw_pca','old_fmt','ntdo','ntdo_v2')} == {138818}
    return rows
...
def audit(spec):
    ...
    for task in spec['tasks']:
        ...
        for baseline in spec[task.lower()]['arms']:
            if baseline=='ntdo_v2':continue
            for dataset in spec['datasets']:
                d=[r for r in part if r['dataset']==dataset]
                delta=[float(next(r['f1'] for r in d if r['arm']=='ntdo_v2' and int(r['seed'])==seed))-
                       float(next(r['f1'] for r in d if r['arm']==baseline and int(r['seed'])==seed))
                       for seed in spec[task.lower()]['seeds']]
                paired.append({'task':task,'dataset':dataset,'baseline':baseline,'ntdo_v2_minus_baseline_f1':float(np.mean(delta))})
...
def main():
    ...
    parser.add_argument('--config',default='config/Verify_Task1235_ObjectiveFMTnTDO_2.1.json')
