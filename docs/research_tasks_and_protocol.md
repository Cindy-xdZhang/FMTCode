...
## 2b. Task4-c 固定线簇数据集 v1（2026-09-18 起长期固定；规则由 `Ablation_Task4C_FPS16_1.2` 起草，实现版本见冻结政策）
...
**Hairpin 实例覆盖与划分（2026-09-18 用户追加规则，替代上文“模板与替换”一段）。**
- 目的：可视化时每个人工 hairpin 实例都能看到测试线束的分类结果。“覆盖”指某划分在该实例的头部区域内有 ≥2 个采样点；头部区域取与 GTHeadCoverage 1.1 相同的定义：采样点在该 GT 实例体积内，且速度–涡量夹角满足头部条件（|cos|²<0.5）。
- 训练集必须覆盖 80% 的 hairpin 实例；余下 20% 的实例在训练集和验证集中完全不出现（其 GT 体积及外扩一个当地网格间距 h 内没有任何训练/验证中心）。测试集必须覆盖**全部**实例：对训练覆盖过的实例，测试采样点相对训练采样点轻微偏移（沿用现有规则：距任意训练中心 ≥1h、距同实例训练中心 ≤4h）；对 20% 未见实例，测试采样点就是该实例头部区域的新采样点。实例分组按流场用固定随机种子抽取，GT 包围盒重叠的实例绑在同一组（沿用 9.15 的做法）；Channel 74 个实例约 59/15，TBL 58 个约 46/12。
- 负样本（无 GT 重叠的涡头区）跟随离它最近的 hairpin 实例的分组：靠近未见实例的负类头区只进入测试集；其余按原头区空间块分给训练/验证/测试。测试报告分“训练覆盖实例上的偏移测试”与“未见实例测试”两栏，以及合并结果。
- 因划分方式改变，本规则下的数据必须全部从头重建（不再是替换模板中的不合格行）；替换式的 `Ablation_Task4C_FPS16_1.2` r2 重建（52046106）已跑完但核验 52046107 失败且已被本规则取代，不冻结、不使用。实现版本与冻结哈希在新版本建成后填入本节。
...
**冻结政策。**
- 数据文件由实现上述全部规则（含实例覆盖与划分）的新构建版本生成；其 `audit-data` 通过后 `data_audit.json` 中的文件哈希即为冻结哈希，后续实验必须核对后只读使用，不得修改、重排或重新积分。`Ablation_Task4C_FPS16_1.2` r2 的替换式重建不满足覆盖规则且核验失败，不是冻结数据。
