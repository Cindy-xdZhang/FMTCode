...
2026-09-14用户指出4.15误删原4.14的72维几何方向频谱，当前纠正为`Verify_Task4C_RetainDirection_4.16`。原233维FMT三种子复跑；fmt_v5在原233维后仅追加165维角度；距离+角度2.2保留原72维分支，整套模型不称完全客观。原数据/训练不变，Conv3D复用经复现的原结果。协议`docs/Task4C_retain_direction_protocol_4.16.md`；4.15成绩保留为偏离用户要求的比较，不能回答单独加入角度的效果。当前推进授权优先于以下4.15完成状态。
