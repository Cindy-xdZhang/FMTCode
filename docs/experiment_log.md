...
## 2026-09-09 — 原FMT、客观核心与轮廓系数的核查及解释修订

用户要求重新核查当前证据，并部署Task1/2性能下降的机制对照。以下保持历史数值、冻结算法及主表不变。

| 先前概括/待核查说法 | 本轮结论 | 证据与差异原因 |
|---|---|---|
| Task1/2/3/5均由含中心时间差分的同一种FMT提升 | 四任务主表相对各自Raw基线均有平均提升，但配方不同；Task3当前核心是aivd1w3_dft，不含中心位移块 | paper_tables_tasks_3d：Task1 .4510→.6014；Task2 .4884→.5840；Task3 .6398→.8583；Task5 .5835→.6727。不能把Task3收益也归因于中心位移傅里叶 |
| 旧邻居傅里叶只读取客观点间距离 | 输入是同时间相对位置向量，随后对其坐标分量作时间差分，再Fourier、频域不变量和排序池化；并非只对距离标量做Fourier | DFT_FMT_3D.py::pathline_dft_features_3d 的 relative、neighbour_delta。输入向量客观，普通分量时间差分/跨时刻Fourier一般不保证客观；两者不能混同 |
| 删除中心块与换成客观FMT等价，前者消融已证明客观傅里叶最重要 | 两个操作不同。旧邻居块的重要性已复现，但该块不具备任意时变旋转不变性 | Ablation_Task123_FMTComponents_1.2：删邻居Task1/2 ΔF1 −.3307/−.1067；删中心−.0058/+.0004。客观v2将整套邻居编码也换成同时间Gram标量，额外改变了信息内容、尺度处理、表示宽度和池化 |
| 客观化后各任务均严重下降 | Task1/2退化明确；Task5保留辅助块时平均F1几乎不变 | Verify_FMTAllV2_1.2：Task1 .601508929→.142771790；Task2 .583613270→.179515022；Task5 .678020588→.676800723。Task5含Raw及运动学块，不证明整网客观 |
| aivd完全没有几何信息 | 它由邻居几何估计标签相关物理量，但不是完整轨线几何表示；无需后续大部分帧符合此实现 | aivd1w3_dft只有前三个估计IVD的和，实际依赖前四帧坐标。其连续满秩公式通过全样本同时间平均抵消共同观察者转动；离散差分有残差，且采样均值不同于全域体积均值。不能称“时间差分均不客观”或已证明任意旋转下数值严格客观 |

### 新找到的轮廓系数证据，修订此前“尚未定位”状态

用户提供 `FLowlineClusteringVisualAnalysis/docs/Other_FeatureSilhouette_1.1_results_zh.md`。本轮已读取该报告、原始results.csv和validation.json；384行对应8条目×8表示×2投影×3聚类。原CSV SHA256 `05967a06137273e509495da502b3e228dd9097335775027195bbdb50a44bd4a0`。原项目validation为passed，本轮另从CSV复算公共KMeans组合均值；不声称重跑了积分、VAE或384个逐点距离。

旧记录仅保存用户观察、没有逐次证据 → 当前可追溯到完整表；但原报告的Task2 VAE实际为 **fmt_all+kin4→VAE**，不是Raw→VAE。报告跨方法均值原FMT .5476、FMT+VAE .5646、几何统计 .5336、几何IVD .3940；DBSCAN各臂可计算条目集合、噪声排除比例不同，故该总列不能直接作公平排名，不能用它确认“原FMT优于普通Raw VAE”。

本轮限定全部方法共同完整的32个KMeans组合（8条目×2投影×K=2/3），不删除任何该网格观测，复算如下。这是事后审计性汇总，不取代原384行，不据此选模型。
...
本轮支持的解释：这组固定窗口、单种子和二维显示流程中，原FMT的平均簇分离优于传统几何及估计IVD，是有价值的几何分组线索；未证明跨时间稳定性、Raw VAE劣于FMT、完整流映射信息保留或物理类别正确。[轮廓系数定义](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.silhouette_score.html)只衡量所选距离空间的簇内/簇间分离。复算值和源哈希保存在 `outputs/Verify_FMTObjectivityMechanism_1.1/silhouette_recheck.json`。

### 为什么客观标签不要求只靠单个primitive距离序列就能预测

令同时间邻居偏移为d(t)。在x*=Q(t)x+c(t)下，d*=Q(t)d，距离及同时间内积不变；这确认d是客观几何向量。普通导数为 d_dot*=Q d_dot+Q_dot d，有限差分也包含相邻Q的变化。因此，对客观向量分量直接跨时间运算不能从输入客观性推导最终特征不变。

更关键的是：v2的每个primitive同时间Gram序列，对每个primitive各自独立的任意转动也不变；真实刚体观察者要求所有primitive共享同一个Q(t)。局部形变标量因此丢掉了各局部结构之间的相对转动信息，而IVD以局部涡量相对于同时间整体均值的差为目标。相同距离历史不保证相同IVD。客观量并不是所有其他客观量的充分表示；“客观信息一点没丢”在当前编码上尚不成立。

新解析验证：三个不相交的局部刚性交叉以角速度0、1、3转动，同时间Gram始终相同；相对三样本平均值的解析涡量偏差分别8/3、2/3、10/3。有限物理时间差分估计通过rtol3e-4校验，Gram6输出接近0，旧邻居块仍可区分转动。此反例证明表示可能不足，尚不能量化它在真实流場F1下降中占多少；要与归一化、频率截断及降维影响分别验证。理论IVD的客观性依据 [Haller等2016](https://arxiv.org/abs/1506.04061)，本反例使用样本平均，不将它冒充全场体积平均。

### Verify_FMTObjectivityMechanism_1.1 — 预注册并部署

协议 `docs/Verify_FMTObjectivityMechanism_1.1.md`，新增实现 `FMT_Utils/ObjectivityMechanism_3D.py`，运行器 `experiments/Verify_FMTObjectivityMechanism_1_1.py`。12预注册表示×10条目×3种子，两任务共60分片，Task1并列PCA8/无PCA，Task2共用700维补零输入及冻结VAE，预计1080行。保持IVD p95、采用已有晚期Cylinder时间划分；不跨新旧population直接比较。测试前冻结全部模型与簇映射，标签打乱与共享Gram距离轮廓系数作为额外诊断，不按结果选候选。

本地6项单元测试PASS：旧块精确遮罩、v2复算、全谱可逆、Gram时变刚体不变、无中心旧邻居块非客观、刚性旋转反例。七线示意图已导出PNG/SVG/PDF；字号最小8pt、碰撞0FAIL/0WARN，单面板无需多面板尺寸对齐。图为示意坐标，不是实验数据。

Ibex 2026-09-09 15:57+03已提交预检51632860、smoke51632861、Task1数组51632862、Task2数组51632864、最终审计51632865；完整配置、来源快照哈希、base commit及设备上限见作业登记表。初次核对均排队，**没有新真实数据性能结论**。不修改或重新归因冻结主表，不保存模型checkpoint。


### Verify_FMTObjectivityMechanism_1.2 — 运行前参数复放修订

1.1预检拒绝旧缓存重算：新诊断误用了函数默认邻居缩放100和权重.5，冻结缓存生成器实际二者均1.0。原6项解析测试通过，数据预检失败；尚无正式训练/测试指标。保留失败51632860、已完成smoke51632861，取消未启动依赖数组51632862/51632864及审计51632865。

1.2显式采用原缓存生成参数1.0/1.0，并同样纠正无邻居差分控制；不放宽数值容差，不修改冻结提取函数。增加缓存参数测试后7项本地PASS。配置、科学候选及新旧区别见`docs/Verify_FMTObjectivityMechanism_1.2.md`；完整代码以1.1/1.2独立上传包及两个远端目录保存。16:01+03提交新预检51633095、smoke51633096、Task1数组51633097、Task2数组51633098、审计51633099。无正式结果前的实现修订，不构成据测试调参。


### Verify_FMTObjectivityMechanism_1.3 — 缓存与重算来源分别评估

1.2预检在Channel通过，但Re160的缓存/重算差异仍超原容差（0.378%分量，max absolute .00777543）。已确认原FMT在中心化前提取，Raw保存时作float32中心化；不能直接断言重算与缓存等价，具体舍入/设备影响尚未完全拆解。该差异只来自训练缓存的代码验证，未产生正式测试性能。保留1.2失败和smoke，取消未启动依赖作业。

1.3新增旧重算full/no_center，与原12臂合为14臂，共60科学分片和1260行；从相同保存坐标比较差分操作，同时保留缓存原样比较及严格中心删除。原等价检查不被放宽或伪称通过，差值单独记录；分别验证各臂来源。配置/说明见`config/Verify_FMTObjectivityMechanism_1.3.json`、`docs/Verify_FMTObjectivityMechanism_1.3.md`。8项本地单元测试PASS。16:05+03提交新预检51633248、smoke51633249、Task1数组51633250、Task2数组51633251、审计51633252。原两版所有源包和失败记录保留，当前正式执行版本1.3。
...
### 1.3正式完成：以实际干预修正性能下降的解释

2026-09-09，Task1/Task2各30分片全部完成，最终审计51633252于17:03:40+03结束。10数据条目×3种子×14表示，Task1另并列PCA8/无PCA，共1260行。已取回逐点预测，并使用独立本地程序 `experiments/Audit_FMTObjectivityMechanism_Local_1_3.py` 从混淆计数复算全部1260行分类指标和42行宏平均，结果PASS；420个VAE模型均执行7000更新、参数数目均1,030,460。未下载或保存模型。此次审计不声称独立复算轮廓系数。

| 此前解释 | 当前修订 | 为什么修订／此前的问题 |
|---|---|---|
| 用相对向量分量的导数公式回应“点间距离输入仍然客观” | 同时间欧氏距离是客观标量；相同距离数组通过相同确定性运算、固定网络或固定聚类模型，输出相同。距离标量的时间差分和Fourier也保持这一性质 | 此前没有把标量距离与三坐标分量输入区分清楚，也没有先追踪实际输入；公式不能代替实现检查，更不能单凭它解释F1下降 |
| 中心或邻居时间差分的删除可能直接解释大幅下降 | 只删除中心，Task1几乎不变，Task2下降约.011；不做邻居分量时间差分也能保持较高F1。大幅下降发生在替换整个邻居表示时 | 新的受控实验排除了“只删中心导致崩溃”，不支持把“去掉时间差分”单独当作大幅下降原因 |
| 局部转动反例可以解释真实数据的大部分下降 | 反例仍只说明同时间距离/内积历史不必足以恢复IVD；不能从反例推断真实数据下降的占比 | 未完成跨时刻方向结构、特征距离与下游优化影响的独立分解，不能把候选机制写成已确认因果 |

下表为当前批次的10条目等权宏平均，先各条目汇总3种子。Task1使用8维主成分分析（PCA）后KMeans；Task2使用同一变分自编码器（VAE）架构再KMeans。所有输入统一补零至700维，保持同一辅助运动学块；此宽度及晚期Cylinder划分用于本批对照，不能替换历史主表。

| 表示干预 | Task1 F1 | Task2 F1 |
|---|---:|---:|
| 原缓存完整FMT | .595775 | .559109 |
| 原缓存仅中心23维置零 | .595101 | .548489 |
| 同保存坐标重算完整FMT | .595769 | .561027 |
| 同保存坐标重算、中心置零 | .595101 | .533926 |
| 中心置零，邻居相对坐标直接Fourier、不作时间差分 | .607492 | .581165 |
| 同时间内积矩阵（Gram）标量→6频率Fourier | .141523 | .167965 |
| Gram取消初始尺度归一化 | .141564 | .162891 |
| Gram标量时间差分→6频率Fourier | .188707 | .271559 |
| Gram保留全部Fourier频率 | .147559 | .146810 |
| Gram直接使用完整时间序列 | .179345 | .214413 |

“只删除中心”的配对种子宏平均差为Task1 −.000674（种子差标准差.000294）、Task2 −.010621（.022279）。无邻居差分臂相对于同坐标重算且无中心臂，分别为+.012392、+.047238；不混用缓存/重算来源作单一操作的精确比较。Gram6在Task1取消PCA时为.141642，因此PCA8并非该下降的充分解释；取消尺度归一化、恢复全频率也未恢复旧水平。全谱与时域可以保留同一序列的信息，但逐维标准化及欧氏聚类几何不必等价，信息可恢复不意味着聚类标签不变。

标签打乱诊断：原完整FMT的Task1/Task2真实F1均值.595775/.559109，对应打乱标签均值.063562/.063540，两个任务各30次结果均高于对应打乱分布的95%分位数。打乱仅在同一测试时间片内重复100次，保留正例比例；这是关联诊断，不是考虑空间依赖的正式显著性检验，也不能证明跨流场普适性。当前证据不支持将高F1直接概括成纯随机巧合。

证据目录 `outputs/Verify_FMTObjectivityMechanism_1.3/`：`summary.csv`、`per_run_metrics.csv`、`independent_audit.json`、`local_independent_audit.json`、`independent_confusion_counts.csv`、`execution_events.json`、`scheduler_final.txt`。逐点预测归档SHA256 `434bf2dca3ac0e00b86f7ff6fac9a33869b32d6494b540b698129bfb4e85d4c3`；独立审计源码SHA256 `98f5c8ebc0d386d28823d7b242a5286719f06530049b805056ad76c2bd01d70a`。仍为已有benchmark上的机制诊断，不用测试结果选择新主表方案，不改写冻结结果。

### Verify_DistanceInputIdentity_1.1 — 距离标量的直接代码验证

独立本地验证读取轮廓系数项目8个真实几何文件，共29,571个七线primitive；对所有材料点施加同一个时变刚体变换，不重新播种，不读取IVD标签。完整协议见 `docs/Verify_DistanceInputIdentity_1.1.md`，代码 `experiments/Verify_DistanceInputIdentity_1_1.py`。距离→Fourier→固定标准化/PCA/KMeans流程只在变换前拟合一次，变换后复用全部参数。固定小网络只验证输入/输出恒等性，不是训练后的性能模型。

- 真实数据：21对同时间欧氏距离最大绝对漂移1.253e-13以内，距离Fourier漂移6.218e-13以内，固定示例网络漂移3.331e-14以内；29,571个簇标签全部不变。相同数组重复输入固定网络，最大差严格为0。
- 整数坐标及精确90°旋转构造：距离、距离时间差分、距离Fourier、固定网络输出逐位相同，聚类标签全部不变。该构造去除了浮点三角函数误差。
- 同时在内存中捕获冻结旧算法实际传给Fourier的数组，8个真实数据及构造均严格等于 `diff(neighbors-center, time)`，形状为 `[N*6,31,3]`；不是欧氏距离标量。仅临时包裹函数观察输入，随后恢复函数，未编辑冻结算法。
- 在同一精确构造下，距离输入完全不变，但旧算法实际三分量输入最大变化211、旧邻居输出变化194.482；Gram输出保持完全不变。这是对实际不同输入的检验，不是否认相对向量的客观几何意义。

所以“相同距离数组进入固定计算，结果仍相同”得到直接验证；旧编码与Gram编码没有在执行这个相同输入、相同计算的比较。真正只遮掉中心时的结果另由1.3给出，并未发生所说的大幅下降。至于替换邻居表示时具体哪些可区分类别的结构或度量发生变化，当前尚未完全分解，不能仅凭客观性给出因果结论。

结果 `outputs/Verify_DistanceInputIdentity_1.1/results.json` 为PASS，含各几何文件SHA256、逐数据误差、实际输入形状与源版本。验证源码SHA256 `6ed4d4b4c7521de465dd3a47f7aebc62271c2b411371522f3bcda7187d9df0c2`；冻结编码器SHA256 `efff993f10a8db7b481784198beecce1c3c74b2f81cd2eddf5a523ba0a19222d`。本验证没有新训练、没有标签筛选、不构成新的分类性能结果。

### 2026-09-09 — 用户要求登记纠正并扩展到Task3/Task5

用户提出“客观信息已经可以拟合IVD并获得较好F1”，并要求将新FMT用于Task3/Task5。这里必须把已证实的性能和整套输入的客观性分开记录；不能把不同实验的两项结果拼成同一方法的证明。IVD是瞬时涡量偏差，本批Task1/2的F1评价其p95二分类标签，并不是回归连续IVD数值；Task1不训练编码器，Task2训练的是无监督VAE。

| 方法／版本与输入 | Task1 F1 | Task2 F1 | 整套编码的观察者不变性证据 |
|---|---:|---:|---|
| Mechanism 1.3：原FMT+kin4 | .595775 | .559109 | 未满足严格时变观察者不变性 |
| Mechanism 1.3：只去中心，保留旧邻居块+kin4 | .595101 | .548489 | 解析共同旋转下输出变化，不能称整套严格客观 |
| Mechanism 1.3：邻居相对坐标直接Fourier+kin4，无中心、无邻居时间差分 | .607492 | .581165 | 解析共同旋转下输出变化，不能用这组F1证明纯客观标量输入已达同等性能 |
| Mechanism 1.3：纯同时间Gram标量Fourier，无kin4 | .143133 | .131303 | 核心严格不变，已做数值验证 |
| Mechanism 1.3：同时间Gram标量Fourier+kin4 | .141523 | .167965 | Gram核心不变，但辅助运动学块不在该保证内；整套输出有变化 |
| DistanceInputIdentity 1.1：21对同时间距离标量Fourier | 未测分类F1 | 未测分类F1 | 距离核心不变；8真实数据固定聚类标签全部不变，未做IVD分类评价 |

修订对照：此前“只删除中心没有大幅下降”保持成立；若把它进一步读成“纯客观距离编码已经达到.6075/.5812”，当前证据不支持。`outputs/Verify_FMTObjectivityMechanism_1.3/analytic_checks.json` 的同一个共同旋转构造中，去中心旧编码、无差分邻居+kin4、纯Gram、Gram+kin4的输出差L2范数依次为6.068316、29.859339、7.582e-15、.017910。范数受各表示尺度影响，仅用于是否不变的核查，不作方法之间误差大小排名。用户的距离标量不变原则与这些不同输入的数值结果并不冲突。

后续Task3/Task5已获得运行授权，尚需明确用户所指的新配方是高F1的相对坐标Fourier、严格客观的距离标量Fourier，或两者并列。已提出此输入定义问题，未将未回复视为方法选择。Task3/Task5旧主表继续冻结，扩展采用新版本，不将尚未运行的结果写入性能表。

### Verify_FMTNoCenterObjectivity_1.1 — 第三种方案的独立直接验证

2026-09-09，用户明确要求先验证第三种方案是否客观。本次定义严格固定为：`pathline_dft_features_3d`，num_freq=6、neighbor_scale=1、neighbor_weight=1、sort、gram模式、保留手性，**仅将中心23维置零，邻居138维不变**。另行计算28维kin4辅助块，不将其误差混入邻居核心的判断。没有使用aivd，不训练模型，不提交Task3/Task5，不修改冻结编码器。

程序 `experiments/Verify_FMTNoCenterObjectivity_1_1.py` 已实际完成：8份真实几何文件共29,571个primitive，加1个静止单位七点构造；5种预定义观察者×float64/float32，共90组观察者/数据/精度检查。每次同一时刻所有物质点共享同一Q(t)、c(t)，不重新播种；验证Q正交且det(Q)=1，同时检查相对位置向量遵守Q变换规律。程序成功完成为 `execution_status=PASS`，编码器判定另列为 **`encoder_observer_invariance=FAIL`**，两者不得混淆。

双精度真实数据的汇总如下。变更数量以逐维rtol=atol=1e-5判断，分母均29,571个primitive；不以不同表示间的绝对误差比较方法质量。

| 共同观察者变换 | 同时间距离最大绝对差 | 无中心FMT核心最大绝对差 | 核心发生变化的primitive数 |
|---|---:|---:|---:|
| 恒等 | 0 | 0 | 0 |
| 时变平移 | 4.263e-14 | 1.052e-11 | 0 |
| 恒定90°旋转 | 0 | 0 | 0 |
| 时变旋转（0至1.3弧度） | 1.232e-13 | 31.740358 | 29,571 |
| 同一时变旋转加时变平移 | 1.232e-13 | 31.740358 | 29,571 |

所有真实样本的同时间相对向量均遵守正确的观察者变换，最大残差1.302e-13以内。因此并非将相对向量坐标变化误判成几何变化。**结论针对后续生成的标量特征：第三种方案不满足任意时变刚体观察者下的不变性，而且去掉kin4后仍然失败。**恒定旋转及平移对照在双精度下通过；这不等于任意时变旋转客观。

最直接的解析反例：中心与六个轴向单位邻居均静止。原观察者下邻居时间差分全零，删除中心后的全部核心特征为零。对所有点施加相同的平滑旋转，窗口内旋转1.3弧度，同时间中心—邻居距离保持1（全部点对距离最大误差2.220e-16），但第一个邻居模长特征从0变为1.2103728114720793。该值可不调用编码器独立算出：零频率分量是差分之和，模长为 `||d_last-d_first||=2*sin(1.3/2)=1.210372811472079`。这是真正标量模长的变化，不只是向量方向变化；说明该特征计算跨时间相对向量差，不能把它当作同时间点间距离。静止例的完整前后坐标、旋转、距离和特征已保存，便于逐点复查。

单精度也复现同一结构性失败：静止构造的对应特征为1.2103726863861084；真实时变旋转下核心29,571/29,571变化。另观察到float32在平移后存在坐标量化及频域归一化的数值敏感性（核心max .0315003），kin4在恒定旋转下也有数值残差；这些数值现象单列保留，不用来证明理论上平移/恒定旋转不客观。否定时变观察者不变性的结论以双精度结果和独立解析反例为依据。

此前“第三种保住F1，但不能称严格客观” → 本次独立检验确认其不满足时变观察者不变性。此前“删除中心导致大幅F1下降”的解释仍然被1.3对照否定；两项结论互不替代。本验证不重新计算或改写F1。

证据目录 `outputs/Verify_FMTNoCenterObjectivity_1.1/`：`results.json`、`feature_errors.csv`、`summary.json`、`stationary_cross_counterexample.npz`。协议 `docs/Verify_FMTNoCenterObjectivity_1.1.md`。新验证源码SHA256 `c653addb4bff087d13984471a8c4a231341a31ce72f9c76046fdccf17b0a76a0`；冻结编码器执行前后SHA256均 `efff993f10a8db7b481784198beecce1c3c74b2f81cd2eddf5a523ba0a19222d`；输入8文件SHA256及辅助适配器哈希保存在结果JSON。未产生checkpoint。

### 2026-09-10 — 固定客观性定义，并明确第三种方案判断的对象

用户要求将其提供的客观性定义抄入基础协议和指定的optimal-connection文档。已在本项目 `AGENTS.md` 固定标量/向量/二阶张量的三种判据，并在 `docs/research_tasks_and_protocol.md` 第0节抄录英文定义与公式。相同完整定义已写入 `C:/Users/xingdi/sources/optimal-connection/docs/referenceframe/symplecticRFT_first_principles_audit_zh.md` 第0节，原第1–9节逐字保留。定义来源为用户本轮提供文本，仅整理排版并显式加上时间参数，没有添加未经核对的论文归因。

| 先前表述 | 本轮明确表述 | 原因及证据 |
|---|---|---|
| “第三种方案不客观” | 第三种的同时间相对位置输入是客观向量；旧邻居编码继续跨时间处理坐标分量，生成的标量描述量不满足任意时变观察者不变性 | 不限定输入/中间量/输出的简称容易误导；本轮纠正表述对象，不改变已有数值结论 |
| “观察者变化后特征数值变化” | 必须先声明特征类型。几何向量坐标变化且满足Q变换不构成失败；此次反例变化的是明确计算的模长标量，因此适用标量数值不变判据 | `Verify_FMTNoCenterObjectivity_1.1`同时验证了相对向量正确的Q变换，失败判据针对后续模长等标量，不针对相对向量本身 |

第三种实际属于用户指出的原因a：冻结代码 `FMT_Utils/DFT_FMT_3D.py` 先计算 `relative = neighbours - center`，这是同时间的客观相对位置向量；随后执行 `neighbour_delta = relative[:,:,1:] - relative[:,:,:-1]`，再对这一三分量序列沿时间做实离散傅里叶变换，最后计算频域范数、内积和手性。删除中心块没有删除邻居这一跨时间运算。因此不能把第三种实际实现描述成“仅将各时刻点间距离标量输入FMT”。

用户指出的原因b所涉及的定义本身完全成立：客观标量数值不变、客观向量按Q变换、客观二阶张量按QTQ^T变换；同时间任意点对相对位置向量和欧氏距离分别满足对应定义。若先形成不变的距离标量序列，再经相同固定确定性编码，输出仍不变。若保留相对向量作为输入，其输入客观性也成立，但后续编码输出的类型与变换规律须另行核查。不得从输出反例倒推输入不客观。

本轮只更新定义和解释；没有修改冻结编码器、重写F1、重新运行分类实验或提交Task3/Task5。

### 2026-09-10 — objective_fmt_nTDO 1.1 独立模块与输入验证

用户指定实现 `objective_fmt_nTDO`，其中nTDO为No timeDirectionOperation：交给傅里叶变换的只允许是同时间 `relative = neighbours - center`，以及relative所表示的同时间点间欧氏距离，不使用跨时间差分量。已新增独立模块 `FMT_Utils/objective_fmt_nTDO.py`，版本1.1；不导入或调用旧完整编码器，不修改冻结实现。这里按用户请求保留时间傅里叶变换，nTDO具体指傅里叶之前没有时间差分、减初始帧或其他跨时刻预处理，并非声称傅里叶本身不沿时间运算。

| 版本／块 | 输入与技术细节 | 主代码路径 | 输出与验证 |
|---|---|---|---|
| objective_fmt_nTDO 1.1 相对向量块 | 同时间六个邻居减中心，直接时间Fourier前6频率；沿用旧频域实/虚部模长、夹角余弦、手性及逐维邻居排序的计算方式，独立实现；缩放与权重均1 | `build_fourier_inputs` → `_vector_descriptors` | `[N,6,L,3]`输入，138维输出；与旧无差分频域后处理逐元素一致 |
| objective_fmt_nTDO 1.1 距离块 | 以中心相对位置0和六个relative构成七点，同时间全部21对欧氏距离（6中心—邻居+15邻居—邻居）；直接时间Fourier前6频率，实部与除DC之外的虚部拼接 | `build_fourier_inputs` → `objective_fmt_nTDO` | `[N,21,L]`输入，231维输出；距离未经平方、尺度归一化、差分或初始帧扣除 |
| objective_fmt_nTDO 1.1 合并 | 两块直接拼接，无中心运动、kin4或aivd附加输入 | `objective_fmt_nTDO` | 默认369维；两块可单独返回。当前未测分类F1，不替换历史主表 |

使用示例：

```python
from FMT_Utils.objective_fmt_nTDO import objective_fmt_nTDO, build_fourier_inputs

# pathlines: [N, 7, L, 3], center is line 0
features = objective_fmt_nTDO(pathlines)  # [N, 369], NumPy by default
blocks = objective_fmt_nTDO(pathlines, return_blocks=True)
relative_features = blocks['relative_fourier']  # [N, 138]
distance_features = blocks['distance_fourier']  # [N, 231]
inputs = build_fourier_inputs(pathlines)  # inspect actual same-time arrays
# return_numpy=False preserves Torch device, precision and gradients.
```

8项单元测试已实际通过：捕获真正送入两次rFFT的数组并独立复算输入；核对保留完整L时刻；旧无差分向量后处理逐元素匹配；静止几何保留非零DC；平移与恒定旋转；时变观察者下输入向量正确变换和距离输出不变；已知正弦距离序列的频率/幅度；Torch精度和梯度、附加第四通道与非法输入处理。测试文件 `tests/test_objective_fmt_ntdo.py`，命令 `python -m unittest discover -s tests -p test_objective_fmt_ntdo.py -v`，8项耗时.252秒。

另外已执行 `Verify_ObjectiveFMTnTDO_1.1`：固定8份真实几何文件各取前128个primitive，共1024个，不读取标签。输入均32采样点，对所有材料点施加同一平滑时变旋转和平移，以float64运行。相对向量变换规律最大残差1.119e-13以内；同时间距离最大误差1.006e-13以内；距离Fourier块最大误差3.971e-13以内。相对向量Fourier块在各数据中仍有变化（最大绝对差1.310344至6.677583）。所以模块已按指定输入实现，但不能仅凭名称或“没有时间差分”宣称合并输出严格客观；距离块的客观性与向量输入本身的客观性成立，直接向量Fourier后的标量描述量须单独判断。本验证不做分类性能判断。

证据：`config/Verify_ObjectiveFMTnTDO_1.1.json`、`experiments/Verify_ObjectiveFMTnTDO_1_1.py`、`outputs/Verify_ObjectiveFMTnTDO_1.1/results.json`。模块SHA256 `9ddb3bc4471d4ef3191b78567061956cdfd5bb9c11e9ebbcff888c9f18246e4c`；运行器SHA256 `3e4793bb361f96bc4f25fd7a946a5a64a7a38e14d1aafc7bb113358df33d3703`；配置SHA256 `7b331f34359dc254089c44a09eb005d7b9cb350c1cec500ae216386c4d62d5f0`；测试源码SHA256 `020c33cf140cfe3bb97855319eb31f0d7876de94694e9278e1e9cb4555439b0f`。冻结旧编码器哈希保持 `efff993f10a8db7b481784198beecce1c3c74b2f81cd2eddf5a523ba0a19222d`。本批仅本地数秒验证，无Ibex作业、模型训练或checkpoint。


<a id="task678-final-2026-09-09"></a>
## 2026-09-09 17:21+03 — mainExp_Task678_FlowMap_1.1 全部完成

本节更新此前15:44的待运行状态，不修改原协议、旧分类结果或冻结编码器。54/54 GPU分片完成，涵盖9个非定常3D条目、3种子、32/64两种token宽度，每分片评估Task6/7/8及13个方法分支。独立审计作业51630121从保存的预测数组重新计算全部2106项评估及4212行窗口指标，PASS；检查未保存checkpoint。本地只取回汇总、逐窗口CSV、审计和执行事件，核验文件SHA256，并从CSV复算78项条目/物理family宏平均全部一致；没有将本地汇总复算称为再次重放预测的独立验证。

冻结运行commit `237f4841c0c8f848cf69ad88f3743ad3eaf879e7`；配置 `config/mainExp_Task678_FlowMap_1.1.json`，SHA256 `e27e18f54c49a0da9477291f6c709f5a60cc47f4c1fb832896e5c9eaaad48e34`。远端目录 `/home/zhanx0o/FMT_Task678_20260909/outputs/mainExp_Task678_FlowMap_1.1`；本地指标副本同为 `outputs/mainExp_Task678_FlowMap_1.1/`。原summary SHA256 `7ef1c2e21de4c4281c4f0f3eab5999f5544c78235a225c9003826eae010b6b51`，CSV SHA256 `50876a448d5bd40f78f37285203f8a30dbbfcb668ffda7566d48e5c67f3649b9`。

### 主指标：对应粒子位置误差

数值为位置均方根误差除以初始primitive半径，越低越好。先对每条目的两个测试窗口及三个种子等权平均，再对九条目等权；不是九个条目混池后的单一均方根，也不是分类准确率。两个宽度分别报告，未从测试结果选择宽度。Task6是局部查询，Task7是遮挡区域补全，Task8是已知两段流映射组合。

| 方法 | Task6 / 32 | Task6 / 64 | Task7 / 32 | Task7 / 64 | Task8 / 32 | Task8 / 64 |
|---|---:|---:|---:|---:|---:|---:|
| raw_pca7 | 0.303532 | 0.290026 | 0.444083 | 0.449156 | 0.766000 | 0.723277 |
| raw_pca25 | 0.395966 | 0.370050 | 0.600800 | 0.586180 | 0.981237 | 0.922669 |
| raw_vae7 | 0.304143 | 0.290659 | 0.439706 | 0.444526 | 0.800412 | 0.746847 |
| fmt_all | 1.949635 | 2.148821 | 2.101157 | 2.057446 | 4.343471 | 4.509110 |
| fmt_all_kin4 | 5.309538 | 9.195507 | 2.208221 | 2.121627 | 18.161304 | 30.322959 |
| fmt_all_v2 | 2.183847 | 2.193167 | 2.315992 | 2.413716 | 5.512749 | 4.993864 |
| largeNeighbor | 2.338844 | 2.287358 | 1.980947 | 2.102555 | 5.289880 | 4.997345 |
| aivd1w3_dft | 1.816965 | 1.816678 | 2.378348 | 2.492446 | 4.336736 | 4.293759 |
| differential_geometry | 1.693336 | 1.755202 | 1.931569 | 1.874563 | 4.073392 | 4.005853 |
| coordinate_fourier | 0.301660 | 0.253975 | 0.386730 | 0.363289 | 0.770863 | 0.623570 |
| center_pca | 0.224237 | 0.221690 | 0.320962 | 0.319634 | 0.534295 | 0.533194 |
| coordinates_only | 1.901653 | 1.901226 | 2.351661 | 2.350292 | 4.501257 | 4.464491 |
| affine_interp7 | 0.038207 | 0.038207 | 0.124829 | 0.124829 | 0.182042 | 0.182042 |

方法定义及代码已冻结在协议和 `FMT_Utils/FlowMapModels_3D.py:raw_features`：raw_pca7/25为七线/二十五线原坐标经主成分分析，raw_vae7为七线变分自编码器；coordinate_fourier直接保留坐标Fourier系数的实部和虚部；center_pca只输入中心轨线；coordinates_only没有流动token。aivd1w3_dft的有效信息仅1维，补零不增加信息。affine_interp7读取完整七条轨线，存储量高于32/64维token，不属于同存储预算的神经方法比较。

### 相对几何与物理family汇总

下表的距离误差为同时间粒子对间距绝对误差/r；形状误差为分别去除预测与真值质心后的对应点误差/r，未消除旋转。最后一列为先在同family内平均、再对五个family等权的位置误差。其他末端、后半段、采样间时刻及四面体体积误差保存在原summary/CSV中。

| Task | 宽度 | 方法 | 粒子对距离误差 | 去质心形状误差 | family位置误差 |
|---|---:|---|---:|---:|---:|
| Task6 | 32 | raw_pca7 | 0.020499 | 0.027614 | 0.264546 |
| Task6 | 32 | raw_pca25 | 0.020425 | 0.027424 | 0.345616 |
| Task6 | 32 | raw_vae7 | 0.020572 | 0.027667 | 0.259134 |
| Task6 | 32 | fmt_all | 0.024898 | 0.033406 | 1.720127 |
| Task6 | 32 | fmt_all_kin4 | 0.025457 | 0.034457 | 4.758870 |
| Task6 | 32 | fmt_all_v2 | 0.027905 | 0.037103 | 1.982975 |
| Task6 | 32 | largeNeighbor | 0.025617 | 0.035136 | 2.034365 |
| Task6 | 32 | aivd1w3_dft | 0.029981 | 0.041183 | 1.679583 |
| Task6 | 32 | differential_geometry | 0.022113 | 0.029806 | 1.501082 |
| Task6 | 32 | coordinate_fourier | 0.020613 | 0.027689 | 0.265084 |
| Task6 | 32 | center_pca | 0.020656 | 0.027891 | 0.198089 |
| Task6 | 32 | coordinates_only | 0.033195 | 0.044101 | 1.741287 |
| Task6 | 32 | affine_interp7 | 0.004236 | 0.005065 | 0.037428 |
| Task6 | 64 | raw_pca7 | 0.020453 | 0.027590 | 0.256163 |
| Task6 | 64 | raw_pca25 | 0.020382 | 0.027428 | 0.323000 |
| Task6 | 64 | raw_vae7 | 0.020660 | 0.027691 | 0.245549 |
| Task6 | 64 | fmt_all | 0.024329 | 0.032777 | 1.879272 |
| Task6 | 64 | fmt_all_kin4 | 0.023363 | 0.031485 | 8.598182 |
| Task6 | 64 | fmt_all_v2 | 0.027158 | 0.036102 | 2.049689 |
| Task6 | 64 | largeNeighbor | 0.025207 | 0.034076 | 2.075692 |
| Task6 | 64 | aivd1w3_dft | 0.029620 | 0.040665 | 1.681163 |
| Task6 | 64 | differential_geometry | 0.021774 | 0.029424 | 1.555974 |
| Task6 | 64 | coordinate_fourier | 0.020554 | 0.027727 | 0.230110 |
| Task6 | 64 | center_pca | 0.020718 | 0.027940 | 0.195310 |
| Task6 | 64 | coordinates_only | 0.033209 | 0.043994 | 1.740860 |
| Task6 | 64 | affine_interp7 | 0.004236 | 0.005065 | 0.037428 |
| Task7 | 32 | raw_pca7 | 0.020842 | 0.027772 | 0.392819 |
| Task7 | 32 | raw_pca25 | 0.020431 | 0.027169 | 0.521126 |
| Task7 | 32 | raw_vae7 | 0.020996 | 0.028022 | 0.377330 |
| Task7 | 32 | fmt_all | 0.021445 | 0.028868 | 1.843361 |
| Task7 | 32 | fmt_all_kin4 | 0.021666 | 0.029252 | 1.922014 |
| Task7 | 32 | fmt_all_v2 | 0.020936 | 0.028985 | 2.121993 |
| Task7 | 32 | largeNeighbor | 0.019455 | 0.027527 | 1.778517 |
| Task7 | 32 | aivd1w3_dft | 0.025698 | 0.034752 | 2.387815 |
| Task7 | 32 | differential_geometry | 0.021496 | 0.029243 | 1.703199 |
| Task7 | 32 | coordinate_fourier | 0.020901 | 0.027963 | 0.345136 |
| Task7 | 32 | center_pca | 0.021037 | 0.028149 | 0.299348 |
| Task7 | 32 | coordinates_only | 0.024942 | 0.032887 | 2.155387 |
| Task7 | 32 | affine_interp7 | 0.005308 | 0.006095 | 0.124812 |
| Task7 | 64 | raw_pca7 | 0.020767 | 0.027694 | 0.392683 |
| Task7 | 64 | raw_pca25 | 0.020674 | 0.027515 | 0.512195 |
| Task7 | 64 | raw_vae7 | 0.020946 | 0.027994 | 0.386780 |
| Task7 | 64 | fmt_all | 0.021113 | 0.028266 | 1.826877 |
| Task7 | 64 | fmt_all_kin4 | 0.020899 | 0.028218 | 1.851075 |
| Task7 | 64 | fmt_all_v2 | 0.021799 | 0.029434 | 2.210408 |
| Task7 | 64 | largeNeighbor | 0.018928 | 0.026755 | 1.882007 |
| Task7 | 64 | aivd1w3_dft | 0.026586 | 0.035448 | 2.579458 |
| Task7 | 64 | differential_geometry | 0.021186 | 0.028372 | 1.660353 |
| Task7 | 64 | coordinate_fourier | 0.020950 | 0.028271 | 0.326226 |
| Task7 | 64 | center_pca | 0.020835 | 0.027984 | 0.297113 |
| Task7 | 64 | coordinates_only | 0.024856 | 0.032752 | 2.153494 |
| Task7 | 64 | affine_interp7 | 0.005308 | 0.006095 | 0.124812 |
| Task8 | 32 | raw_pca7 | 0.040616 | 0.051068 | 0.667249 |
| Task8 | 32 | raw_pca25 | 0.040448 | 0.050561 | 0.855748 |
| Task8 | 32 | raw_vae7 | 0.040649 | 0.051036 | 0.672375 |
| Task8 | 32 | fmt_all | 0.050368 | 0.063944 | 3.858135 |
| Task8 | 32 | fmt_all_kin4 | 0.052309 | 0.066336 | 16.287526 |
| Task8 | 32 | fmt_all_v2 | 0.058346 | 0.072669 | 5.022140 |
| Task8 | 32 | largeNeighbor | 0.052568 | 0.067606 | 4.576091 |
| Task8 | 32 | aivd1w3_dft | 0.061106 | 0.079096 | 4.008166 |
| Task8 | 32 | differential_geometry | 0.044176 | 0.056361 | 3.610727 |
| Task8 | 32 | coordinate_fourier | 0.040740 | 0.051090 | 0.673457 |
| Task8 | 32 | center_pca | 0.040714 | 0.051542 | 0.466159 |
| Task8 | 32 | coordinates_only | 0.070259 | 0.087387 | 4.088367 |
| Task8 | 32 | affine_interp7 | 0.008137 | 0.009300 | 0.171256 |
| Task8 | 64 | raw_pca7 | 0.040547 | 0.051089 | 0.637700 |
| Task8 | 64 | raw_pca25 | 0.040427 | 0.050796 | 0.804539 |
| Task8 | 64 | raw_vae7 | 0.040875 | 0.051105 | 0.622713 |
| Task8 | 64 | fmt_all | 0.048996 | 0.062325 | 3.968369 |
| Task8 | 64 | fmt_all_kin4 | 0.047123 | 0.059952 | 27.922176 |
| Task8 | 64 | fmt_all_v2 | 0.057027 | 0.070710 | 4.704420 |
| Task8 | 64 | largeNeighbor | 0.051020 | 0.064926 | 4.500650 |
| Task8 | 64 | aivd1w3_dft | 0.059169 | 0.077144 | 3.990733 |
| Task8 | 64 | differential_geometry | 0.043474 | 0.055699 | 3.559875 |
| Task8 | 64 | coordinate_fourier | 0.040569 | 0.051217 | 0.559242 |
| Task8 | 64 | center_pca | 0.040887 | 0.051565 | 0.465260 |
| Task8 | 64 | coordinates_only | 0.069433 | 0.085961 | 4.075422 |
| Task8 | 64 | affine_interp7 | 0.008137 | 0.009300 | 0.171256 |

### 结果解释与证据边界

1. 原fmt_all在三个任务、两种宽度、全部九条目的种子/窗口平均位置误差均高于raw_pca7，54/54项配对方向一致。宏平均误差倍数依Task6/7/8分别为32维6.423/4.731/5.670倍，64维7.409/4.581/6.234倍。三个种子的宏平均也各自同向，不是单个随机种子反转；未把种子重复当成独立物理流场显著性证据。本批不支持“现有FMT在这三个完整输运重建任务上优于Raw压缩”的主张。
2. 这不是对此前IVD分类或用户报告轮廓系数结论的反转：旧任务评价类别/聚类结构，此处评价新粒子在固定坐标系中的对应轨迹，研究目标不同。此前“提出Task6/7/8可能展示token用途”是候选任务动机；现在真实评估没有证实既有fmt_all的性能优势，不能把动机写成成功结论。
3. 实现存在明确的信息适配限制：冻结 `DFT_FMT_3D.py:dft_rotation_invariants_3d` 把每条向量序列的Fourier系数转换成实/虚部范数、夹角及三重积；`pathline_dft_features_3d` 又按特征槽对邻居排序。该输出不保留实验室坐标方向。一个可直接推导的反例是等速反向的纯平移流+U与−U：中心特征相同、邻居差分均为零，但查询轨迹沿相反方向运动。给定相同播种坐标、时间和尺度，单凭这种token无法唯一恢复二者。Raw和坐标Fourier保留方向，因此本协议同时考查绝对输运信息，不能将该差距全归因于几何描述失败。反例解释的是表示到目标的非唯一性，不声称已定量分解实际误差的全部来源。
4. center_pca在两个宽度的三个任务中均比七线Raw的宏平均位置误差低；这提示当前小半径局部查询很依赖整体平移输运。相对几何误差的差距明显小于位置误差：例如Task6/64原FMT与Raw的去质心误差为.032777/.027590，Task7/64为.028266/.027694，Task8/64为.062325/.051089。原FMT在这些宏平均形状指标上仍未超过Raw，但不能把位置误差7倍说成形状误差7倍。
5. 客观Gram方法fmt_all_v2及largeNeighbor也未恢复完整位置预测的优势；aivd1w3_dft在本批未成为好的完整输运表示。largeNeighbor在Task7的同时间距离误差上有局部优点（32/64维.019455/.018928，对应Raw .020842/.020767），如实保留，不能扩大成三个任务全面领先。两种宽度下，直接保留坐标Fourier系数的方法均明显优于原fmt_all；这不等于证明原FMT机制成功，二者输入保留的信息不同。
6. 原始七线仿射插值在三个任务的主误差均低于压缩神经方法，但使用完整轨线存储，必须注明预算差异。本实验是每流场独立训练后的跨时间测试、两段已知映射组合，不是跨流场泛化或未知未来预测，也未在本批进行观察者变换实验。

本轮仅分析并登记冻结结果，没有根据test修改特征、网络、训练步数或重跑以寻找赢家。若后续要单独评价客观几何token，需要另立版本明确区分输运坐标信息与相对几何目标，并保留本批完整负面结果。

### 2026-09-10 — Verify_RelativeFourierObjectivity_1.1：输入客观与频域变换规律

用户要求明确验证 `relative` 的客观性，并解释直接rFFT这一行的性质。新增独立验证 `experiments/Verify_RelativeFourierObjectivity_1_1.py`，对一个静止七点构造及256个固定随机种子primitive，施加所有物质点共享的Q(t)和c(t)，实际检查 `relative_star - Q(t)relative`。双精度最大残差为0（静止构造）及1.776e-15（256个随机primitive），距离残差不超过1.776e-15。**relative作为同时间相对位置向量，客观性检查通过。**没有使用“向量分量数值不变”作为其判据。

需要进一步限定此前对“相对向量傅里叶分支”的表述：

| 先前说法可能造成的理解 | 当前精确表述 | 理由与验证 |
|---|---|---|
| “relative_spectrum不客观”，仿佛完整Fourier变换破坏了客观输入或不存在正确的变换规律 | 完整频谱是原客观向量序列的可逆表示，具有由时间域Q(t)诱导的频域变换规律；不应直接把频率索引当时间索引，套用单个Q旋转或逐分量不变判据 | 全谱逆变换重建原序列后逐时间施加Q，再做Fourier，与直接变换坐标得到的频谱完全一致，本构造残差0 |
| 频域每个模长都能继承同时间向量模长的不变性 | 时变Q通常使频率之间混合；单个频率上的三分量系数及其模长不保证按一个普通空间旋转或不变标量的规则变化 | 固定输入d=(1,0,0)，32点平滑整圈旋转后，k=1系数由0变为约(16,-16i,0)，其模长从0变为22.627417，而同时间距离始终为1 |

具体公式：令F表示完整离散Fourier变换，M_Q表示在每个采样时刻乘以Q(t)的算子，则完整频谱S的观察者变换是 `S_star = F M_Q F_inverse S`。当Q不随时间变化时，可逐频率写 `S_star[k]=Q S[k]`；时变Q不能一般地从时间求和中提出，故通常混合频率。这不否定d(t)本身的客观向量性质，也不否定完整频谱作为同一几何序列的正确表示。用户所问代码另有 `:num_freq` 截断；可逆性只针对未截断的完整频谱，不把前6个频率说成完整表示。

本例k=1在当前前6频率范围内，因此后续 `_vector_descriptors` 的逐频率模长输出仍没有任意时变观察者不变性；这一输出结论保留，纠正的是把rFFT表示转换本身笼统说成“失去客观性”的表述。先构造同时间距离标量再Fourier时，输入数值不变，频谱数值也不变。

结果 `outputs/Verify_RelativeFourierObjectivity_1.1/results.json` 分别记录：relative向量客观性PASS、距离客观性PASS、完整频谱诱导变换规律PASS、逐频率模长不变性FAIL。`counterexample.npz`保存前后材料点、Q、relative及完整频谱。验证源码SHA256 `4487d2078f1bfa9f968a8de62d872885267dd28d2547bae388096b5034fa526b`，nTDO模块哈希保持 `9ddb3bc4471d4ef3191b78567061956cdfd5bb9c11e9ebbcff888c9f18246e4c`。随机种子11092026；只做代数验证，无标签、训练或分类指标。本轮未修改nTDO或旧编码器。
...
## 2026-09-10 — Verify_Task678_DirectFMTFit_1.1：完整FMT直连网络与拟合验证

用户要求去掉FMT后的主成分分析、扩大网络及自监督训练数据，允许过拟合检查拟合能力。协议已预先提交至 `docs/Task678_direct_fmt_fit_protocol_1.1.md`：完整161维冻结FMT进入网络，内部仅用训练支持统计进行可逆逐通道缩放。大网络3,504,131参数，约为旧网络59–63倍。条件为memorize16（原首训练窗口16区域，最多40000更新，完整训练误差≤0.001r方可按预设提前停）、small_base（71939参数/2000更新）、large_base（原训练数据/30000更新）、large_expanded（约8倍新播种训练数据/同样30000更新）。覆盖原九流场、种子9100，分别报告训练和留出误差；Task8复用Task6网络，长目标不进入优化。旧实验没有完整训练拟合指标，不能从旧test误差直接判断是否拟合；本轮专门补充这一证据。

本地6项验证PASS，含161维全部进入网络、真实反向传播拟合解析样本、可逆缩放、数据隔离和预测终点组合。四条件短运行完成30项评估及保存预测复算，故意篡改指标被拒绝。短运行仅3更新，不是性能结论；证据 `outputs/Verify_Task678_DirectFitSmoke_1.1/local_20260910_01/`。Python编译、Shell语法、差异检查通过。代码及协议commit `05cb78bbed88c75c695a05f01335cea64c03a6bd` 已push；运行配置 `config/Verify_Task678_DirectFMTFit_1.1.json` 的Git/Linux原件SHA256 `8a4be2e873b0c8919b19da5c1ef779accd2a6789dfcd9c6fdadb7aeb6ea2c9e2`。Windows工作副本由Git转为CRLF，SHA256为65b147daf34aedfe3bfd567ba9e77fa32e50f0a0d558bbb5a3a6211fb38cd898；只移除CRLF中的CR后与Git原件逐字节相同，未改变配置值。

独立Ibex检出 `/home/zhanx0o/FMT_Task678Fit_20260910` 固定上述commit，旧实验目录只读。首次Shell入口报模块查找失败且未提交作业；核对文件存在后改为明确目录/PYTHONPATH的独立SSH命令成功提交，科学代码与配置没有改变。2026-09-10 12:31+03提交六阶段：smoke51696908、prepare51696909、build51696910、train_base51696911、train_expanded51696912、audit51696913。真实拟合误差待计算，模型只在内存中使用。
...
### Verify_Task678_DirectFMTFit_1.1 — 九流场小训练集拟合全部完成

正式数组51696911_0–8全部完成，运行commit仍为05cb78bbed88c75c695a05f01335cea64c03a6bd、Linux配置SHA256仍为8a4be2e873b0c8919b19da5c1ef779accd2a6789dfcd9c6fdadb7aeb6ea2c9e2。下表为每流场首16个训练区域的全部非初始时刻对应点误差除以初始半径（越低越好），优化种子9100。Task6有两段支持/查询实例；Task7为隐藏区域恢复；Task8只做Task6网络的两段组合，长目标不参与优化。

| 数据 | Task6训练 | Task7训练 | Task8训练区域组合 | Task6更新数 | Task7更新数 |
|---|---:|---:|---:|---:|---:|
...
18个Task6/7网络最终训练误差范围0.000818219–0.023125945，DeltaWing resampled、Boeing747、smokeBuoyancy三个流场的两个网络都达到预注册0.001r目标，其他流场完成40000更新。Re160 Task6从未训练的8.197304315降到0.003759435，Task7从9.046948019降到0.003548373。该结果支持完整FMT输入后的大网络能够显著拟合这些真实训练样本；没有证明留出轨迹泛化良好，也不是FMT优于Raw的比较。此时完整训练集的large_base/large_expanded条件仍有作业运行，最终270项预测复算尚待全部完成。

九联学习曲线采用全部1238个已记录点，无平滑或择优截段；每个点测量全部训练粒子及非初始时刻。输出 `outputs/Verify_Task678_DirectFMTFit_1.1/figures/small_set_fitting.pdf`（另有SVG/PNG），数据副本source_snapshot.json和training_curves.csv与figure_contract.json同目录。绘图代码commit 51f4be5a971e7964e9beaeb6179a43b82e494477，`experiments/Plot_Task678_DirectFMTFit_1_1.py`；画布183×157mm，字体最低7pt，文本/碰撞/布局审计无失败，9面板逐一目视检查通过。单优化种子不绘制误差条，不把训练误差表述为泛化能力。
...
科学 commit `c3f95c9b9d5d89750fd7ff38f7ff9467c7aa1885`；配置 SHA256 `efa42423912b2dfbfbf29acf13aa6ddd7034a675c512b637931fe72a0310323a`。九流场、每流场 3072 primitive、开发种子 94110、七组方法，共 63 模型全部完成 72000 次更新。GPU 实际最多同时 18 个，训练从 2026-09-12T14:06:19.276849+03:00 到 2026-09-12T15:05:14.040336+03:00。以下全部是**开发验证集九流场等权平均，单种子**，不是最终三种子测试，不与旧 Task3 主表或旧 Task6 三种子 test 混算。

| 方法 | Average Precision（平均精确率，越高越好） | F1 | 重建均方根误差 / r（越低越好） | 分类 A 分支平均权重 | 重建 A 分支平均权重 |
|---|---:|---:|---:|---:|---:|
| FMT＋几何，各任务独立门控 | 0.683724 | 0.680349 | 0.670255 | 0.936201 | 0.062735 |
| FMT＋几何，共享一个门控 | 0.682399 | 0.683081 | 1.594269 | 0.672258 | 0.672258 |
| 两个几何专家，各任务独立门控 | 0.607363 | 0.612714 | 1.234164 | 0.327154 | 0.972199 |
| 固定分类走 FMT、重建走几何 | 0.662217 | 0.666661 | 0.420098 | 1.000000 | 0.000000 |
| 单个几何专家，联合两任务 | 0.644112 | 0.632975 | 1.168916 | 0.000000 | 0.000000 |
| 只训练分类，FMT＋几何 | 0.624508 | 0.665848 | — | 0.768910 | — |
| 只训练重建，FMT＋几何 | — | — | 0.367331 | — | 0.510728 |

A 在主模型及 FMT 对照中为 FMT 专家；双几何对照的 A 是几何专家，不能把其权重称作 FMT 权重。单任务另一头指标不适用，不计为 0。

结论：主模型分类 FMT 权重 93.6201%，重建 FMT 权重 6.2735%，支持用户提出的任务分工在本验证集上确实能够学到；门控权重不等于信息贡献。主模型相对同配置只训练分类的 AP 增加 0.059215（相对 +9.48%，8/9 流场更好），相对只训练重建的误差从 0.367331 增到 0.670255（+82.47%，仅 2/9 流场更好）。相对固定分工，AP 增加 0.021507，但重建误差增加 59.55%。因此没有证明联合模型同时优于各自单任务或固定分工；相对更弱的共享门控、双几何联合对照获得提升也不能替代这一判断。

主方法逐流场（同一选中模型的两任务指标，不拼接不同 epoch）：

| 流场 | AP | 重建误差 / r | 选中更新次数 | 分类 FMT 权重 | 重建 FMT 权重 |
|---|---:|---:|---:|---:|---:|
| cylinder3d | 0.582163 | 0.206346 | 27000 | 0.910347 | 0.001302 |
| halfcylinderRe640 | 0.590464 | 1.150898 | 3000 | 0.911419 | 0.010550 |
| halfcylinderRe6400 | 0.436951 | 0.699159 | 27000 | 0.971712 | 0.001304 |
| tangaroa | 0.607305 | 1.651782 | 6000 | 0.984722 | 0.049462 |
| deltaWing_resampled | 0.846115 | 0.196548 | 9000 | 0.956369 | 0.440387 |
| deltaWing_LBM | 0.826527 | 0.081818 | 12000 | 0.900312 | 0.021877 |
| f22raptor | 0.744340 | 1.763189 | 6000 | 0.955624 | 0.006358 |
| boeing747 | 0.853384 | 0.106322 | 21000 | 0.960222 | 0.006334 |
| smokeBuoyancy | 0.666265 | 0.176237 | 3000 | 0.875085 | 0.027038 |

五个开发对照触发原协议 `RMSE < 3` 检查：

| 流场 | 方法 | 选中模型重建误差 / r | 选中更新次数 | 同次训练曲线最小重建误差，仅诊断 |
|---|---|---:|---:|---:|
| halfcylinderRe6400 | FMT＋几何，共享一个门控 | 6.662471 | 6000 | 0.932198 |
| halfcylinderRe6400 | 两个几何专家，各任务独立门控 | 4.060040 | 3000 | 0.776229 |
| f22raptor | FMT＋几何，共享一个门控 | 4.001582 | 27000 | 1.549754 |
| f22raptor | 两个几何专家，各任务独立门控 | 3.703465 | 3000 | 1.179961 |
| f22raptor | 单个几何专家，联合两任务 | 3.605798 | 3000 | 1.278839 |

原 `selection.json` 保持 `gate_passed=false`；后续提交作业 51809312 按检查退出 1。189 个最终模型及测试标签准备均未提交，当前无该实验运行中的作业。没有抬高阈值、删除失败对照、重新选 epoch 或读取 test 来绕过检查。

**纠正先前解释：**之前认为“两项初始损失都归一化到 1，可以避免某项压住另一项” → 现在确认这只匹配了初始化尺度，不能保证训练后或选模时保持平衡。Re6400 的训练几何方差约 3549.36 r²；共享门控在第 6000 次更新的重建误差 6.662471 r，经方差归一化只贡献 0.012506，分类项却为 2.596550。后续同次训练的重建误差曾到 0.932198，但原联合评分仍选择早期模型。因此这版损失缩放和选模规则有明确问题，不能把大误差单独解释成网络完全无法拟合；也不能把曲线最小重建误差偷换为这轮正式选中模型结果。下一版应另行登记修正，尚无修正后训练结果，不能预先断言会超过基线。

证据检查：`experiments/Summarize_Task36_MultiGate_1_1.py` 核对 63 个 result/fit/selection、完整配置、子集样本数、种子、更新数和实际按联合验证评分选中的步骤，并输出 `development_metrics.csv`、`development_summary.json`。这是已保存标量记录的一致性检查，不是从模型预测重新评估。下载仅含 JSON/JSONL、日志和调度记录；无数组或 checkpoint。证据目录 `outputs/Verify_Task36_MultiGate_1.1/evidence_development`；归档 SHA256 `be1da3b99a005a06960212dc965f03ba11eea08c96dac483370b06e048b5ac13`。


### 2026-09-12 — Verify_Task36_BalancedScale_1.2 最终测试与三倍训练集

用户要求进行最终测试，并提高 FMT＋几何共享门控的性能；1.1 验证已显示共享门控不足，因此在读取任何新 test 结果前固定 3072、9216 两档训练样本。9216 为三倍，逐个验证冻结旧训练前缀的索引、几何与标签一致。主方法明确为同一个共享门控 `joint_shared_gate`，并新增等参数、相同初始化、相同共享门控的双几何输入对照 `joint_raw_shared_gate`；原七方法保留，共八组。

训练损失改为 `lambda*balanced BCE/log(2)+MSE/r²`，不再除以训练几何方差。lambda=1、4 只按九流场验证平均 `(1-AP)+RMSE/r` 选择；每个方法每个档位独立选候选，同一选择跨流场固定。模型也按该任务指标评分选同一个步骤，分类阈值仍只用验证 F1。网络、72k 更新、dropout、优化器、FMT 配方和源时间划分保持，两个档位及全部基线公平重训。此版本同时修改损失和数据量，不把相对 1.1 的变化全算作扩大数据收益。

252 个开发模型后进行 432 个最终模型，九流场、两档、三种子、八方法。最终数组随整条链提前提交，条件满足后自动执行。用户已明确要求最终测试：本版本有限但 RMSE>=3 的结果标记工程失败并保留测试，避免一项对照失败卡住整批或静默省略差结果；非有限数值和数据错误仍失败，缺记录不得报完整平均。1.1 的停止协议与历史结果不改写。

本地十项测试通过（46.661 秒）：原六项回归、新评分避免旧高误差被方差隐藏、共享门控实际拟合两任务、双几何共享门控容量/初始化完全相同，以及两档八方法完整准备/选模/最终测试/审计流程，包含提前读 test 禁止与损坏预测检测。未存任何 checkpoint。尚无真实 1.2 性能；不能预先声称达到最好。详细协议 `docs/Task36_balanced_scale_protocol_1.2.md`，配置 `config/Verify_Task36_BalancedScale_1.2.json`。


### 2026-09-12 — Verify_Task36_BalancedScale_1.2 Ibex 最终测试链部署

已 commit/push 科学代码 `d7fbf812345b9b855001a8d418eb774514c660ed` 并在 Ibex 独立 checkout 校验完整配置 SHA256，Bash 语法检查通过。2026-09-12 18:24:41（+03:00）一次提交八阶段：预检 51823057、数据准备 51823058、252 开发模型 51823059、冻结选择 51823060、测试标签 51823061、432 最终模型 51823062、18 审计进程 51823063、汇总 51823064。十项集群预检通过，已观察到 5/9 个数据准备完成，尚无模型性能。按上一轮约 10–18 分钟/模型、最多 18 GPU 粗估，整批约 10–12 小时，排队或数据读取会改变时长；该时间为估计，不是调度保证。详见 `docs/Task36_balanced_scale_protocol_1.2.md`。


### 2026-09-13 — Verify_Task36_BalancedScale_1.2 完成：三倍数据改善共享门控，但未成为两任务的全指标最优

科学 commit `d7fbf812345b9b855001a8d418eb774514c660ed`；配置 SHA256 `dfd9e7c444f5f96897ad0b1b193e8d7645e8520ca8e008c08a2f7f2cd60c5648`。252 个开发模型、432 个最终模型和 18 个审计进程均正常结束；最终审计 `audit_passed=true`，没有工程失败。每个汇总格是九流场 × 三个最终种子的等权平均（27 次），候选和 checkpoint 只由开发验证集冻结，未用 test 或未见尺度结果选择。

主方法 `joint_shared_gate` 是原 FMT Fourier 特征分支与全连接几何分支共用一个可训练门控，并同时输出 Task3 IVD p95 二分类和 Task6 七线 primitive 重建。训练样本由 3072 扩至 9216 后，其 test `AP / F1 / RMSE/r` 从 `0.62097 / 0.66699 / 0.33742` 变为 `0.69106 / 0.70679 / 0.21918`；未见尺度从 `0.62399 / 0.67551 / 0.34940` 变为 `0.70938 / 0.71827 / 0.23198`。因此三倍数据同时提升分类和重建，测试重建误差下降 0.11823 r（35.0%），未见尺度下降 0.11742 r（33.6%）。

9216 样本的关键公平对照如下：

| 方法 | test AP / F1 / RMSE/r | 未见尺度 AP / F1 / RMSE/r | 结论边界 |
|---|---:|---:|---|
| FMT＋几何共享门控（主方法） | 0.69106 / 0.70679 / 0.21918 | 0.70938 / 0.71827 / 0.23198 | 联合模型中 F1 最好；重建优于单独 Task6 |
| Raw＋Raw 共享门控（等容量、等初始化） | 0.69827 / 0.68475 / 0.18489 | 0.70154 / 0.69217 / 0.19519 | FMT 使 F1 提高，但 Raw 的 AP 略高、重建更低误差 |
| 任务专用双门控 | 0.69410 / 0.69319 / 0.20798 | 0.69964 / 0.70010 / 0.22028 | test AP 和重建均优于共享门控，F1 较低 |
| 固定路由（分类用 FMT，重建用几何） | 0.65980 / 0.67694 / 0.17310 | 0.66894 / 0.68438 / 0.18117 | 重建最佳，分类较弱 |
| 单独 Task3 | 0.71964 / 0.70122 / — | 0.73829 / 0.71521 / — | 分类 AP 最佳，联合训练仍有负迁移 |
...
### 2026-09-13 — mainExp_Task4C_HairpinBinary_2.1 六次实验与独立复算完成

科学commit `63b44d8c9cd9d4a6d2d7ec84eed255202f127571`已推送到用户既有仓库；先前的项目整理单独提交为`a474dcf9`。
Ibex独立checkout `/ibex/user/zhanx0o/FMT_Task4C_HairpinBinary_20260913`，配置SHA256 `fc896a75bd7ba6a5b98448e4f030ba79b44d553977be2178f13c307086da45e4`。
数据准备51857585、六模型数组51857586[0-5]及汇总51857587全部COMPLETED/exit0；GPU均为V100-SXM2-32GB，最后汇总于13:59:55 UTC结束。

正式train/validation/test分别30000/7500/7500条线簇，45000条全部有效。
负/正类数分别24660/5340、6494/1006、6544/956；实际正实例44/8/7个，原生插值节点和实例ID均隔离。
下表为固定同一空间划分、三个优化种子的均值±样本标准差，不是三个独立流场。

| 方法 | 参数数 | Hairpin F1 | 平均精确率AP | 平衡准确率 |
|---|---:|---:|---:|---:|
| FMT＋MLP | 74,497 | 0.316073 ± 0.010245 | 0.295637 ± 0.008707 | 0.631163 ± 0.005041 |
| Conv3D＋MLP | 84,361 | 0.265871 ± 0.036398 | 0.270663 ± 0.050074 | 0.578571 ± 0.022270 |

FMT的平均F1高0.050202（5.02个百分点），三个配对种子均提高；平均AP高0.024973，但种子94212的AP低于Conv3D。
**本次初始配置支持FMT在该GT二分类benchmark上的F1相对收益，但两方法绝对F1均偏低，尚未解决可靠hairpin识别。**
FMT三个种子的F1为0.306685/0.314532/0.327001；Conv3D为0.279196/0.293728/0.224688。
最优验证epoch分别为FMT 2/3/2、Conv3D 1/1/4；实际执行22/23/22及21/21/24epoch，按预定20epoch patience停止，并非只训练1–4epoch。
没有根据这些测试成绩重新选择模型、改阈值、改输入尺度或追加参数搜索。

**独立复算**：归档下载后，不导入正式指标函数和sklearn，用NumPy混淆计数及按相同分数成组的精确率–召回率求和，
复算六次F1/AP/平衡准确率、均值及标准差，误差均小于1e-12；验证阈值和所选epoch一致。
六方法/种子使用相同7500个测试中心、标签、实例与源单元编号；另用独立VTK读取对全部7500个中心重新查询原始GT，标签和实例编号逐项一致。
证据`outputs/mainExp_Task4C_HairpinBinary_2.1/independent_prediction_audit.json`，14:04:10 UTC完成。
复算以临时命令执行，没有新增保留的验证源码。完整证据归档SHA256 `063c9547af535904c5311635ee2be1afb3e69654f798b1231f1043d273bc92cf`。
归档包含预测、指标、配置、运行登记及仅均值/标准差的特征统计；已检查无`.pt/.pth/.ckpt`或神经网络权重。

**记录修复**：部署时发现运行器与工具模块同名，而identity字典只用basename，导致运行器哈希键被覆盖。
原科学commit仍唯一确定源码；`deployment_manifest.json`已补充两个完整路径的独立SHA256。
随后仅修复本地记录函数的键名并检查三条源码记录齐全，不改变算法、配置或已完成的科学checkout；运行器原SHA256为`5e6cf749b5a1ffba79df61cf53a06b3ca0cefec4afb7a678b94243c16e254f83`。
结果文件为`results.json`、`metrics.csv`和六个`runs/*/result.json`、`predictions.npz`；方法比较与Task1/2/3/5主表分开。


<a id="task4c-paper-bundles-2026-09-13"></a>
### 2026-09-13 — Task4-c论文预处理修订 mainExp_Task4C_PaperBundles_3.1

用户要求按论文3.1.1加入lambda2/正展向脉动涡量头区、RK45双向涡线、短线/轴退化清洗、少于10有效线整束剔除、32点等弧长重采样及整束质心/Rmax归一化。
用户确认Channel和TBL合计30,000训练开发+1,000测试primitive，每流场13,500拟合+1,500验证+500测试。完整定义和所有适配见`Task4C_paper_bundles_protocol_3.1.md`。

| 之前2.1的实际定义 | 当前3.1 | 修订原因与证据边界 |
|---|---|---|
| Channel单帧，lambda2候选中心，固定七线；积分可离开候选区域 | Channel+TBL，从lambda2/正oyf头区播种，在候选网格内RK45追踪及清洗，最多256线 | 2.1并非没有lambda2筛选，但没有实现论文头区、线束清洗及整束归一化；源代码和原始记录支持此区分 |
| 单个中心是否属于GT | 完整头区同一实例覆盖至少50%为正，完全无重叠为负，其余排除 | 两帧只有区域标签；这属于本项目的区域转整束标签适配，不是论文人工四类标签 |
| 30,000/7,500/7,500个局部中心 | 27,000/3,000/1,000个不同线子集，头区/完整实例/源节点隔离 | 多视图不代表独立物理涡数；同时记录头区数量和每区一票指标 |

不能把未来2.1与3.1的分数差全部归因于单一清洗操作：数据、标签、线簇范围及FMT整束聚合都已改变。2.1结果不覆盖。
新方法版本：冻结161维FMT作用于每条线及六个最近种子邻居，整束均值+最大值形成322维，MLP共115,713参数；Conv3D沿用2.1网络84,361参数，输入同束24³占据率/有符号单位切向量体素。不是等参数对照。

**核验记录 Verify_Task4C_Preprocessing_3.1**：解析螺旋场RK45半径最大误差3.339985725270278e-6；整束平移/尺度归一化、线顺序置换、补零mask、短线/退化/不足10线拒绝、两网络反向传播通过。
真实两帧各抽一个正头和一个负头，共各1,024个种子：Channel剔除626条短/缺失线、保留398条；TBL剔除466条、保留558条。本次抽查无轴退化或非有限线；不推断全量无此问题。
两流场各20个不同真实线子集的构造及归一化通过；随机数据的训练/验证选模/测试读取/汇总路径也通过，该工程夹具的分数没有科研含义。
证据`outputs/Verify_Task4C_Preprocessing_3.1/validation_report.json`，保留源文件哈希与报告，临时验证脚本和合成夹具删除。

本条写入时尚未提交正式3.1训练，尚无其模型性能。Ibex作业、完整采集数量和终态后续登记，不以代码通过预设FMT获胜。


### 2026-09-13 — Task4-c 3.1 完整采集通过

科学commit `f1865b160a2c4784f66fddd07cd27fbef3d2b2df`已推送；Ibex准备数组51859013[0-1]均COMPLETED/exit0，UTC14:47:32开始，Channel于14:48:33、TBL于14:48:36结束。
运行环境Python3.12.9、VTK9.5.0、NumPy1.26.4、SciPy1.15.1、PyTorch2.6.0+cu118。

| 流场/集合 | primitive数 | 负/正类 | 独立候选头区 | 正类GT实例 | 每束有效线数 |
|---|---:|---|---:|---:|---|
| Channel拟合 | 13,500 | 10,711 / 2,789 | 518 | 42 | 18–256 |
| Channel验证 | 1,500 | 1,355 / 145 | 114 | 5 | 122–256 |
| Channel测试 | 500 | 429 / 71 | 95 | 7 | 39–256 |
| TBL拟合 | 13,500 | 11,779 / 1,721 | 615 | 26 | 11–256 |
| TBL验证 | 1,500 | 1,238 / 262 | 294 | 14 | 16–256 |
| TBL测试 | 500 | 424 / 76 | 168 | 6 | 11–256 |

Channel追踪385,536个种子，剔除120,361条短/缺失线，265,175条通过单线清洗；TBL追踪573,440个种子，剔除267,106条短/缺失线，306,334条通过。
两者本轮轴退化/非有限计数均0，相关检查仍执行；通过单线清洗不等于其整束最终被保留，另有不足10线及接近人工空间边界等整束剔除。
...
### 2026-09-13 — Task4-c 3.1 六次训练与完整汇总完成

此前“训练等待Priority”是提交后的状态；现已从Ibex调度记录确认全部训练与汇总COMPLETED/exit0。
科学commit `f1865b160a2c4784f66fddd07cd27fbef3d2b2df`，训练数组51859015[0-5]，汇总51859016。
UTC15:04:43（沙特18:04:43）全部完成，从UTC14:47:31提交起约17分11秒，包含调度等待；不需要继续等待作业。

下表是同一空间划分的三个优化种子均值±样本标准差，测试合计1,000个primitive（853负/147正）。

| 流场/范围 | FMT F1 | Conv3D F1 | FMT AP | Conv3D AP |
|---|---:|---:|---:|---:|
| pooled | 0.351928 ± 0.013785 | 0.212361 ± 0.113357 | 0.309204 ± 0.048810 | 0.220226 ± 0.014693 |
| channel | 0.320919 ± 0.010015 | 0.256299 ± 0.127355 | 0.278353 ± 0.037311 | 0.274811 ± 0.030050 |
| tbl | 0.383739 ± 0.022667 | 0.157414 ± 0.093949 | 0.355387 ± 0.063329 | 0.184251 ± 0.041470 |

**本次固定3.1比较支持FMT相对于所用Conv3D基线的平均F1/AP收益，但绝对F1仍低，不能声称已实现可靠hairpin识别。**
合并F1提高0.139567（13.96个百分点），三个配对种子均提高；但Channel的种子94312 FMT F1低于Conv3D（0.325879 vs 0.402985），不能声称所有单流场/种子都获益。
合并AP提高0.088977；Channel平均AP差仅约0.003542，TBL差约0.171135。Conv3D合并F1标准差0.113357，稳定性有限。
FMT三个模型均执行21epoch，验证选择第1epoch；Conv3D实际32/38/22epoch，分别选择12/18/2epoch。FMT训练损失持续下降但后续验证AP未超过第1epoch，显示在该划分上后续训练泛化没有改善；没有据此追加测试驱动调参。
按每个候选头区平均概率后一票，Channel FMT/Conv3D平均F1为0.316789/0.228070，TBL为0.348791/0.132512；仍是候选头区指标，不是独立物理实例/曲面检测指标。
FMT参数115,713、Conv3D参数84,361，不是等参数比较；多个primitive可来自同一头区，独立测试头区Channel95、TBL168。
3.1与2.1同时改变了数据、标签、线簇和聚合方式，不能把两版分数差单独解释为预处理的贡献。

**完成证据与核验**：下载归档只含配置、日志、准备/编码清单、六次逐样本预测及指标，没有模型权重。
...
### Verify_Task4C_DirectionalSpectrum_4.1：完成，未达到目标

科学commit `069c9ef6`，config SHA256 `3afbc845c6e086009472913b49dbcb80f3d5be8bdb1f161a7cd17cf2ee127f98`。encode51861017、train51861018[0-11]、merge51861019均完成。下面均为固定4.1验证集3000束（2564负/436正）的单种子94611结果，不是测试成绩。

| 表示 | 最好验证F1 | AP | lr / dropout |
|---|---:|---:|---|
| gram6 | 0.302686 | 0.191231 | 0.0003 / 0.5 |
| signed6 | 0.389904 | 0.305817 | 0.0003 / 0.5 |
| signed17 | 0.364865 | 0.268252 | 0.0001 / 0.5 |

四组配对lr/dropout设置中signed6均高于gram6，支持在这批数据和网络设置中保留空间方向的作用；输入维数/参数量不同，不能称为等参数结论。signed17四组均未超过对应signed6，因此当前没有支持增加到全部频率的证据。即使改为有方向的频谱仍低于0.6，不能归因于单一旋转不变性就解释全部误差。
整个新系列相对原FMT还改变了差分输入和池化，不替代原FMT322维结果。主实验首轮原FMT/Conv3D最佳验证F1为0.305569/0.429119，三种子复核仍在运行；3.1测试FMT高于Conv3D与4.1当前验证排序属于不同数据/阶段，不能静默合并成一个结论。
已独立用NumPy复算主实验当前24次（20搜索+4FMT复核）及本核验12次的F1、AP、验证阈值最优性和预测哈希，全部通过，未读取测试概率。报告位于两输出目录的`independent_development_audit.json`。主实验当前证据包SHA256 `3febed467f8e7ba225d0e39adf3bea9507b5e8b3ace43f16962aa27fdc0000de`（复核未全结束），方向核验完整包SHA256 `985c516e1d153d0dc57b4fcb93071873639f9487a0745e0059e9ef40a6847a0b`；包内不含权重。
针对坐标/涡量分量一致性的额外核验只使用Channel训练区20000点：速度有限差分旋度与存储涡量三个同分量相关系数为0.9999895/0.9999980/约1，相对均方根误差0.004723/0.002018/0.00000248；未发现轴交换或符号错误。该检查不证明标签充分或几何表示最优。报告`channel_velocity_curl_consistency.json`。

### mainExp_Task4C_Regularized_4.2：固定数据上的方向/切向频谱、增强与分类间隔

4.1物理样本、尺度、标签和测试10,000束完全不变。原322维FMT原样拼接288维坐标/单位切向的有方向六频统计，非训练编码器共610维。通过六候选×两模型比较正类训练镜像/倾斜增强、普通两类线性头及余弦间隔头；具体技术、假设与边界见`Task4C_regularized_protocol_4.2.md`。只以训练/验证选择，不宣称已经提高F1。
科学commit `6b67d2ef`，config SHA256 `5ce777f6fd3392f8e93ebf466c26047d28d3f52f2f865b74d621606aded8c664`。工程核验原FMT前缀完全一致、增强几何性质、两网络/两损失反向传播及真实小规模完整训练通过，4.2测试编码目录不存在；证据`outputs/Verify_Task4C_RegularizedCode_4.2/checks.json`。UTC16:58:02提交encode51861332、search51861333[0-11]、rank51861334、confirm51861335[0-7]、select51861336。公开推送等待明确授权；已通过本地提交和个人Ibex部署继续实验。

#### 4.1最终开发结果：28次训练均完成，未进入测试

UTC16:57:24选择完成，所选两模型均为candidate05（lr3e−4、dropout0.5、weight decay0.01、label smoothing0.05）。

| 方法 | 三种子验证F1均值±样本标准差 | 三种子验证AP均值 |
|---|---:|---:|
| fmt_mlp | 0.301773 ± 0.004672 | 0.194824 |
| conv3d_mlp | 0.420529 ± 0.021411 | 0.322929 |

两者均未达到验证门槛0.6；测试未解封，无最终测试指标。4.2继续使用同一预先固定物理样本，不能将4.1验证指标与历史3.1测试指标直接比较或混入论文主表。
完整证据包SHA256 `5d7e24666e330b62a5db0b33a66529633390739dae90ae8ef5943a606b394371`；`independent_development_audit_completed.json`独立复算全部28次预测的F1/AP、验证阈值及三种子均值通过。旧24次部分证据/报告保留，当前以此完整记录为准。

4.2编码完成且源码/配置哈希一致，test_encoded=false。无增强拟合27,000束；正类镜像/倾斜后拟合35,662视图（Channel19,158，TBL16,504），物理原始样本仍27,000；验证3,000。候选内两方法共享完全相同的训练视图。不同候选增强选择可能不同，优选结果用于各模型能力判断；FMT与Conv3D的表示比较须同时提供匹配增强/分类头的候选结果。增强情况下每epoch优化步数随样本数增加，因此不能把其增益全部归因于几何变换本身。

### Verify_Task4C_LabelScope_4.1：训练/验证中心归属核查

来源4.1原始metadata哈希与两流场GT哈希，使用vtkStaticCellLocator.FindCell，ID0有效，完全不读取test。Channel训练正类中心22/2829在GT外、验证0/169；TBL训练104/1502在GT外、验证31/267。所有负类中心均在GT外。当前标签描述完整候选头区，而中心归属是局部检查，差异不自动构成错标；不重标、移除或选择这些样本。该结果不支持用大面积中心错标解释全部低F1。证据`outputs/Verify_Task4C_LabelScope_4.1/report.json`及逐样本membership文件。

### mainExp_Task4C_LinePooling_4.3：逐线学习聚合与卷积容量对照
...
### Verify_Task4C_AnnotationProvenance_4.1：伴随文件核查与执行前提修正

当前GT文件哈希仍与冻结配置相同。只读检查两流场各4份GT/Regions伴随文件的数组模式和已有8个验证案例中心，未查阅测试预测或据此改变标签。文件含区域/实例编号及流场量，没有人工Non-hairpin类别数组或标注完备性声明；数据目录内也未定位到这两帧的标注说明。仅由这些文件不能确认GT完整性。

`*_Regions_Split_hairpinRegion_with_ids.vtk`不是指定GT的可互换副本：Channel验证1421与TBL验证846在当前GT内，却都不在伴随hairpin文件内；两个最高分误报中心在两套文件中都位于标注之外。这里只检查8个验证中心，不能估计整体差异或判断哪套文件物理正确。完整哈希、数组模式和逐例查询见`outputs/Verify_Task4C_AnnotationProvenance_4.1/{metadata,case_membership}.json`；未把伴随文件作为新监督。

重新定位到9月9日历史记录：用户曾报告Task4人工标注存在不少错误；这是用户当时的报告，没有独立错标比例。本轮2.1/4.1协议早已声明当前指标衡量指定GT成员识别，不证明所有GT外结构物理上非hairpin。因此修正上一轮执行判断：

| 先前判断 | 修正后 | 原因 |
|---|---|---|
| GT完整性回复是继续训练前提 | 固定GT benchmark可继续；完整性限制物理结论，回复不作为额外运行门禁 | 先前混淆了科学解释的条件和已授权任务的执行条件；现有标签、测试、目标均无需改变 |

### mainExp_Task4C_ScaleConsistency_4.5：同中心物理尺度配对

基于4.3保存预测中的尺度分歧，新增同中心、不同有效物理尺度的成对训练。两分支都有原GT监督，概率均方差只约束真实不同尺度对；零权重候选保留相同配对抽样/计算量。不是重标、额外快照、测试多视图平均或更换任务。六候选×两模型，learning rate3e−4/1e−3及一致性权重0/1/10；完整方法来源、代码与预注册参数见`docs/Task4C_scale_consistency_protocol_4.5.md`。仍要求各方法三种子固定测试F1均值≥0.6，不预设此损失有效。
...
4.6科学commit `8b755d23a179c7f37644e95bac933311162db059`，config SHA256 `2331c062d10394929e8be4283293e78a91b62b9327bab96ef6a35947c8ca543e`。私有Ibex目录`/ibex/user/zhanx0o/FMT_Task4C_FinalAssessment_20260913`，增量传输包SHA256 `1c31c077eee27c2495b715d92de0edcaea42767fc8414003e7d54b044c2db828`。UTC19:25:03提交prepare51863465、encode_test51863466、train51863467[0-5]、merge51863468，均已自动登记。选择冻结UTC 2026-09-13T19:25:03.136704+00:00，selection SHA256 `c8910d7c92cb1c9b9f55f5dc40a43bad5c7c2b30ac763ee7b26b24f0469ac83e`。两个方法均复用4.5，测试结果尚未产生。


### mainExp_Task4C_FinalAssessment_4.6：最终测试完成，目标未达成

科学commit `8b755d23a179c7f37644e95bac933311162db059`，config SHA256 `2331c062d10394929e8be4283293e78a91b62b9327bab96ef6a35947c8ca543e`。UTC19:34:27完成；prepare51863465、encode51863466、train51863467[0-5]、merge51863468共9个Slurm任务全部COMPLETED、exit0。固定选择锁`c8910d7c92cb1c9b9f55f5dc40a43bad5c7c2b30ac763ee7b26b24f0469ac83e`，两个方法均来自4.5已选配置，三个全新种子95811/95812/95813。以下是相同10,000束真实测试成绩；均值±样本标准差。
...
FMT三次合并测试F1为0.326291、0.324145、0.356141；Conv3D为0.325432、0.267831、0.320581。两模型均未达到0.6，`target_reached=false`，不将目标标记完成。此前4.5开发验证0.399091/0.436234与本表测试0.335526/0.304615不是同一个集合或种子；验证结果没有被改写，更不能代替测试结果。两方法各自选优的一致性损失权重不同，本结果不是仅替换FMT的严格匹配消融。

本版覆盖用户要求的实际邻居距离/积分长度/积分参数变化、同位置多尺度、扩大测试、dropout/weight decay及学习率选择；这些尝试没有实现所要求的测试目标。Conv种子95812在选中epoch的训练诊断F1为0.818421、验证0.435784、测试0.267831，说明该次较高训练分数未转化为测试表现。不能据这些结果断言所有几何编码无效、证明GT错标或归因于单一预处理步骤。现有GT语义和两帧数据限制仍按原协议保留；没有修改标签、删除困难测试样本或调测试阈值。
...
### Verify_Task4C_TrainingInstanceBalance_4.7：训练抽样集中度核对

仅检查冻结完整训练metadata。Channel42个正GT实例覆盖105个正头区，TBL26个正实例覆盖58个正头区；当前head均匀→view均匀抽样下，合并68个正实例中最大一个占正类抽样概率3.681%，前5个合计15.337%。按1/sum(p_i²)计算的有效实例数55.237（这是权重集中度指标，不是新增物理实例数量）。Channel/TBL最大单实例份额分别5.714%/6.897%。

没有发现由一两个标注实例占据大部分正类训练抽样的问题；不能据此排除所有数据不平衡，也不证明重采样无用。当前没有足够证据把改采样权重加入新训练分支，继续等4.7编码分辨率对照。证据`outputs/Verify_Task4C_TrainingInstanceBalance_4.7/report.json`，没有验证/测试访问、没有改标签/训练权重，核验以内联命令执行。

4.7首轮部分结果（seed95911，尚未三种子复核）：6频率FMT在lr0.0003/0.001下验证F1为0.343891/0.346021，17频率为0.321918/0.324357；两个学习率下分别差−0.021974/−0.021664。24³ Conv为0.411125/0.432532，48³仍运行。这里仅支持该首种子下全频率未改善，不代替三种子结论，更不计作测试成绩。

早先已下载的两次低分辨率预测在本地独立复算通过（最大误差1.11e−16，证据`outputs/Ablation_Task4C_RepresentationResolution_4.7/independent_partial_audit.json`）。随后尝试下载包含六次完成结果的归档被自动审批拒绝，理由为未明确授权这些具体非公开预测/日志从Ibex到本地。原归档SHA256 `325fbb864a698cb346ceaff46fd8c0d0c1ab4089dd0529a6777baea8113bc6c2`，保留在Ibex；没有换通道下载或导出原始数据。已询问两处个人工作目录间结果同步权限，等待回复。

作为不传输数据的替代，六次预测的F1、平均精确率、阈值/epoch与哈希在原Ibex目录内用独立NumPy计算复核，全部通过。详细审计保存在远端`outputs/Ablation_Task4C_RepresentationResolution_4.7/independent_partial_audit_remote.json`，本地仅收到检查通过/运行数状态；没有下载新预测或日志。已在远端registry登记。该下载限制不停止已授权的远端训练和分析，不把它解释为F1未达标的原因。

4.7仅作调度优化：48³首轮一项在A100-SXM4-80GB、一项在P100-PCIE-16GB，后者较慢。待运行的两次48³复核限定到原允许设备集合中的A100/V100；当前运行保持，算法/配置/数据/种子不变。资源变更已登记，不依据分类分数更换硬件或重跑种子。

### Ablation_Task4C_Sharpness_4.8 — 固定4.5的SAM正则化对照
...
### Verify_Task4C_GeometryCorrespondence_4.8：训练几何对应检查

只检查原4.1训练pilot，每流场120束，将原种子投影到32点折线以估计它的弧长分数；不是重新积分或精确原始轨线误差。Channel/TBL束内种子弧长分数跨度的中位数为0.009177/0.012721，即0.2845/0.3944个采样间隔；跨度大于两个间隔的束分别占10%/4.17%。该检查没有发现整个训练pilot普遍存在较大的种子起点偏差，不足以把改重采样作为当前修复，也不能排除少数束的影响。证据`outputs/Verify_Task4C_GeometryCorrespondence_4.8/report.json`含全部源哈希，无验证/测试访问，未改旧缓存。

传输权限补充：自动审批允许只读远端聚合验证F1，用于当前目标进度检查；它不等于允许下载此前被拒绝的逐样本预测/日志归档，相关问题仍待用户回复。4.8 seed96011已有FMT rho=0/0.05验证F1 0.382203/0.387585，仅单种子小幅变化，无三种子结论。4.7 48³ Conv、lr0.0003完成，验证F1 0.433684，40epoch耗时884.97秒；同学习率24³为0.411125、另一学习率24³为0.432532。只读汇总不导出新预测或日志，冻结read_run在远端复核一致性通过。

### Ablation_Task4C_CenterDiversity_4.9：固定视图数量、增加训练中心

完整训练覆盖核对：Channel/TBL每头区中心数中位数4/3；27,000训练视图来自4,460中心。证据`outputs/Verify_Task4C_GeometryCorrespondence_4.8/training_center_coverage.json`。新对照保留每个旧中心最多两视图，其余按原head/scale精确配额重新采样中心；原训练视图Channel/TBL分别保留4,368/4,203，待替换9,132/9,297。保持GT标签与实例计数、尺度参数、训练总量以及验证/测试。新旧两组各用三个相同种子96111–96113，共12次；固定4.5参数，只比较数据采样。见`Task4C_center_diversity_protocol_4.9.md`。

本地工程检查：无标签依赖的保留计划、配对原子接受、配额/标签/实例/中心篡改拒绝、无test阶段通过；两组使用相同真实缓存时的12次训练/汇总逐种子结果相同，验证路由无偏差。较早本地pilot的4.1 runner校验值20366e74与冻结正式版920b48a8不同，来源保护正确拒绝；没有修改旧manifest或放宽生产保护。Ibex完整正式数据的所有生成源码哈希均与当前一致。因此实际新采样核验在Ibex进行：每流场先选原训练数据中各两个正/负头区，经真实追踪与配额审计成功后，依赖链才进入完整生成和训练。不能把尚待完成的远端核验说成已通过。

科学commit `4ac7e6ab30e91d1e1169d738161d81306f4cd22b`，紧凑配置SHA256 `b05dbc41028da799de8403efe28419c989c1c9c38a86fe53fead43981af21ae5`，父4.8配置SHA256 `f32d089d6e76d33c0323c750da7caa4d1bbf3d850f6d60762b92457172031d15`，代码包SHA256 `1c18b6ea7ff7c80b0b44729b3d35274eb3874639ee9acdc955b8e22795d23985`。个人Ibex路径`FMT_Task4C_CenterDiversity_20260914`，仅传输新runner/config/bash，不含预测或实验归档。已提交7个阶段、18个Slurm任务，尚无科学效果结论。

4.9实际数据完成：preflight51865061成功，用Channel94/TBL81个原训练视图检查，中心分别13→49及13→42，配额和保留几何均一致。完整训练生成随后完成，Channel13,500视图/6,932中心、TBL13,500视图/6,943中心；合计13,875中心、13,125个双视图中心与750个仅有一个尺度视图的中心。原head/scale/标签/实例数量不变，保留旧视图4,368/4,203。独立NumPy从落盘metadata与geometry再次检查配额、物理中心唯一性、线数/退化/有限值、质心与最大半径归一化、旧几何逐值保留和训练分区边界，全部通过；报告保存在原Ibex`outputs/Ablation_Task4C_CenterDiversity_4.9/independent_training_data_audit.json`。只返回状态和总量，未导出几何或逐样本数据。尚无分类收益结论。

### Ablation_Task4C_Sharpness_4.8：完成，三种子未显示足够收益

科学commit81533108，全部6次搜索+8次复核结束。三种子验证F1（均值±样本标准差）：FMT rho0=0.395441±0.012713，rho0.1=0.392568±0.009520；Conv3D rho0=0.425527±0.010460，rho0.1=0.429179±0.002506。选择为FMT普通AdamW、Conv3D rho0.1。配对平均差分别−0.002874、+0.003652；不能声称SAM为两方法带来稳定或足以达到0.6的收益，不追加半径搜索。这只限定当前数据/网络/设置，不概括所有SAM应用。

此前记录单种子FMT轻微提高→现在三种子平均略低；原因是当时只完成seed96011，其正差未在其它种子稳定重现，之前没有三种子收益证据。配置/选择规则未变。独立NumPy在原Ibex目录复算14份预测的F1、平均精确率、平衡准确率、混淆计数、验证最优阈值、选定epoch和三种子均值/标准差，全部通过，最大误差1.11e−16，同一3000验证样本，test未读取。报告`FMT_Task4C_Sharpness_20260913/outputs/Ablation_Task4C_Sharpness_4.8/independent_final_audit.json`及完整原预测留在Ibex。只读取聚合分数和通过状态，没有下载受限归档。

4.9首种子96111的FMT原数据/更多中心数据验证F1分别0.412729/0.392308；各自选定模型的训练诊断子集F1为0.400593/0.614806。这是单种子、各自验证选定epoch/阈值以及不同训练子集的诊断，不将其差解释为严格训练拟合因果效应，也不代替三种子结论。保持全部预注册复核，不能因首种子降低而取消数据组。

### Task4-c 4.7/4.9：2026-09-14部分三种子复核

上一目标轮只回答状态，没有改变方法或完成实验，按无进展重新核实；本轮调度器确认原作业仍在运行，未重启或新增替代任务。对4.7已完成14份、4.9已完成9份预测在原Ibex目录内独立NumPy复算合并/分流场/分头区/分尺度F1、平均精确率、平衡准确率、混淆计数，检查验证阈值、选定epoch、3000样本身份及源码/预测哈希，均通过，最大误差1.11e−16。尚无完整selection，不能声称已核验最终选择。报告分别存于原输出的`independent_validation_audit_20260914.json`，未传输逐样本预测或训练日志。

4.7（科学commit8d483808）FMT6/17频率已选学习率均0.001，种子95911–95913验证F1分别为[0.346021,0.406746,0.343685]与[0.324357,0.387513,0.336581]，均值±样本标准差为0.365484±0.035753与0.349484±0.033497。三个配对种子均降低，平均−0.016000；当前等参数逐线学习聚合设置下，全频率未改善，支持不继续追加频率搜索。它与旧固定统计聚合signed17负结果方向一致，但两版算法/数字仍分开。24³ Conv三种子验证0.427184±0.014001，48³仍待两次完成，暂不判断分辨率对Conv的效果。

4.9（科学commit4ac7e6ab）FMT原采样/更多中心的种子96111–96113分别为[0.412729,0.401480,0.432331]与[0.392308,0.402421,0.420108]，均值±样本标准差0.415513±0.015613与0.404945±0.014071。配对平均−0.010568，两负一正；目前不支持这套增加中心策略改善FMT，不因小样本负结果概括所有数据扩充无效。Conv复核未全结束。4.9原采样组是4.5同配置、不同种子的复跑；此前4.5验证0.399091→这里0.415513，是种子组变化，不能作为新方法增益或择优替换4.5原记录，更不能改变4.6测试0.335526/0.304615。目标仍未实现。

### Ablation_Task4C_CenterDiversity_4.9：全部完成，保留原采样

UTC2026-09-13 21:41:17选择完成。Conv3D原采样/更多中心三种子验证F1为0.440833±0.006090 / 0.407675±0.016259；逐种子96111–96113分别[0.442060,0.446215,0.434223] / [0.401606,0.426096,0.395324]。三次配对差均为负，平均−0.033157。FMT结果与上段一致，选择器两方法均保留原采样。验证平均精确率原采样/更多中心为FMT0.326311/0.301388、Conv3D0.355421/0.323991。当前证据不支持继续沿这套保持视图总量、增加中心的策略扩大搜索；不概括所有数据扩充无效。

补齐剩余3次预测的独立NumPy复算，并检查先前9次证据文件未变、完整源码/配置身份、阈值/epoch、全部分组指标、同一3000验证样本、三种子F1/平均精确率汇总与最终选择，全部通过，最大误差1.11e−16。18/18调度任务COMPLETED、exit0，无模型文件或test读取。selector SHA256 `d9f43b58ec026d4933fc99479018fac32f21a56057c737fe6bc56b04a290fb3f`；报告留在原Ibex`outputs/Ablation_Task4C_CenterDiversity_4.9/independent_final_audit.json`。没有导出受限预测或训练日志。

### Ablation_Task4C_SupervisedContrastive_4.10：固定分类器，辅助特征损失

4.9完整结果为两方法负消融，保留原4.1采样和4.5配方。新版本在原分类器64维隐藏特征上增加训练投影器及Khosla等NeurIPS2020式(2)监督式对比损失，检验同类线簇的特征约束能否改善当前分类。该假设尚无本任务收益证据。Task3的`Verify_Task3_SupervisedContrastive_18.1`已使用类似技术且未明显提高其FMT绝对成绩；不宣称首次使用或推翻旧结论。原4.5分类器、数据、dropout、学习率、尺度一致性与推断函数保持，权重0/0.1/0.5、温度0.1；这是联合辅助损失改编，不是原论文两阶段训练复现。完整来源、公式与边界见`docs/Task4C_supervised_contrastive_protocol_4.10.md`。

工程核验全部通过：两个网络零权重连续三步与原模型参数/预测/随机数逐值一致；独立NumPy公式及有限差分梯度最大误差7.32e−10；排列、无正样本、同表示、非法温度和test拒绝；正权重全部参数梯度有限；真实缓存240训练束的14次pilot训练、零对照保留与三种子选择。记录集中`outputs/Verify_Task4C_SupervisedContrastiveCode_4.10/checks.json`，没有新增一次性test/verify代码文件。pilot分数只用于工程检查，不能计入科学成绩或0.6目标。

科学commit `6bfc357bc8d9ac321f91cfb512eb39d2facf0332`，config SHA256 `d2a1d8df0b2f71416be8dd125107c9770cc1a92ef846167184ddb6f5f15f1882`，代码包SHA256 `64571b26fcb0745f0bc3868ac98484659dcf83a141e72a38aba076958339617e`。仅3份新代码/配置部署到个人Ibex`FMT_Task4C_SupervisedContrastive_20260914`，不含待授权预测/日志/报告，不公开push。UTC21:53:23五阶段全部提交，seed96211搜索6次，零对照+选定正权重以96212/96213复核8次。完整27,000训练+3,000验证复制校验通过，test_encoded=false；UTC21:54:35确认首批4项已运行，尚无完整科学结果。

部署命令在全部五阶段成功提交后，由PowerShell管道追加的末尾CR引发远端bash额外空行报错，调用返回非零。未重新部署/提交；只读核实源码/配置和submissions.jsonl中五个唯一作业链，调度器证明encode已完成、首批search运行。该命令封装错误不等于训练失败，保留`tmp/task4c410_deployment.log`及实际作业登记。

4.10首轮seed96211已完成5/6次：FMT权重0/0.1/0.5验证F1为0.382848/0.389321/0.376083，Conv权重0/0.1为0.407494/0.410920，Conv0.5仍运行。冻结read_run在原Ibex目录校验保存预测、验证阈值和指标一致性通过，额外核对每epoch27,000视图、4,460训练中心、零权重无辅助损失及推断/投影参数数。尚未进行完整独立最终复算，不能把一致性检查说成最终审计，也不能以该单种子小幅差值宣称收益或达标。原测试未读取，全部预注册三种子复核保持。

### Ablation_Task4C_RepresentationResolution_4.7：全部完成

科学commit8d483808，UTC2026-09-13 22:02:48选择完成，16次训练及20个调度任务均COMPLETED、exit0。FMT结果与前面部分三种子记录一致：6/17频率验证F1为0.365484±0.035753 / 0.349484±0.033497；平均精确率0.272968/0.274296。因此全频率F1降低，平均精确率未同向降低，不应概括所有指标都变差。

Conv24³/48³三种子验证F1为0.427184±0.014001 / 0.428600±0.007650；平均精确率0.348239/0.347186。48³逐种子95911–95913为[0.433684,0.432314,0.419802]，与24³的配对差两正一负，均值+0.001416，幅度小于这里的种子波动且平均精确率略降。24³选lr0.001，48³选lr0.0003，故不是只改变分辨率的严格单因素效应。选择器选FMT6频率、Conv48³，但当前证据不足以支持继续扩大分辨率搜索或声称有明显泛化改善；4.10已冻结的24³对照不因此改动。

在Ibex原目录独立NumPy复算所有合并/分流场/分头区/分尺度指标、验证阈值和epoch，核对原4.1验证metadata逐文件哈希与全部预测身份、当前源码/配置/预测哈希、学习率排名以及完整三种子均值/标准差和最终选择，全部通过，最大误差1.67e−16。报告`outputs/Ablation_Task4C_RepresentationResolution_4.7/independent_development_audit.json`；此前14份结果的独立证据通过原文件哈希延续，新增2份完整复算。无test读取，原4.6目标状态仍未达成。

4.10首轮6/6已完成：最后Conv权重0.5验证F1为0.411245。按预注册规则，FMT选择0.1、Conv选择0.5，均与零权重对照进入另两个种子复核；这些权重由完整首轮验证选择，不能改用pilot选择。当前9份完成预测的全部分组指标、阈值/epoch、原4.1验证metadata身份、源码/配置/预测哈希及首轮排名独立复算通过，最大误差1.67e−16；报告`outputs/Ablation_Task4C_SupervisedContrastive_4.10/independent_development_audit.json`留在Ibex。尚无完整三种子选择结论。

### Ablation_Task4C_SupervisedContrastive_4.10：完成，FMT小幅改善、Conv未提高F1

科学commit6bfc357b，UTC2026-09-13 22:11:13选择完成。FMT零权重/0.1权重三种子验证F1为0.384054±0.008555 / 0.394229±0.004253，平均精确率0.307715/0.321668。种子96211–96213对照为[0.382848,0.393148,0.376167]，0.1为[0.389321,0.396841,0.396524]，三次配对均提高，平均+0.010175。这里支持该有限对照下的小幅改善，不把三个种子当作独立物理数据证据，也不声称达到0.6或证明大幅泛化收益。

Conv3D零权重/0.5权重三种子验证F1为0.422313±0.013825 / 0.421731±0.009352，平均精确率0.358736/0.362657。种子对照[0.407494,0.424581,0.434864]，0.5为[0.411245,0.424739,0.429207]；配对差两正一负，平均−0.000583。此前首种子F1轻微提高→完整三种子没有平均提高；原因是最后种子的负差抵消前两次增益，之前只记录单种子而未宣称稳定收益。平均精确率略升，不应说所有指标都变差。按预注册F1优先规则，最终选择FMT0.1、Conv0。
...
### Ablation_Task4C_GeneralizedCrossEntropy_4.11：补齐最终结果

科学commit97e3f7ea，选择实际于UTC2026-09-13 22:51:04完成；2026-09-14在4.13训练等待期间补齐独立审计。14次训练及17个Slurm任务全部COMPLETED、exit0。FMT的q=0/0.3三种子验证F1为0.392898±0.006960 / 0.398741±0.013471，平均精确率0.308946/0.329412；Conv3D的q=0/0.7为0.428428±0.012428 / 0.432079±0.012808，平均精确率0.362565/0.350359。种子96311–96313，FMT对照[0.385217,0.398785,0.394693]、q0.3[0.406000,0.407025,0.383197]；Conv对照[0.421053,0.442777,0.421456]、q0.7[0.436455,0.442125,0.417657]。

按冻结F1优先规则选FMT q0.3、Conv q0.7。平均F1分别增加0.005842/0.003651，但配对种子并非均提高，Conv平均精确率还下降；只支持本次有限比较的轻微均值变化，不支持稳定、大幅泛化改善或原空间测试达到0.6。没有新测试读取，4.6历史结果保持。

独立NumPy复算14份验证预测的全部分组指标、验证阈值与epoch、原4.1验证身份、配置/源码/预测哈希、首轮排名和三种子选择均通过，最大误差1.67e-16。无模型文件。报告`outputs/Ablation_Task4C_GeneralizedCrossEntropy_4.11/independent_development_audit.json`留在个人Ibex；selector SHA256 `6159c985130c3235bc925da6c84a6ed742b9ada0652dd6e046b0fd6a67c386ff`。该补记不增加实验，不改变当前4.13设计。
...
4.12首个完整结果：原FMT（88,514参数）55轮完成，连续三轮训练F1≥0.99；按训练集选择的最佳第53轮F1=0.997007，错误26/27,000，Channel/TBL分别0.996829/0.997344，耗时98.52秒。此前进度报告第55轮0.994705、46错是最后一轮；最终保存第53轮0.997007、26错，二者是不同epoch，非指标改写。独立NumPy复算全部分组训练指标、阈值、原样本身份、55轮全量无放回排列及源码/预测哈希均通过，最大误差1.11e-16。报告留在Ibex`outputs/Verify_Task4C_TrainingMemorization_4.12/independent_training_fit_audit.json`。此时Conv仍在训练，不声称两个方法均已达到目标。

4.12独立输入核查通过：Channel/TBL训练正类2829/1502；原元数据、编码与冻结源哈希一致。忽略无效填充及线顺序后的FMT有效输入、Conv体素各27,000个精确不同输入，没有相同输入对应冲突标签；不推断近似几何是否可区分。报告`independent_training_input_audit.json`留在Ibex，未访问验证/测试。


### Verify_Task4C_TrainingMemorization_4.12：两个原结构均完成全量拟合

科学commit26f52805，原FMT与原Conv3D在全部27,000训练样本、阈值0.5下均连续三轮F1≥0.99。原FMT最佳第53轮F1=0.997007，26错，55轮结束，98.52秒；原Conv第32轮F1=0.995148，42错，32轮结束，330.20秒。Channel/TBL分别FMT0.996829/0.997344、Conv0.995400/0.994674。两个原网络参数数仍88,514/219,602，无需增容即满足用户本次训练拟合要求。

初始计划四组结构；两个原网络独立复算通过后，决定停止不再需要的容量对照。取消前状态保护第一次发现增容FMT已启动，第二次发现其已经完成；这两次保护均在scancel之前退出，没有取消或重启任务。增容FMT实际26轮、最佳24轮F1=0.994675、46错、42.55秒，保留结果但不同设备/结构不能简单归因比较速度。UTC22:56:40实际取消已启动的增容Conv51867308_3和待运行四组merge51867309；调度器确认CANCELLED。不能声称四组比较全部完成，也不修改冻结config以隐藏取消。三个已完成训练及encoder均COMPLETED、exit0。

最终独立NumPy从全部保存概率复算合并/分流场/分头区/分尺度指标及训练最佳阈值，核对原4.1训练metadata、源码/配置/预测哈希、全部113轮无放回全覆盖排列（55+32+26）、选定epoch、三轮通过条件，均通过，最大误差2.22e-16。仅读取训练；无模型文件。原结构确认报告`original_models_confirmation.json` SHA256 `0ee71f0f462c5c37e5492451cffe7b152356a1f8fc6bc0ff6dde2458b72bc419`；完整审计`independent_training_fit_audit.json`、调度终态`scheduler_final_status.json`与预测均保留在个人Ibex原4.12输出目录。本地只记录获准的聚合结果和运行元数据。

支持：当前输入表示、标签对应、两个原网络及本版优化/评估链路具备全量训练拟合能力。关闭正则化、改为全量无放回采样、按训练损失调学习率并按训练选择模型是联合变化，不能把成功单独归因于dropout或宣布所有历史训练器/物理标签都无错误。用户本次训练F1接近1的要求已满足；4.6测试结果没有重测或改写。


### mainExp_Task4C_NearbySampling_4.13：用户指定训练点附近的新测试

用户要求先解释三维卷积的体素化，再构造中心不同但接近训练集的新测试，加入dropout等正则化比较。已在运行前说明冻结4.3体素化：归一化线段4等分、5采样位置、8邻点三线性分配，4×24³占据/单位切向xyz，3层3³卷积16/32/64及512→256→64→2分类，219,602参数；低分辨率及同格方向平均会丢失细节，没有改基线。

新版本原27,000训练束不变；Channel/TBL各均匀无放回选1,500验证锚点、5,000测试锚点，新中心偏移原邻居距离5%–15%，保持尺度参数并重新沿真实涡量RK45积分/清洗。所有新中心与原中心不重合，验证/测试新中心也不重复；记录锚点及距离。新样本刻意共享原训练头区、GT实例和源网格支持，不是原空间留出测试；旧4.6测试不读取、不改写。详见`docs/Task4C_nearby_sampling_protocol_4.13.md`。

原FMT与Conv3D各比较dropout/weight decay为0/0及0.15/0.0001，两组三种子96511–96513共12次。每epoch全训练无放回一次，AdamW0.001、普通交叉熵；模型epoch仅用邻近验证0.5阈值F1选择，保存选择锁后一次读取新测试，报告全部预声明组，不用测试挑赢家。

工程核验通过：确定性/不重复锚点、解析旋转场真实RK45和清洗、新中心距离/错误标签/跨集合重复拒绝、独立四通道体素化（最大误差1.76e-6）、12次真实小缓存训练及选择锁后唯一测试读取、三集合F1独立复算与原参数数。证据`outputs/Verify_Task4C_NearbyCode_4.13/checks.json`。该fixture测试是旧pilot验证副本，不计科研成绩；真实流場小预检在Ibex作为完整生成前置，不能先称其已通过。

科学commit `52ae26a4117a39d713e20ad972d516e3c5a7f014`，config SHA256 `9b7b11e0c76344ce18bb9e89794240bb688936b14cd093cc3efe8fcc3f576b18`，代码包SHA256 `bbafdaa0b3ccfd7938b44d02d0504d2340b4cf8baaa7536eacf244b1cfcc8492`。仅三份代码/配置部署个人Ibex `FMT_Task4C_NearbySampling_20260914`。UTC03:36:11–12提交preflight51869122[0-1]、prepare51869123[0-1]、encode51869124、train51869125[0-11]、merge51869126，全部登记。尚无正式邻近测试成绩。

4.13真实流场preflight已通过：Channel/TBL各8验证+8测试新线簇，原生产追踪与新中心/标签身份检查均成功。UTC03:37:12完整两流场生成作业仍RUNNING；各1,500邻近验证样本已生成，测试继续生成。没有用工程pilot结果替代科学成绩。

4.13完整新采样已完成：Channel/TBL验证各1500（正328/170），测试各5000（正1080/562）。独立NumPy/SciPy从实际落盘坐标重新核对锚点排列、标签/实例/尺度、最近训练中心距离及全部清洗后几何，均通过；偏移/d范围0.05001–0.149995、中位数约0.1，新中心与原中心以及新验证/测试之间精确重合数均0，13,000束归一化几何均不同于对应原训练锚点。最大质心/半径误差约6.11e-8/5.97e-8，报告`independent_nearby_data_audit.json`保留在Ibex。共享训练头区和源支持是本版定义，不宣称空间留出独立性。

4.13实际网络输入精确重复核查完成（UTC04:07:56）：FMT按真实训练标准化和截断后，忽略无效填充与线顺序，比较有效线特征集合；Conv比较实际四通道体素。两种表示各27,000训练输入精确不同，新3,000验证及10,000测试各自精确不同、与任何训练输入精确重复数0、验证/测试交叉精确重复数0。报告`independent_nearby_input_overlap_audit.json`留在Ibex。这仅排除精确重复，不排除相似输入或共享物理结构；没有据此改变训练、选择或测试规则。

### mainExp_Task4C_NearbySampling_4.13：完整比较完成

科学commit52ae26a4，UTC2026-09-14 04:11:57完成，18/18Slurm任务COMPLETED、exit0，12/12训练均完成。实际GPU均Tesla V100-SXM2-32GB。原申请数组并发4，UTC03:50:37确认已调整为8；scontrol返回1是因为数组成员2/6已完成，实际并发已生效，经只读确认后没有重试或重启训练。运行数量、单任务资源、数据和科学配置不变。

以下是阈值0.5的Hairpin F1。正则化为dropout0.15、weight decay0.0001联合变化；种子96511–96513，验证选epoch后一次测试。训练列为均值，验证/测试为均值±样本标准差。

| 方法 | 正则化 | 训练F1 | 邻近验证F1 | 邻近测试F1 |
|---|---|---:|---:|---:|
| FMT | 无 | 0.999462 | 0.946796 ± 0.008165 | 0.943793 ± 0.001428 |
| FMT | 有 | 1.000000 | 0.994615 ± 0.003098 | 0.989491 ± 0.004233 |
| Conv3D | 无 | 1.000000 | 0.985588 ± 0.002286 | 0.984681 ± 0.001704 |
| Conv3D | 有 | 1.000000 | 0.995645 ± 0.002098 | 0.990186 ± 0.005765 |

FMT无正则化逐种子测试[0.944238,0.944946,0.942195]，正则化[0.992647,0.984681,0.991145]；配对差[0.048409,0.039736,0.048950]，均值+0.045698。三个种子均提高，支持此邻近采样设置下联合正则化改善FMT局部泛化。Conv无正则化[0.986209,0.984992,0.982843]，正则化[0.995425,0.984010,0.991123]；配对差[0.009216,-0.000983,0.008280]，均值+0.005505，两次提高一次下降，不能称每个种子均获益。两种正则项同时变化，不能把改善单独归因于dropout。

正则化后Channel/TBL测试F1均值：FMT0.995813/0.977300，Conv0.994570/0.981708；合并测试Conv比FMT高0.000695。两者表现接近，这一差值小于本次各自种子标准差；没有额外显著性检验，不宣称FMT优于Conv，也不将三个优化种子当作三个独立物理流场。原网络参数仍88,514/219,602，结构不同，本比较不是同网络插入FMT的单因素消融。

高邻近测试F1支持两个模型在已知训练结构附近新积分线簇上的局部分类能力，不证明未见物理涡或独立空间泛化。此前4.6原空间留出测试FMT0.335526±0.017885、Conv0.304615±0.031948与本表的测试定义不同，训练/模型选择设置也不同；不将两版分数变化单独归因于新采样或正则化，不改写旧结果。全部预声明组与种子均报告，未根据新测试挑选正则强度、网络或种子。

全部新坐标、标签/锚点/尺度、清洗后几何，以及网络输入精确重复检查通过。12份保存预测的训练/验证/邻近测试合并、分流场、分头区、分尺度指标，固定阈值及验证阈值、epoch选择、每epoch全训练无放回排列、原训练身份和源码/配置/预测哈希均独立核对通过，最大数值误差4.44e-16；四组三种子均值/样本标准差和最终summary复算通过。无权重文件，原空间test未读取。报告`independent_nearby_data_audit.json`、`independent_nearby_input_overlap_audit.json`、`independent_predictions_audit.json`、`scheduler_final_status.json`保留在Ibex原4.13输出；summary SHA256 `c36bbdda028904073c7294ab264fc7cbb195dc7e404c44e857420ea43002bdef`。调度器sacct时间为UTC+03:00，runtime事件及完成时间为UTC；本地只保留获准汇总与运行元数据，完整分流场/逐种子表见`docs/paper_tables_tasks_3d.md`。

### mainExp_Task4C_PhysicalLength_4.14：用户指定物理积分长度与中等距离采样

用户从可视化指出单向积分长度对形状判断至关重要，指定Channel ds0.002×50=0.1、TBL0.05×100=5，并要求重建全部训练/测试、评价点比4.13更远且不再采用原整段空间留出。旧4.1/4.13按完整壁面高度缩放积分参数，没有固定到上述物理长度；本版改成直接物理单位。这是可核实的设计差异，但没有控制实验把旧低F1唯一归因于该原因；4.6、4.13历史指标保持。协议`docs/Task4C_physical_length_protocol_4.14.md`。

固定每流场三档ds/步数：Channel(0.001,80)/(0.002,50)/(0.004,30)，TBL(0.025,160)/(0.05,100)/(0.04,150)，单向长度分别0.08/0.1/0.12与4/5/6。乘三档邻距0.25/0.5/1.0h，共9种配置在训练/验证/测试相同；所有几何新积分。RK45初始/最小/最大步均ds，每方向只保留最多N段，要求实际半弧长达到目标95%且不超过100.2%；保留原lambda2/oyf种子与完整头区GT二分类规则。完整源场积分，不在集合边界截断线。

每候选头区沿单元中心第一主轴分5个小块，固定随机分配3训练/1验证/1测试；原生中心单元角色先固定。评价点距任意实际训练中心至少1h、距同头区训练中心最多4h，test另距validation至少0.5h；h为所在单元间距的几何平均。合计27,000/3,000/10,000，中心距离无法满足则失败，不据F1放宽。共享头区/实例/源场，不声称未见涡实例泛化。旧测试地域可能在本版完整快照中再次采样，属于用户指定的探索性新评测。

工程解析螺旋线验证最初按VTK文档扣一步，发现部分组合少保留一步；改为N仍发现精确传播上限处提前终止。最终内部界限增加0.001ds、落盘严格截最多N段，六组解析端点最大误差3.35e-8、步数均正确。此前关于“VTK多一步”的表述只来自接口文档；实际端点还受传播终止影响，最终以解析轨迹实测为准。基准半弧长Channel0.099999987、TBL4.999518，符合目标。证据`outputs/Verify_Task4C_PhysicalLengthCode_4.14/integration_checks.json`，不修改冻结旧积分器。

两个真实流场各90训练/18验证/36测试预检完成，全9尺度与长度/距离检查通过。测试最近训练距离/h中位数Channel2.396456、TBL2.085667，范围分别[1.128457,3.714258]/[1.014209,3.981899]；这些只是小预检分布，不代表全量最终分布。实际新几何编码后六次两epoch工程训练、选择锁后唯一测试读取、参数数及三集合F1独立复算通过，无权重文件。工程结果不计科研F1，实际覆盖配置另存`fixture_applied_spec.json`，核验结果`training_checks.json`；没有留下临时test/verify源码。

科学commit `5c9c904228d5185447a084953b2652a8938c0485`，配置SHA256 `3909eaab4093c0cb85f9bbd400fcf81e9dc036c879df7b8b9f6f214b6a281e6d`，代码包SHA256 `2e3e56200a0617e659aa0a5c55e5afa402036e2547e52bc19c0a311cf9593daa`。仅三份新增代码/配置发至个人Ibex。首次本地打包被自动审批服务以“模型容量不足”拒绝，未执行；只读核对范围及无副作用后同操作重试成功。没有绕过审批、公开push或原始预测/报告传输。正式作业正在部署提交，尚无全量科学F1。

4.14已于UTC2026-09-14 08:49:08提交个人Ibex `FMT_Task4C_PhysicalLength_20260914`：preflight51877188[0-1]、prepare51877189[0-1]、encode51877190、train51877191[0-5]、merge51877192，共12个调度任务，全部已登记。当前未取得正式F1。

4.14全量生成完成，Channel/TBL各13,500/1,500/5,000，分别于UTC08:54:10/08:56:07结束；编码完成后六次训练在09:00:20起全部运行。独立原位置审计重新读取源场和GT，复算lambda2/oyf头区、完整头区GT多数标签、原生单元角色、实际中心与网格尺度、所有几何清洗/归一化/弧长及跨集合距离，全部通过。初次独立检查的单精度向量幂导致网格尺度复算误差，改用双精度数学计算后最大相对误差3.38e-8，原2e-7阈值通过；没有修改科学数据、训练或放宽标准。完整报告`independent_physical_data_audit.json`留在Ibex。

训练正类Channel2682/TBL1937，验证267/182，测试1075/692。训练实际不同中心2847/3088，测试1104/1030；多尺度样本不能当作独立物理实例。测试最近训练距离/h分别[min,median,max]=[1.002166,1.986419,3.971310]/[1.000679,1.990095,3.999088]，实际半弧长/目标最小值0.950075/0.950021、中位数0.999880/0.999593；长度与距离条件均按预注册执行。所有归一化线簇在各自集合内精确不同，尚未以此声称跨集合输入完全不同。源数据和各split身份哈希、完整数值核验见原报告。09:00:36状态查询已恢复，此前一次只读查询因审批模型容量不足被拒绝，未影响已提交作业。

4.14进一步完成实际网络输入检查（UTC09:06:24）：FMT使用真实训练标准化与截断，忽略无效填充及线顺序；Conv使用实际四通道24³体素。两方法各27,000训练、3,000验证、10,000测试输入在各自集合内精确不同，训练/验证、训练/测试、验证/测试间精确重复数全部为0，没有相同训练输入对应冲突标签。报告`independent_input_overlap_audit.json`留在Ibex。该检查不排除近似几何相似性，也不改变共享头区和源场的评价范围。

全量新测试到最近训练中心的实际物理距离[min,median,max]：Channel[0.00371632,0.01152531,0.02743932]，TBL[0.07164269,0.21455957,0.55330800]；对应局部h中位数0.00562774/0.10520229。该距离与4.13直接偏移原邻距5%–15%的定义不同，不能把h和邻距d混为一个单位。

### mainExp_Task4C_PhysicalLength_4.14：完整结果与证据边界
...
本次合并测试FMT0.673624±0.009154、Conv3D0.811226±0.013250，六个单次测试均超过0.6；Conv均值高0.137602，且三个对应种子及两个流场均较高。本实验不支持FMT优于该Conv基线。FMT训练均值0.999314、Conv1.000000，不能将新测试差距解释为两模型无法拟合训练集。FMT的TBL测试均值0.591833仍低于0.6，因此不能声称每个方法在每个流场均达标。模型结构和参数数不同，不属于同结构插入FMT的单因素消融。

此次更改了训练数据、物理积分长度、空间块划分、尺度覆盖及模型选择所用验证集，不能从4.6/4.13/4.14分数变化唯一确定某项修改的因果贡献。旧4.6独立头区/空间段及未见尺度测试FMT0.335526/Conv0.304615、4.13微小邻近测试正则化FMT0.989491/Conv0.990186均保持原定义和结果；本版回答已知候选头区内部、不同中心单元且有距离间隔的新线簇分类能力，不是未见涡实例或独立流场泛化。全部三种子及唯一预声明正则化配置均报告，没有按测试成绩再改距离、长度、模型或超参数。
...
### 夹角1.1 已部署（2026-09-14 14:37 +03）

科学commit `5fabbe4d031a3311bd9d43dfc4e5d4630183dc9d`，配置SHA256 `6d09f3fbca732ff5d399d625643046f172983384b7a0bda6e26e306b937b21b0`，373文件源清单SHA256 `7015ae32dcc59a7d1ad7f1cb73c678c95d0caa458da77158dbff4b697e69317b`。Ibex路径`/ibex/user/zhanx0o/FMT_AngleFeatures_20260914`。单元/流程检查51882185和40组合数据预检51882186完成PASS；正式Task1数组51882494、Task2数组51882496、Task3→Task5数组51882498及总审计51882499已提交。检查不构成性能结果；当前尚未汇总真实性能。

### 夹角1.1 Task1完成并独立复算（其余任务进行中）

同一科学commit5fabbe4d、job51882494，30分片150行、原始源标签及保存预测逐项核对PASS。三种子10条目平均F1：Raw0.423541±0.000117，原FMT0.595787±0.000252，fmt_v5为0.599186±0.000047；距离版0.305793±0.003446，距离+夹角0.322713±0.000199。fmt_v5相对原FMT增加0.003399，距离+夹角相对距离版增加0.016920；在当前固定Task1管线中均为小幅提高，距离+夹角仍低于Raw及原FMT。标准差是三个种子宏平均的样本标准差。证据为远端`Task1_progress_audit.json`及shards/Task1，Task2/3/5不得据此修改。

### 夹角1.1 Task2完成并独立复算（Task3/5进行中）

同一科学commit5fabbe4d、job51882496，30分片150行、原始源标签及保存预测逐项核对PASS。三种子10条目平均F1：Raw0.482806±0.010384，原FMT0.560551±0.004939，fmt_v5为0.517190±0.022260；距离版0.233557±0.013586，距离+夹角0.362336±0.011118。新FMT相对原FMT降低0.043361；客观距离扩展相对父版提高0.128779，但仍低于Raw及原FMT。与Task1的小幅提高相比，这表明当前显式夹角分支的效果取决于任务和基础表示，不能概括为统一改进；尚未做机制消融，不归因于特定训练或特征机制。证据为远端`Task2_progress_audit.json`及shards/Task2。Task3/5配置保持冻结。


<a id="task1235-angles-2026-09-14"></a>

## 2026-09-14 — Verify_Task1235_AngleFeatures_1.1 全部完成
...
| Task | 比较 | 平均F1差值±种子标准差 | 提高的流场数 |
|---|---|---:|---:|
| Task1 | fmt_v5+kin4 − fmt_all+kin4 | +0.003399±0.000256 | 5/10 |
| Task1 | 距离+夹角−距离 | +0.016920±0.003618 | 6/10 |
| Task2 | fmt_v5+kin4 − fmt_all+kin4 | -0.043361±0.025972 | 3/10 |
| Task2 | 距离+夹角−距离 | +0.128779±0.012080 | 8/10 |
| Task3 | fmt_v5+kin4 − fmt_all+kin4 | -0.014613±0.005538 | 1/10 |
| Task3 | 距离+夹角−距离 | +0.011787±0.004321 | 8/10 |
| Task5 | fmt_v5+gram2+kin6 − fmt_all+gram2+kin6 | -0.008440±0.002861 | 3/10 |
| Task5 | 距离+夹角−距离 | +0.007128±0.003777 | 9/10 |

Task3当前配方aivd1w3_dft在本批共同容量下重跑F1为0.868073±0.004278；Task5固定Task3 Raw迁移F1为0.397254±0.023236。其原主表保持不变。

保留kin4/kin6等附加块时，fmt_v5相对同样附加块的fmt_all：平均F1提高的任务为Task1，降低的任务为Task2、Task3、Task5。本批没有测量独立fmt_v5与独立fmt_all的性能差异。
距离+夹角版相对各自父版本：平均F1提高的任务为Task1、Task2、Task3、Task5，降低的任务为无。

以上判断只针对本版15条中心夹角的六频率表示与既定训练流程，不说明全部客观特征有效或无效，也不能把当前差异归因于未检验的单独机制。含运动学估计的FMT扩展没有在四任务上一致提高，Task2的下降保留。Task3/5含Raw输入，不能由编码器客观性推断整个网络客观。未根据本次测试改变数据、频率、模型、阈值选择规则或训练预算。
...
<a id="task1235-angles-baseline-correction-2026-09-14"></a>

### 2026-09-14 — 基线身份与结论更正

用户质疑“Task2/3/5均低于原FMT”是否实际依赖估算IVD的特殊特征。核对科学commit `5fabbe4d` 的配置、特征分派和运动学实现后，确认此前简称掩盖了关键输入，不能把该比较称为纯原始FMT比较。此前Task1/2阶段记录中的“原FMT”和“fmt_v5”也按下表解释；历史数值不变。

| 任务 | 输出文件中的old_fmt | 输出文件中的fmt_v5 |
|---|---|---|
| Task1/2/3 | fmt_all+kin4，189维 | fmt_v5+kin4，354维 |
| Task5 | fmt_all+gram2+kin6，268维 | fmt_v5+gram2+kin6，433维 |

`kin4/kin6`是运动学标量的4/6频率傅里叶特征：`FMT_Utils/Task12Data_3D.py::_extended_feature`调用`DFT_FMT_3D.py::pathline_velocity_gradient_dft_features_3d`，由`pathline_velocity_gradient_scalar_sequences_3d`估算速度梯度`L = D_dot D^+`，从其反对称分量得到涡量，再减去同一时刻输入样本的涡量均值并取范数，形成`ivd_like`。同时包含应变、散度和Q-like标量。此处是对速度取curl，不是对IVD取curl。它没有直接读取真实IVD标签，但明确构造了接近标签定义的物理估计量，不能将含这些输入的性能直接归因于纯FMT几何编码。`gram2`是同时间相对向量内积的傅里叶特征，不属于上述速度梯度估计。

`aivd1w3_dft`作为单独的Task3参照`current_task3`运行，F1为0.868073；表中old_fmt的Task3 F1为0.765756。因此没有将该aivd参照的数字直接替换成old_fmt，但kin分支与aivd分支调用同一个运动学标量估计函数，不能仅凭名称不同排除用户所指的问题。

| 之前的表述 | 更正后的结论 | 更正原因 |
|---|---|---|
| fmt_v5在Task2/3/5低于原FMT | 含相同运动学估计附加块时，加入夹角后的平均F1下降；纯fmt_v5与纯fmt_all的差异未测量 | 将fmt_all+kin（Task5另含gram2）简称为原FMT，也将fmt_v5+kin简称为fmt_v5，混淆了独立编码器与完整实验输入 |
| 距离+夹角四项改善，但仍低于原FMT | 相对纯距离父版的四项改善保持；与表中FMT的差距是对含运动学估计方案的差距，不能表述为低于纯原始FMT | 两个距离方案没有kin分支，而被比较的FMT方案包含该分支 |

本次更正不改变代码、冻结配置、预测或指标，不重写旧实验。纯几何比较仍需新实验版本；现有审计通过仅证明运行与配置一致，不能补足该实验问题的缺失。

## 2026-09-14 — Verify_Task4C_Encoders_4.15 预注册

用户要求在Task4-c 4.14上比较两个新编码器与Conv3D，并在提交前明确选定距离+夹角2.2。原4.14的27,000训练、3,000验证、10,000测试样本及训练规则冻结，三组各96611/96612/96613。实际输入是纯fmt_v5 5.1的326维、纯fmt_objective_ntod_v2 2.2的396维，以及原Conv3D体素；没有kin/IVD估计或旧72维方向频谱旁路。新协议`docs/Task4C_encoders_protocol_4.15.md`，尚无科学性能结论。原4.14 FMT含原161维+72维方向特征，仅作历史参照，不混称纯原FMT。

4.15工程预检51886507与全量编码51886623通过，正式科学commit785f96ce3ba4d9736ccb066420852ee84fa88df3、配置SHA256 df8f592250e11ff34331788b0df17a67e7631f704b781d30563ceb07ac3046bf。原161维前缀精确一致、三组完整训练接口通过，全部40,000样本未过滤且父数据哈希一致。三个分类器参数数100,418/109,378/219,602。正式九次训练51886656[0-8]、汇总51886680、独立复算51886720已登记。首次预检51886086的体素逐位重算检查失败保留；修复仅改变工程数值检查，实际Conv输入继续使用原缓存。尚无正式性能结论。

<a id="task4c-encoders-4-15-2026-09-14"></a>

## 2026-09-14 — Verify_Task4C_Encoders_4.15 完成

**用户质疑后的更正（2026-09-14）**：此前将此运行作为“将新编码器应用于原4.14”的答复 → 现在明确为偏离用户要求的比较，不足以判断向原4.14加入角度的效果。原因：擅自去掉了原4.14的72维坐标/切向方向频谱；该分支是几何特征，不是用户反对的IVD估计。错误在于把禁止IVD取巧输入扩大成移除已有几何输入。原始0.320565等指标计算未变，但不能归因于角度；目前也不能断言全部下降都是移除72维造成。另立4.16保留完整原输入重新运行，下表和4.15源码不改写。
...
| 方法 | 种子 | 验证选定epoch | 测试F1 |
|---|---:|---:|---:|
| fmt_v5 | 96611 | 15 | 0.310041 |
| fmt_v5 | 96612 | 5 | 0.331181 |
| fmt_v5 | 96613 | 7 | 0.320473 |
| fmt_objective_ntod_v2 | 96611 | 9 | 0.207247 |
| fmt_objective_ntod_v2 | 96612 | 14 | 0.232615 |
| fmt_objective_ntod_v2 | 96613 | 23 | 0.168480 |
| conv3d | 96611 | 118 | 0.824630 |
| conv3d | 96612 | 148 | 0.798137 |
| conv3d | 96613 | 149 | 0.810910 |

本次固定接入方式和4.14训练配置下，两个新编码器均未优于Conv3D；fmt_v5测试F1比Conv低0.490661，距离+夹角比Conv低0.608445。验证F1和测试Average Precision也更低，不能将当前差距仅解释成固定0.5阈值的选择。该结果不证明其他网络、训练预算或全部客观特征必然无效；没有根据本次测试追加调参或筛选种子。

实际输入严格为各独立编码器输出加有效线mask，没有kin4/kin6、速度梯度/curl/IVD估计，也没有额外72维坐标/切向方向频谱。4.14历史FMT输入为原161维+72维方向特征，测试F1为0.673624±0.009154，Channel/TBL为0.722775/0.591833；本次既增加夹角又去掉原方向分支，因此不能把它与历史FMT的差距单独归因于夹角。这不是纯原161维FMT与fmt_v5的单因素消融。Conv3D三种子复跑的训练/验证/测试F1与4.14各自对应种子完全一致。

本次输入是静态涡线的弧长采样，局部七线按原近邻规则构造；结论限于该接入方式。数据共享头区、实例与源快照，是已使用benchmark，不宣称独立涡实例泛化。只保持与4.14相同训练规则，不宣称已经为新编码器找到各自最优网络或超参数。
...
<a id="task4c-retain-direction-4-16-2026-09-14"></a>

## 2026-09-14 — Task4-c 4.16完整输入纠正及4.17原设备复核完成
...
此前答复使用移除72维的4.15成绩0.320565来回应“向原4.14加入角度” → 修正后完整保留原输入，fmt_v5成绩为0.556003±0.005039 → 原因是此前擅自删除合法几何方向信息，混淆了该分支与IVD估计。错误设置的原成绩不删除、不改值，不能作为角度单独效果的证据，也不把4.15到4.16的数值变化全部归因于72维单一因素（输入宽度及其首层初始化同时变化）。

在本次固定网络与训练规则下，追加165维角度后，fmt_v5相对原4.14测试F1下降0.117621，验证也下降；距离＋夹角接入版相对原FMT下降0.157084。两者均低于Conv3D。新方法训练F1接近1，证据显示当前接入没有改善这一benchmark的泛化；不证明所有角度特征或所有客观标量无效，未搜索各自最优网络/超参数。没有依据本次测试追加调参。

环境差异另列：4.16原FMT seed96611被分配A100，归一化均值/标准差最大差异7.3217e-9/2.9369e-9，测试0.683322518而原V100为0.665374677，混合设备三种子均值0.679606614不得替换冻结基线。signed-log在CPU由NumPy执行，节点变化同时涉及CPU和GPU，不能断言差异只来自GPU；4.17在原V100环境复现后，冻结参照仍为0.673624±0.009154。复核选择由设备/基线不一致触发，并非按分数择优。
...
### Ablation_Task4C_ConvCapacity_4.18：用户指定Conv参数预算

2026-09-14用户要求`Ablation_Task4C_ConvCapacity_4.18`：新Conv3D 83,918参数，为原4.14 FMT 88,514参数的94.8076%；卷积通道12/24/48、分类头384→96→64→2。原4.14数据、体素缓存、训练器及三种子固定，限定V100，原FMT和219,602参数Conv结果保留作为参考。工程检查已通过，尚未产生新科学F1；协议`docs/Task4C_conv_capacity_protocol_4.18.md`。该实验与4.16/4.17编码器核验分开。

参数预算先于训练固定，90%–95%的参照是本轮对话所指4.14原FMT模型。新Conv83,918个参数，保持卷积/归一化/激活/池化顺序，仅缩小通道与分类头宽度；原方法源码不改。比较相同4.14数据和优化规则的三种子96611–96613，原FMT0.673624±0.009154及原Conv0.811226±0.013250测试结果作为冻结参考。训练时间记录同时包含数据加载、最后验证总耗时及第一轮之后每轮耗时，暂不预设性能或速度优势。

内联工程检查通过参数独立公式、同算子类型顺序、所有参数有限梯度、27组含单类与同分情况的独立指标、三个真实小夹具训练及选择锁后唯一test读取。首个夹具断言因局部正样本少于16失败，改为每流场每类8个已有工程训练样本；生产数据与配额未变。证据`outputs/Verify_Task4C_ConvCapacityCode_4.18/checks.json`，没有保留一次性验证源码。
...
固定参数预算的新Conv3D为原FMT参数量的94.8076%，满足用户90%–95%约束。新合并测试F1为0.7499±0.0126（用户独立核验修订），相对原大Conv均值变化约-0.061326；原FMT仍为0.673624±0.009154。原记录的Channel/TBL测试均值0.829256/0.726442为修订前明细，不代表用户修订后的分流场结果。本比较只针对这一预声明缩窄结构，不选择其他宽度或按测试换种子，也不替换原4.14模型。

新Conv平均训练至最后验证耗时18.432分钟，原大Conv24.248分钟，实际减少23.98%；第一轮后每轮耗时中位数的三种子均值6.551秒。总训练时间包含数据加载和各轮验证，排除几何生成、体素预计算及最终测试；受早停轮数及运行节点影响，不能将总时间差直接解释成每步计算速度。原与新Conv的卷积/GroupNorm参数71,184→40,428，分类头148,418→43,490；参数削减主要发生在分类头，因此不能预设高分辨率卷积开销同比下降。训练实际都为V100-SXM2-32GB/4 CPU线程，节点和运行起止时间见登记表。

这是已用过的4.14同头区空间块benchmark，所有输入缓存逐文件哈希与父版本相同，样本及标签没有重采样或重标。只改变模型宽度，训练规则和模型选择不变；固定0.5为主阈值，验证阈值仅为补充。

科学commit33db52c6088081ac5f41985ba8450d2efb564c85，最终汇总于2026-09-14T14:40:58.419222+00:00完成；prepare51888655、train51888656[0-2]、merge51888657共5项全部COMPLETED、exit0，10条生命周期事件完整。全部三份预测的合并/分流场/分头区/分尺度指标、训练/验证/测试身份、验证选择epoch/阈值、逐轮27,000行无放回覆盖、源码/配置/预测哈希及汇总均值/样本标准差独立核对通过，最大指标误差4.44e-16。无模型文件。
...
## 2026-09-14 — Task4-c 4.19 fmt_v5五块快速消融完成
...
按预先定义的完整减去块验证F1，排序为方向72维（+0.286129）＞中心23维（+0.018956）＞邻居138维（−0.001351）＞角度虚部75维（−0.043912）＞角度实部90维（−0.059333）。

最重要的是方向72维：删除后验证F1从0.587302降至0.301172，Channel/TBL均大幅下降；验证选定模型的训练F1也较低（0.822451）。当前删除后改善最多的是角度实部90维（到0.646635），其次为角度虚部75维（到0.631214），两流场均改善。若“最不重要”指删除后合并F1几乎不变，则是邻居138维（到0.588652）；但Channel从0.636190升至0.652000、TBL从0.515406降至0.497110，不能断言它在两流场都无用。

这些结果描述原学习率/正则化/初始化和单个种子下重新训练的表现，不排除其他学习率或特征交互，不证明角度标量本身没有信息。沿用验证选模型，未根据重要性排序新增测试或挑选更好的种子。
...
<a id="task4c-conv-resolution-4-22-2026-09-14"></a>
## Task4-c 4.22：小卷积体素分辨率比较（准备完成，待数据修订确认）

2026-09-14用户指定32³、36³、48³各三种子，保持4.18网络83,918参数及其训练设置，比较24³用户独立核验修订值0.7499±0.0126。协议`Task4C_conv_resolution_protocol_4.22.md`；入口`experiments/Task4C_ConvResolution_4_22.py`，配置`config/Ablation_Task4C_ConvResolution_4.22.json`。三个尺寸的CPU前向/全部参数梯度、独立NumPy体素重算、九组分辨率/种子枚举与运行阻止检查均PASS；体素重算最大误差不超过5.26e−7，证据`outputs/Verify_Task4C_ConvResolutionCode_4.22/checks.json`。只做工程检查，无新科研F1。

24³修订尚未说明是否改变样本、标签或评估代码，已询问用户。配置`data_revision_status=pending_user_clarification`明确阻止正式生成/提交，尚无Ibex作业。修订参考不回退到旧0.789915，不用旧三种子值推导新均值。待数据确认后使用一致的数据和评估；GPU完整batch128显存预检包含在每个准备任务内。48³缓存约32.96 GiB，训练加载临时副本约44.49 GiB，因此申请64 GiB主机内存，batch128保持。全部结果报告，不按测试选择分辨率。


<a id="fmt-v8-five-combinations-2026-09-14"></a>

## 2026-09-14 — FMT v8.1五组实验执行记录

2026-09-14正在执行FMT v8.1五组比较：用户确认邻居138维按绝对值取前4项保留符号＋全部138项带符号均值；五组为原频域161、加方向233、加角度326、方向+角度398、V8频域100。Task4-c版本mainExp_Task4C_FMTv8_4.21，科学commita25bcc0d，预检51892537通过，训练51892616[0–14]及汇总51892617；Task1/3/5版本mainExp_Task135_FMTv8_1.1，科学commit0ae17ea2，预检51892661和真实小样本51892662均通过，Task1 51892691[0–29]、Task3→5 51892692[0–29]、审计51892705。所有几何组无kin/IVD估计，参数量与训练成本并列记录。协议docs/FMT_v8_protocol_1.1.md。用户明确将小Conv3D测试F1修订为0.7499±0.0126，来源为其另代码核验且要求不再复查；旧值不作为当前比较值。

三部分原特征均经过DFT；中心23和邻居138为由复系数进一步计算的几何频域描述量，角度165保留复系数实虚部。Task1/3/5原161维缓存保持，72维方向分支新做每primitive质心/最大半径归一化；不把这五组与旧含kin配方结果混合。Task4-c五组三种子均重新训练。Task3/5五组补零至433辅助维、同总参数量及同可训练参数量；公共Raw骨干训练开销和残差分支开销分别报告。Task1只报告编码/聚类耗时及零神经编码器参数。方法结论等待完整结果，不据已完成test改选组合。


<a id="task4c-fmtv8-five-4-21-2026-09-14"></a>

## Task4-c 4.21：五种FMT表示三种子完成
...
V8三种子测试F1=0.728480±0.004107，较原233维FMT的0.673624高0.054856（5.486个百分点）；参数由88,514降至71,490，但本轮平均训练2.206分钟，长于原233维的1.744分钟。100维V8在此次五种FMT表示中最高；相对用户修订的小Conv仍低0.021420，而原小Conv训练时间18.432分钟显著长于本次V8，时间来自分别运行且早停轮数不同，不能直接解释为算子加速比。

相对4.20的95维V7（0.723036±0.012807），V8均值仅增加0.005444；两者均用72维方向和23维中心，不能将V8相对原233维的全部改善归因于top4+均值这5维。未进行独立新测试或显著性检验。161/326维无方向分支的结果较低，与4.19方向特征重要性的诊断一致；这只适用于该涡线分类协议，不能推广为所有任务都应压缩邻居特征。
...
<a id="task135-fmtv8-five-1-1-2026-09-14"></a>

## Task1/3/5：FMT v8.1五种组合完整结果
...
Task1：V8平均F1=0.399681；相对纯161维-0.194932，相对纯fmt_v5 326维-0.198891；五组最高均值为fmt_v5_326（0.598571）。

Task3：V8平均F1=0.666199；相对纯161维-0.056665，相对纯fmt_v5 326维-0.059807；五组最高均值为fmt_v5_direction_398（0.726144）。

Task5：V8平均F1=0.638731；相对纯161维-0.025700，相对纯fmt_v5 326维-0.029604；五组最高均值为fmt_v5_direction_398（0.680079）。

Task4-c的V8提高不能自动推广到物质轨线聚类或IVD分类；这是固定预处理、PCA和训练规则下的描述性结果，不是每种编码器各自优化后的最高性能，也不证明邻居压缩对所有任务有益。Task4-c完整五组结果见task4c-fmtv8-five-4-21-2026-09-14。
...
<a id="fmtv8-search-2-1-2026-09-15"></a>

## FMT v8.2池化搜索立项

2026-09-15用户授权新版本Ablation_FMTv8_Search_2.1：固定V8池化前中心23、方向72、邻居138，50种池化全流场验证筛选，再对前三种池化各测试三种训练配置，唯一共同组合与原V8以三个新种子在Task3/5及Task4-c 4.14配对测试。无角度或kin/IVD估计输入，沿用旧数据/标签/尺度，测试不参与选择。目标暂按三任务等权平均F1相对提升15%–20%，不预设达成。源码及协议docs/FMT_v8_search_protocol_2.1.md已完成，待Ibex工程预检；每个作业及成本必须登记。原v8.1结果保持。

预注册50种池化与h0原训练，验证选前三种后各追加h1正则化/h2激活与学习率/h3三分支投影，共59候选、1239次验证拟合。最终采用Task3/5新种子44–46和Task4-c新种子96621–96623，选定唯一共同配置对比原p00/h0；原始测试集已用，不能称新独立确认。配置config/Ablation_FMTv8_Search_2.1.json。全部科学结果和训练代价待完成后记录。
...
## FMT v8.2：2.1停止与确定性2.2修订

2026-09-15推进Ablation_FMTv8_Search_2.2：2.1因历史控制无法精确复现而停止，35次验证保留且未用于选择、无测试。诊断51897403证明同卡默认GPU运算重复权重差0.0111464，确定性模式差0；输入/标签/归一化一致。2.2固定CUDA确定性，原V8与全部候选共用；50配置对应48表示，前三种细化去重。先21对原V8重复概率一致，再搜索并以3个新种子配对测试。科学commitbc09a242，预检51897487已提交；协议docs/FMT_v8_search_protocol_2.2.md。目标仍为三任务等权F1相对提升15%–20%，旧指标不改写。

先前判断“应逐值复现历史原V8”→修订为“当前原V8在确定性条件下必须自身重复一致，与候选配对比较”：旧训练未固定GPU确定性，同卡同种子重复也会变动，不是池化输入变化。Task3前三个发现的旧/新验证F1为Channel0.328767/0.332103，Re1600.686981/0.683060，Re6400.718129/0.707403。偏差首先出现在Raw骨干第一轮；逐值核对Raw/标签/均值/标准差一致。独立诊断代码commit5c547064、job51897403，默认两次权重hash不同、最大差0.0111464，确定性两次hash均eea0e047b601af8c89e3c359bd4fde06b4940a9eb04ad69bd71b9547d8008743。该证据不否定旧已记录运行，只说明不能要求非确定性旧运行逐值复现。

2.1共258逻辑作业：23完成（含诊断）、1失败预检、234取消。35次科学验证已独立复算并保留；没有特征选择或测试，临时权重全部删除。395份证据包SHA2568478f7f1fb72623e87f60dc96b9ecb37906f184df02dc8bde1a335960e4b9e70，路径outputs/Ablation_FMTv8_Search_2.1。2.2预检前首次sbatch因Windows换行拒绝（无job ID），已修为Unix换行并保留submission_rejection.json。2.2源码归档c44adde7814adbf41759b9e64a7208ff3d74e04916533a9be850ffc5600e9955，配置a1bf07ef1e6bf2e24be5ad2d0699957e45fc6b981f8d381ae1816d9fc7b58bdf。


### 确定性2.2已全部部署

2026-09-15正在执行Ablation_FMTv8_Search_2.2：50配置（48不同表示）在Task3/5及Task4-c 4.14验证搜索，前三种不同表示各追加三种训练配置，选唯一共同方案对原V8三种子测试。显式固定GPU确定性，先21对原V8重复概率一致再搜索。科学commitbc09a242；预检51897487通过，准备51897574[0–20]、对照51897579[0–20]、搜索51897580[0–104]、初选51897582、细化51897583[0–62]、选择51897584、最终51897585[0–62]、汇总51897586全部已提交（277进程，最多12个V100）。2.1停止的35次验证及258作业、同卡非确定性诊断已完整保留，不用于选择，旧指标不改写。协议docs/FMT_v8_search_protocol_2.2.md；目标三任务等权F1相对提升15%–20%，不预设成功。

预检再次通过所有池化独立数值检查和四种训练设置。科学commitbc09a2422738048c6a63d3371bb721fd20b93aa1，配置a1bf07ef1e6bf2e24be5ad2d0699957e45fc6b981f8d381ae1816d9fc7b58bdf，源码清单8ea48292e0c555f83227645887f51866b95ef167b7bc699604171ab311d569e1，源码归档c44adde7814adbf41759b9e64a7208ff3d74e04916533a9be850ffc5600e9955。个人Ibex目录/ibex/user/zhanx0o/FMT_V8Search_Deterministic_20260915。当前科学结果待完成，未读取test。


<a id="fmtv8-search-final-2-2-2026-09-15"></a>

### Ablation_FMTv8_Search_2.2 — 池化与训练设置搜索完成

50个池化执行配置（48种不同表示）及前三种不同表示的三项训练配置，共59候选、1,239次主验证拟合完成。21组额外原V8重复和42对等价表示检查全部通过。仅按验证集三任务等权F1选择`p35/h0`，之后新种子与原V8配对测试126次；三任务等权测试F1相对变化+3.01%，**未达到15%–20%目标**。该结论限于本次固定候选和已用benchmark，不外推所有池化或FMT方法。

选定操作：每个描述量分别对六个排序值计算均值＋最大值；有效特征141维，输入信息保持中心频域23＋方向频谱72＋邻居频域138，不增加角度、kin或IVD估计。训练配置：`{"description": "Frozen baseline training and projection", "residual_model": {}, "residual_training": {}, "task4": {}}`。
...
本次选定的p35/h0提高Task3与Task5，但Task4-c下降，不能据此声称在三个任务上同时改善。Task4-c三个配对种子的差值均为负；其参数由71,490增至76,738，平均拟合时间由183.297秒增至470.107秒，当前没有体现该任务上的性能或训练成本优势。

验证搜索的最高三任务均值为p35/h0的0.753102，相比原V8 0.717429相对增加4.97%；最终测试的3.01%增益另行报告，二者不可混用。完整59组验证结果及各自池化定义见`outputs/Ablation_FMTv8_Search_2.2/validation_rankings.md`。单看各任务验证最高值，Task3为p38/h1（0.801992），Task5为p38/h2（0.747469），Task4-c为p13/h0（0.736591）；这些任务各自最优方案没有单独作最终测试，不据此替换预注册的共同方案。

此前v8.1历史测试F1为Task3 0.666199±0.009876、Task5 0.638731±0.008603、Task4-c 0.728480±0.004107；这些旧指标保持。上表原V8来自本批新种子和显式确定性执行，增益只按本批配对比较计算，不能把执行设置或种子差异算成池化收益。

Task3/5最终种子44/45/46，Task4-c种子96621/96622/96623；Task3/5先各取十流场平均，再与Task4-c的Channel+TBL合并F1等权平均。Task4-c继续使用4.14物理划分和固定0.5阈值；Task3/5沿用验证阈值规则。三个任务选同一个池化/训练配置，未按流场或测试结果分别挑方案。

#### 完整结果及成本

选定`p35/h0`：每个描述量分别对六个排序值计算均值＋最大值。输入为中心频域23＋方向频谱72＋邻居池化46，共141维。

| 任务 | 本批原V8 | 选定方法 | 相对变化 | 配对差值均值 ± 标准差 |
|---|---:|---:|---:|---:|
| Task3 | 0.670735 ± 0.000845 | 0.728163 ± 0.001514 | +8.56% | 0.057428 ± 0.001408 |
| Task5 | 0.637286 ± 0.003723 | 0.658147 ± 0.010829 | +3.27% | 0.020860 ± 0.011656 |
| Task4C | 0.725739 ± 0.017413 | 0.708685 ± 0.009891 | -2.35% | -0.017055 ± 0.007526 |

三任务等权测试F1：0.677920 → 0.698331，相对变化+3.01%。

标准差来自三个优化随机种子，不能解释为独立物理流场泛化的不确定性。选择只使用验证集；测试是此前已使用的benchmark。

| 任务/流场 | 本批原V8 | 选定方法 |
|---|---:|---:|
| Task3/boeing747 | 0.809848 ± 0.009650 | 0.852247 ± 0.008445 |
| Task3/channel | 0.196420 ± 0.029469 | 0.527566 ± 0.017364 |
| Task3/cylinder3d | 0.353326 ± 0.037812 | 0.409800 ± 0.014316 |
| Task3/deltaWing_LBM | 0.857471 ± 0.005985 | 0.865461 ± 0.004615 |
| Task3/deltaWing_resampled | 0.899851 ± 0.005070 | 0.900071 ± 0.004179 |
| Task3/f22raptor | 0.817673 ± 0.010975 | 0.815750 ± 0.010278 |
| Task3/halfcylinderRe640 | 0.697123 ± 0.007014 | 0.711453 ± 0.016646 |
| Task3/halfcylinderRe6400 | 0.598223 ± 0.011279 | 0.646754 ± 0.012339 |
| Task3/smokeBuoyancy | 0.723815 ± 0.007386 | 0.769596 ± 0.030250 |
| Task3/tangaroa | 0.753600 ± 0.008160 | 0.782928 ± 0.004779 |
| Task5/boeing747 | 0.755770 ± 0.013292 | 0.794884 ± 0.002764 |
| Task5/channel | 0.168953 ± 0.040199 | 0.196686 ± 0.079426 |
| Task5/cylinder3d | 0.466700 ± 0.022080 | 0.511112 ± 0.023327 |
| Task5/deltaWing_LBM | 0.831464 ± 0.005245 | 0.836270 ± 0.006455 |
| Task5/deltaWing_resampled | 0.852227 ± 0.007449 | 0.864926 ± 0.005118 |
| Task5/f22raptor | 0.746740 ± 0.002499 | 0.737060 ± 0.003329 |
| Task5/halfcylinderRe640 | 0.659729 ± 0.010419 | 0.671545 ± 0.006010 |
| Task5/halfcylinderRe6400 | 0.593502 ± 0.011397 | 0.619613 ± 0.003093 |
| Task5/smokeBuoyancy | 0.610173 ± 0.008403 | 0.642849 ± 0.006968 |
| Task5/tangaroa | 0.687606 ± 0.006104 | 0.706522 ± 0.007358 |
| Task4C/Channel | 0.769305 ± 0.018163 | 0.755521 ± 0.012932 |
| Task4C/TBL | 0.656131 ± 0.018273 | 0.632971 ± 0.019456 |

| 方法 | 任务 | 特征维数 | 总参数 | 可训练参数 | 含参照网络的完整拟合秒 | Raw＋本模型秒 | 分类/残差阶段秒 |
|---|---|---:|---:|---:|---:|---:|---:|
| p00/h0 | Task3 | 100 | 142,914 | 52,993 | 203.698 | 138.239 | 73.470 |
| p00/h0 | Task5 | 100 | 142,914 | 52,993 | 109.609 | 65.079 | 19.517 |
| p00/h0 | Task4C | 100 | 71,490 | 71,490 | 183.297 | 183.297 | 183.297 |
| p35/h0 | Task3 | 141 | 142,914 | 52,993 | 194.622 | 129.163 | 64.394 |
| p35/h0 | Task5 | 141 | 142,914 | 52,993 | 110.495 | 65.965 | 20.402 |
| p35/h0 | Task4C | 141 | 76,738 | 76,738 | 470.107 | 470.107 | 470.107 |

特征维数仅指有效几何特征：Task4-c另加一个有效线标记；Task3/5辅助输入补零到433槽并保留原Raw骨干。
...
本批总分配GPU时间24.198小时；停止的2.1及其诊断1.275小时；合计25.472小时。此数包含预检、缓存处理和评估期间的GPU占用，不等于纯训练时间。

1,239次主验证拟合的分类/残差训练合计17.316小时，共同Raw/Raw-wide骨干0.527小时，额外21次重复性拟合0.302小时。

完整59候选验证排名见`validation_rankings.md`和`candidate_ranking.csv`；原50执行配置含两对等价映射，即48种不同表示。

科学源码`bc09a2422738048c6a63d3371bb721fd20b93aa1`；独立复算1,239份验证及126份最终测试预测、21对重复和42对等价结果，核对277个调度进程/554次起止事件，全部通过。

Task4-c Conv3D参考仍为用户修订的0.7499±0.0126，本次没有重训。

证据：`outputs/Ablation_FMTv8_Search_2.2/`下的`summary.json`、`candidate_ranking.csv`、`report_metrics.json`、`independent_audit.json`、`completion_audit.json`和逐次预测。归档SHA256 `0abf74c3438cf400d3cfd499bb423ed3c04de9447407b2eac6ec1583e6012634`；科学commit `bc09a2422738048c6a63d3371bb721fd20b93aa1`，配置SHA256 `a1bf07ef1e6bf2e24be5ad2d0699957e45fc6b981f8d381ae1816d9fc7b58bdf`。277个Slurm进程和554条起止事件全部核对，临时模型已清除；2.1失败/取消及35次部分验证单独保留，不用于选择。调度从12卡改为24卡的实际命令和回执亦保留，不改变科学配置。


<a id="fmtv8-table-provenance-1-1-2026-09-15"></a>

### Verify_FMTv8_TableProvenance_1.1 — 验证／测试表格来源复核

用户质疑p35/h0的Task4-c为何在两表分别为0.7271和0.7087。此前展示：两张表的任务列只写Task3/5/Task4-c，通过正文说明验证/测试；现在修正：列标题直接写验证或测试，验证列标注种子，测试列标注三种子，并列出逐次来源。错误在展示未充分区分集合、种子与统计方式；本次复算未发现F1计算或抄表错误，旧科学指标不修改。

p35/h0保持141维、76,738参数、h0训练设置和同一训练标准化；0.7270560190703218来自搜索种子96611、3,000验证样本，所选轮次41，真正例305、假正例85、假负例144，F1=610/839。最终三个新种子分别重训并按各自验证集选轮次，测试集为10,000样本，固定阈值0.5：

| 阶段 | 种子 | 验证F1（3,000样本） | 测试F1（10,000样本） | 验证选定轮次 |
|---|---:|---:|---:|---:|
| 搜索 | 96611 | 0.727056019070 | 未评估 | 41 |
| 最终重训 | 96621 | 0.704600484262 | 0.707663782447 | 107 |
| 最终重训 | 96622 | 0.726627218935 | 0.699344364658 | 43 |
| 最终重训 | 96623 | 0.723854289072 | 0.719046163253 | 73 |

最终测试均值为0.7086847701194756，样本标准差0.009890501950673546。0.7271与0.7087不是同一训练运行在同一集合上的前后成绩，差值也不能单独解释为验证到测试的泛化差距，因为优化种子同时改变。

本次独立代码直接由逐样本标签与概率按整数混淆计数复算F1，不调用原训练器或指标函数：1,239份搜索验证、126份最终训练验证、126份最终测试，F1差异全部为零；59候选验证汇总与6组最终三种子均值/标准差均通过。2,859份相关原始证据与原归档SHA256一致；通过只读SSH重新读取原4.14 metadata，Task4-c预测内标签、流场、尺度、头区、实例数组内容全部一致。没有新增训练或用测试结果重新选择配置。
...
<a id="p35-structure-explanation-2026-09-15"></a>

### p35/h0实现解释与后续双集合评价偏好

按科学commitbc09a242核对固定特征与实际下游代码，相关科学文件未发生变化。p35为中心DFT描述量23＋方向频谱72＋邻居逐特征均值23＋最大值23，共141维；23维是六频率×实/虚向量范数及余弦三项＋五个相邻频率三重积，非23个原始几何量。方向分支采用32点、正交归一化FFT；中心/邻居采用31个步差分、默认归一化FFT。Task3/5保持原161维缓存的neighbor_scale/weight=1/1；Task4-c沿用100/0.5，原有任务适配不得混称完全相同。

Task4-c逐线141→128→128，再对有效线均值/最大聚合为256，分类头256→128→64→2，共76,738参数。Task3/5原轨迹网络89,921参数先训后冻，141补零至433后投影64，与冻结128维几何表示拼接训练残差头，共52,993可训练参数，总142,914。参数逐层公式加总与原运行记录一致。完整公式、数组顺序、原始几何、标准化、训练和选择规则见docs/FMT_p35_h0_structure_2.2_zh.md；本轮没有运行新训练或改写成绩。

用户要求后续不额外拆出第三份数据，偏好训练＋测试/评估两集合；已记录于research_tasks_and_protocol与AGENTS。集合可保持不参与梯度更新，但若用于选择59候选、轮次、融合系数或阈值，仍属于参与模型选择的评价信息。后续使用两集合时明确披露这一用途，不称独立的全程未见测试；历史validation/test用途与数值不改。


<a id="fmtv8-norm-frequency-3-1-2026-09-15"></a>
## P35统一归一化与频率：20组预注册
...
用户确认将标签改为清洗后有效播种点归属：单实例至少50%为Hairpin，全部GT外为Non-hairpin，其余及两个实例50%并列剔除。每实例目标500正＋500负，正类含3个不同中心的必选head束。head由GT内部速度–curl夹角更接近垂直识别，恰好45°归平行。扩张采样盒相交的实例同集合；Channel67/7、TBL52/6划分，合计119,000训练＋13,000测试。

旧4.14正样本训练覆盖Channel62/74、TBL54/58。插值候选采样下132实例全部具备head；最终分组小预检132实例、792束、396必选head通过。初始直接随机实例划分在TBL27只获得1/3 head束，因穿入另一集合GT而失败，促成正式训练前的几何分组修订；没有用新测试成绩选划分。单实例全配额工程检查Channel39及TBL60各1,000束通过，记录保留。不能据此提前宣称全量正式数据完成或分类性能改善。

固定p35为141维、76,738参数；本版小Conv头宽70、72,192参数（94.0759%），32³/36³/48³共用结构。固定150轮、dropout0.15、weight decay0.0001、AdamW0.001余弦降至0.00001，三个种子91511–91513；最后一轮后测试一次，不另设validation，不用新测试选模型。旧h0的验证早停不能与该训练日程混称。

CPU独立体素累加、稀疏float16逐值还原、有效线屏蔽及四模型前后向检查通过。现有查看器增加两个独立预测类别数量滑块；JavaScript语法和配额逻辑通过，浏览器交互检查未完成。训练完成后自动导出每实例20束、含3束head的查看器包，明确按真实标签分层抽样。
...
### P35 3.1：短调度时间及Task4-c单配置执行

V100共享队列限制了进度。已完成准备/预检实际内存峰值低于3GiB，因此将未开始的Task3/5请求从48GiB降至8GiB，Task4-c降至24GiB，102项调整逐项登记。搜索时间预留先由4小时降至2小时；23个五配置进程已完成时，实际用时199.1–431.6秒，尚待运行的Task3/5索引23–79进一步预留30分钟。训练100epoch/patience20以及任务4-c500epoch/patience50均未改变。更新命令对已经结束的索引给出提示，回读确认剩余请求成功更新；这些不是训练失败。

Task4-c原索引51906472_80–83在PENDING/未运行、未产生任何Task4-c搜索结果时先hold。提交51907432[0–19]（12:24:16 UTC），每项只运行一个原候选，24GiB、V100、4CPU、30分钟，最多4并行；Task3/5节流为8，全实验最多12卡。之后取消原四项，并将汇总51906473依赖改为等待原数组51906472与新数组51907432结束，汇总函数仍强制检查420项候选结果齐全。全部149项调度记录保留，预计145项执行、4项启动前取消，290条runtime事件；候选训练总次数仍为420。
...
### P35统一归一化与频率3.1：20组完成

实验`Ablation_FMTv8_NormFrequency_3.1`。用户明确要求全部统一几何归一化、邻居倍率和特征标准化，并直接用原验证集合比较；本次20组×21任务/流场单元，共420次候选拟合，另有21项原p35完整复现。Task3/5 seed40，Task4-c seed96611；不追加最终测试或三种子训练。

唯一共同最佳为`n0_k06`：所有有效点减质心、除最大半径，邻居倍率/权重固定1，训练集逐特征标准化；保留包含DC的6个DFT频率。中心23＋方向72＋邻居逐特征均值23＋数值最大值23＝141维。Task4-c另有有效线标记。沿用h0结构和训练；Raw分支不变，无165维角度、kin或IVD估计输入。

|任务|原p35/h0|统一配方 n0_k06|绝对差|相对变化|
|---|---:|---:|---:|---:|
|Task3|0.794476|0.789635|-0.004841|-0.61%|
|Task5|0.737775|0.751417|+0.013642|+1.85%|
|Task4C|0.727056|0.740214|+0.013158|+1.81%|
|三任务等权均值|0.753102|0.760422|+0.007320|+0.97%|

在本次单种子配置选择集合上，共同最佳相对冻结原p35三任务均值提高0.97%。这是20组实测中的选择结果，不是未参与选择的测试结论；各任务的增减以上表为准，不能把综合变化说成每个任务均提升。原p35的21项逐样本预测概率、阈值、所选epoch完全复现，保留其分任务归一化作参考；新K=6方案也不等同原p35。全部20组数据见paper_tables_tasks_3d同锚点及outputs/Ablation_FMTv8_NormFrequency_3.1/performance_tables.md。

频率结果：四种归一化下，Task5和Task4-c各自均在K=6取得该配方最高F1，Task3则均在K=16最高。因此本轮共同配置保留6频率；不能据此断言高频本身无用或加入高频必然引起过拟合。按五档K平均的三任务分数，n0/n1/n2/n3分别为0.744744/0.744012/0.741610/0.740942；最大半径＋普通标准化在该网格整体表现最高，但这些单种子差值未做统计显著性判断。

|任务|参数|特征准备秒|拟合秒|合计秒|
|---|---:|---:|---:|---:|
|Task3|142914|1.876|43.610|45.485|
|Task5|142914|1.558|19.170|20.729|
|Task4C|76738|6.874|176.357|183.231|

Task3/5各142,914参数（冻结Raw89,921＋训练残差52,993）；Task4-c最佳76,738参数。表中Task3/5为十流场平均，Task4-c为一次合并训练；时间包含验证和选择，但不含原轨迹积分。共享Raw合计15.400分钟、Raw-wide选择参照15.196分钟，只训练一次供20组共用。本批实际分配GPU 6.572卡小时，包含预检、准备、复现和搜索，不能除成单模型的推理成本。

独立复算441份逐样本预测和420项配置身份/维数/参数/训练统计；333份源文件及缓存、21项原p35复现、149项Slurm记录、145项执行和290条起止事件全部通过。原Task4-c四项在启动前取消，拆成20项单拟合作业，没有额外科学候选；所有执行进程COMPLETED/0，模型checkpoint在汇总后清理，证据包无权重。原Task3/5结束后将剩余Task4-c上限从4调到12，整批总上限仍12，调整已登记。

准备/复现科学commit`0d77e3e5`，新候选科学commit`c61434ac`，调度适配器commit`9b6b8451`。主代码`FMT_Utils/FMT_P35_NormFrequency_3_1.py`、`experiments/FMTv8_NormFrequency_3_1.py`；配置`config/Ablation_FMTv8_NormFrequency_3.1.json`。独立核验`experiments/Audit_FMTv8_NormFrequency_3_1.py` SHA256 `86c88e04a32e59260ab3d72ad758acd086c2930d3a50181eaa1303bf3be41431`；证据包SHA256 `44b0f8612b27109dcb9d889708a41fac5fcae2eb4dcce82f75d5bd7b1f5c4a0d`。本次授权实验完成，不自动追加搜索。

<!-- task4c-v915-v2 -->
## Task4-c-v9.15_v2开发与GPU修复（2026-09-15）

用户确认保持132,000束，每实例500正＋500负，只增加长度种类；每实例3个不同中心必选head仍是硬性条件。此前head采样已在132实例小预检通过，不能把“已实现”混同于正式数据构建完成。状态核对：v9.15作业51907304因平均池化CUDA反向确定性要求失败，47个依赖进程全部在开始前取消；旧版没有正式新数据或F1。

v2第一次模型预检51908822（4bc96e6b）进一步发现最大池化CUDA反向同样不支持严格确定性。修复两个池化后，第二次V100预检51908979（af392332）在gpu609-09完成0:0，p35/三分辨率Conv3D前向、反向、优化、体素还原及独立参考梯度通过，参数仍76738/72192。失败与两次尝试全部登记，不关闭确定性。该结论只支持工程实现可运行，不支持分类性能判断。

新长度表生成器支持Channel11档/TBL12档，与4档邻居距离组合成44/48种尺度；用户区间分别[0.08,0.12]/[3,5.5]，ds范围不变。由于“总长度”与此前单向ds×maxiteration存在二倍差异，已询问但尚未确认，正式配置明确待确认并拒绝启动采样/训练。两种含义的46项解析场长度检查（1242条线）通过，不以此代替用户选择或真实场检查。协议`docs/Task4C_instance_coverage_protocol_9.15_v2.md`。
...
<a id="ftle-p35-fusion-final-2-1-2026-09-15"></a>
## Other_FTLEP35Fusion_2.1：P35融合的固定三种子测试

开发科学commit52ec9d8b9be9444a2b49da1b0c0995602562fd4f，最终及数值审计科学commitf0993a58fe744de94991341ee118594e832ba2ed。
最终配置SHA256 `334e2e0051896b9999723112bab8dc908ad4618a64f07d600a241d282bc806b4`。88次开发拟合、264份验证预测；固定pyramid_p35后，四流场×4/8××六方法×三个新种子98211–98213共144次最终拟合、864份验证/测试预测独立复算PASS。
完整时间窗和源帧支撑跨train/validation/test隔离；每次停止点与全部方法固定后才读取对应test。该test是1.1/1.2已用benchmark，不能称全新确认集。历史1.1/1.2不改，未按本轮test追加调参。

| 方法 | 4× PSNR (dB) | 8× PSNR (dB) |
|---|---:|---:|
| ESPCN | 36.715430 ± 0.057489 | 32.583144 ± 0.101566 |
| U-Net | 31.215084 ± 0.133174 | 28.223757 ± 0.195213 |
| 同结构无几何 | 38.921350 ± 0.046051 | 33.196849 ± 0.135066 |
| 同结构Raw | 39.473647 ± 0.031022 | 33.021879 ± 0.066481 |
| 同结构保线身份傅里叶 | 39.218560 ± 0.004964 | 32.817021 ± 0.097170 |
| 选定P35 | 39.630193 ± 0.034499 | 33.157295 ± 0.057020 |

PSNR为峰值信噪比，范围由训练真值固定；每种子先平均三张测试切片，再平均四流场；±为三个优化种子的样本标准差。MSE为均方误差，配对变化在同流场/倍率/种子内计算后等权平均。

- 4×相对`espcn`：配对平均PSNR +2.914764dB，配对MSE相对变化-38.030%。
- 4×相对`unet`：配对平均PSNR +8.415110dB，配对MSE相对变化-57.996%。
- 4×相对`pyramid_none`：配对平均PSNR +0.708843dB，配对MSE相对变化-12.585%。
- 4×相对`pyramid_raw`：配对平均PSNR +0.156547dB，配对MSE相对变化-3.067%。
- 4×相对`pyramid_signed`：配对平均PSNR +0.411633dB，配对MSE相对变化-9.655%。
- 8×相对`espcn`：配对平均PSNR +0.574151dB，配对MSE相对变化-2.091%。
- 8×相对`unet`：配对平均PSNR +4.933539dB，配对MSE相对变化-48.620%。
- 8×相对`pyramid_none`：配对平均PSNR -0.039553dB，配对MSE相对变化+3.351%。
- 8×相对`pyramid_raw`：配对平均PSNR +0.135416dB，配对MSE相对变化-3.628%。
- 8×相对`pyramid_signed`：配对平均PSNR +0.340274dB，配对MSE相对变化-7.987%。

主要结论：4×四流场平均上，选定P35高于原ESPCN/U-Net、同结构none和Raw；三个数值流场相对none均为正，Doublegyre为零。8×四流场平均高于ESPCN/U-Net/Raw，但相对同结构none为−0.039553dB；Doublegyre8×还低于ESPCN1.660879dB。因此本次支持4×的额外p35编码收益，尚不支持8×具有稳定的独立编码收益，也没有达到所有流场均超过ESPCN。8×相对none的开发结果为+0.282670dB → 固定三种子测试为−0.039553dB；验证集与测试集、新优化种子及CPU/GPU执行均有差别，不能仅凭这个变化确定某一个单独原因。原开发成绩保留，不按测试结果追加配置或换选择规则。

分流场结果：

- cylinder2d 4×：P35 PSNR=41.145248；相对ESPCN/U-Net/同结构none/Raw的变化依次为+7.277743/+9.127679/+2.259790/+0.491466dB。
- cylinder2d 8×：P35 PSNR=32.196263；相对ESPCN/U-Net/同结构none/Raw的变化依次为+3.175717/+5.378195/+0.852714/+0.120286dB。
- boussinesq 4×：P35 PSNR=24.243989；相对ESPCN/U-Net/同结构none/Raw的变化依次为+0.318288/+0.650655/+0.318509/+0.286198dB。
- boussinesq 8×：P35 PSNR=21.720870；相对ESPCN/U-Net/同结构none/Raw的变化依次为+0.206744/+0.431094/+0.107325/+0.456736dB。
- pipedcylinder2d 4×：P35 PSNR=28.576885；相对ESPCN/U-Net/同结构none/Raw的变化依次为+0.731312/+1.852963/+0.257073/-0.151477dB。
- pipedcylinder2d 8×：P35 PSNR=25.334891；相对ESPCN/U-Net/同结构none/Raw的变化依次为+0.575024/+1.362554/-0.183501/+0.227332dB。
- doublegyre2d 4×：P35 PSNR=64.554652；相对ESPCN/U-Net/同结构none/Raw的变化依次为+3.331712/+22.029142/+0.000000/+0.000000dB。
- doublegyre2d 8×：P35 PSNR=53.377157；相对ESPCN/U-Net/同结构none/Raw的变化依次为-1.660879/+12.562311/-0.934751/-0.262690dB。

本次推断只针对这套固定比较：相对旧ESPCN/U-Net的差异同时包含新网络结构、完整切片训练、双三次残差和已知节点约束；相对同结构none与Raw才分别检查额外几何与当前p35表示的贡献。不能把整体提升全部归因于傅里叶变换，也不宣称所有流场、种子或输入预算均获益。step0被选中的次数为`{'pyramid_none': 3, 'pyramid_p35': 3, 'pyramid_raw': 3, 'pyramid_signed': 3}`，这种运行的输出是双三次插值，没有显示所接几何编码的额外收益。三种子不视作三个独立流场，未据此宣称统计显著。

五线严格为中心、±x、±y，每条32时刻；p35为102维加共同尺度1维，实际保留Fourier变换。原低FTLE本来使用同一邻居终点，不能称相对本标量基线额外积分四条线。Raw、p35及保线身份傅里叶共用参数张量形状、初始化和下游结构；活动输入列数不同。

调度审计共57进程：44完成、1个原数值审计失败、12个未启动取消，90条执行事件全部核对。原失败原因与修订见`FTLE_p35_numerical_audit_2.1.md`；最终保存统计量对独立float64参考的最大标准化差2.6718559e-06。实际最终设备/拟合数为`{'NVIDIA GeForce GTX 1080 Ti': 144}`，同一配对组共用设备，跨设备耗时不能解释为纯架构加速。没有模型文件。
...
用户明确否决13实例留出、局部标签、每实例500正500近邻负和固定150轮。科学commit9769c68d及作业51911059–51911066所产生的运行标为废弃；其0.469009测试F1保留作过程记录，**不是有效v2结果，不用于论文方法比较**。剩余六个Conv训练、三个排队训练及汇总于集群本地2026-09-15 19:32:33取消，原数据/代码/预测/日志保留。

名称仍为v2，execution_revision=restored_4.14。恢复4.14完整头区GT标签、旧负类头区、同实例内空间块与1–4h距离约束、0.25/0.5/1h邻距、验证平台降lr和最佳轮次/早停。完整实例专门留出数为0；所有评价实例必须在训练中出现。总132,000束沿旧27:3:10比例，为89,100训练＋9,900验证＋33,000测试。仅扩展已确认的单向长度，以及正类头区底部播种密度。p35/n0_k06和三种小Conv结构保持。
...
用户要求停止当前卷积，回到原约0.72对应的4.14数据，仅增加底部各实例采样密度，不增加积分长度。旧修正版v2的51914157_9–11三次Conv48及merge51914158于2026-09-15 23:32:16（Ibex本地；20:32:16 UTC）取消；三次训练耗时1:53:57/1:47:09/0:47:31，不报告未完成模型的测试F1。旧数据/日志/九次已完成结果保留。

修正版v2科学commit1e088e49的最终已完成部分：FMT三种子测试0.734754±0.002949、平均13.352分钟；Conv32为0.838906±0.009295、96.600分钟；Conv36为0.855863±0.006007、145.606分钟。全部九份train/validation/test预测、源标签和覆盖已独立复算通过。它说明该数据上的卷积测试F1较高，不能把差异单独归因于密度或积分长度。

新版本Ablation_Task4C_BottomDensity_1.1直接读原4.14缓存（科学commit5c9c9042），保留全部40,000束和原验证/测试文件；按已说明的默认方案每个原训练实例增加100束，共11,600束，训练38,600、验证3,000、测试10,000。仅在原正类头区、同实例内部、训练空间块的底部一半单元播种，并保持原评价中心1h间隔。旧负样本不变，标签/积分参数/清洗不变，不采用v2新增长度筛选。原Channel三档0.08/0.10/0.12与TBL4/5/6原样恢复。最佳p35/n0_k06及Conv8/24/32各三种子，结构与训练器冻结，不用新test调参。CPU四模型/体素/标准化检查通过，真实实例pilot和正式作业后续登记。

首次1.1预检（科学commit2ea8da44）TBL54实例通过，Channel实例11在新增GT内部硬下半区限制下0/3，全部18个后续进程取消、未构建正式数据或训练。该限制比原4.14允许区域严格，已撤回：每实例偶数轮次使用原候选训练单元池，奇数轮次使用其下半区；不再要求中心位于GT内部，仍由旧完整头区标签赋值。新增样本仅增加密度，不更改旧标签、清洗或空间间隔。新运行不覆盖失败文件。

科学commit99cb6438的第二次部署：V100预检通过，Channel62/TBL54实例各3束（共348）真实预检全部完成。51919126–51919132共21进程已提交，正式11,600束追加及四模型训练由核验依赖控制。新根目录/ibex/user/zhanx0o/FMT_Task4C_BottomDensity_20260915_r2。此时尚无新训练F1。

正式数据和全量审计51919129完成：每实例100束共新增11,600；train38,600/validation3,000/test10,000。原训练所有字段逐值保留为前缀；原验证/测试geometry.npy、seeds.npy、metadata.npz逐字节相同，全部原负样本保持。新训练中心到原评价中心最小间距/h：Channel验证1.011502、测试1.002166；TBL验证1.008002、测试1.000679。原积分配置逐项检查通过；无新增长度档位。编码已启动，训练依赖链继续。

<!-- task4c-bottom-density-final-20260916 -->
## Task4-c底部采样加密1.1完成（2026-09-16）

科学commit99cb6438；51919126–51919132共21进程全部成功，12次训练及汇总完成，Ibex本地结束时间2026-09-16 01:10:38。原4.14全部40,000束保留，仅对116个原训练实例每个新增100束底部偏置正类样本；38,600训练/3,000验证/10,000测试。旧训练逐值前缀、负样本和原验证/测试文件哈希核验通过，原积分参数不变。
...
三种子固定0.5阈值，标准差为样本标准差；时间为V100训练及每轮验证，不含几何生成、编码与最终train/test推理。逐种子测试如下。
...
<a id="task4c-bottom-density-fivefold-final-20260916"></a>
## 2026-09-16：Task4-c五倍训练1.2最终结果
...
三种子均值与样本标准差；时间为每次训练及每轮验证，不含几何生成、编码及最终推理。
...
方法结论：本次五倍训练下FMT测试F1=0.888404，Conv16=0.913490，Conv24=0.933198；FMT准确率仍低于两种卷积，但平均训练28.565分钟，比Conv16的72.521、Conv24的141.377分钟少。相同评价文件下，上版1.1 FMT0.760842→0.888404（+12.756个百分点），Conv24 0.831460→0.933198（+10.174个百分点），两种方法均提高。扩大数据也增加每轮优化步数，且实际早停轮数不同，因此这里不能分离新增数据与更多训练计算的贡献。没有旧1.1 Conv16结果，不为其填造增益。该结果仍衡量同源流场/共享实例的局部空间泛化，不改称独立实例测试；旧1.1结论保持。
...
<a id="task4c-fivefold-conv8-conv12-final-20260916"></a>
## 2026-09-16：五倍数据8³/12³ Conv3D最终结果（1.3）
...
三种子均值及样本标准差；平均时间包括训练和每轮验证，不含体素编码及最终推理。
...
方法结论：新8³/12³测试F1分别0.855112±0.002812、0.896707±0.003988，平均训练38.134/54.009分钟。相同数据的FMT参考为1.2的0.888404±0.011497、28.565分钟，因此FMT均值比8³高0.033293，比12³低0.008303，且训练耗时低于这两种卷积。12³的均值优势未作统计显著性认定；三种子中有一种FMT高于12³。分流场：12³的Channel为0.924214，高于FMT0.900873；TBL为0.855686，低于FMT0.869583。相同参数的卷积测试均值按8³/12³/16³/24³为0.855112/0.896707/0.913490/0.933198；该比较支持本数据上较高体素分辨率的准确率优势，同时训练时间增加，不推断任意分辨率或未见流场必然如此。参考1.2 FMT/Conv16/Conv24全部复用原已核验结果，没有重新训练或修改旧指标。
...
全部完成于2026-09-16T09:00:02.551084+00:00。本次请求完成，不自动追加调参或新分辨率。查看器数据包已由merge生成，本地交互页面尚未重建。


<a id="task4c-geometric-baselines-plan-20260916"></a>
## 2026-09-16：Task4-c三种几何基线1.1立项

用户明确要求切线/曲率池化、整束Point-NN点云特征、参数匹配BiLSTM三种分类基线。完整复用已核验BottomDensity1.2数据（193,000训练、3,000验证、10,000测试，源科学commit a7643890），保持标签、实例、积分、空间划分、训练及验证选轮次规则。三种方法参数分别76,782/76,704/76,786，均与当前FMT76,738相差不到0.1%；种子96611–96613，V100，固定0.5阈值。

Point-NN参考官方a85bfc3，明确使用全束点云四层固定编码，修改为每束归一化、确定性采样及用户指定的MLP分类器，不宣称复现论文完整分类器。BiLSTM用每条线32点的空间顺序，线间对称池化；未使用旧四分类自编码网络。协议docs/Task4C_geometric_baselines_protocol_1.1.md完整记录算法、容量、标准化和预检规则。

CPU预检通过：直线/圆弧曲率、无效线排除、点云排列不变、批次独立、BiLSTM线间顺序不变且线内序列敏感、精确参数量和有限梯度。源代码逐字比较确认训练循环只改变模型工厂和标准化适用方法，没有调整优化与选择算法。当前无新测试F1；不预设三种基线结果。
...
用户要求在冻结p35融合方案上增加2×，并把bilinear/bicubic三个倍率一起列入最终表。科学commit `64378a0554416a84cb1c5f907425edc613b1c940`，config `config/Other_FTLEP35Fusion_2.2.json`，SHA256 `5ee322ef61486da07409e3a21eb0c9a7f03914d6f42f9b651211a5bd967163a7`；协议`docs/FTLE_p35_scales_protocol_2.2.md`。全部直接调用2.1冻结编码/网络/优化，未重选p35或改训练参数；2×新增72次拟合，4×/8×144次拟合来自2.1科学f0993a58。

本轮72次拟合全部GTX1080Ti；同流场/倍率/种子六方法共用一个GPU进程。四场等权，种子98211–98213；每种子先平均3张测试切片，再平均流场，±为三种子样本标准差。PSNR是峰值信噪比，越高越好；范围由训练数据确定，所有倍率/方法相同。插值确定性，不报伪随机波动。
...
p35四场均值47.527190±0.045904 dB，相对ESPCN +5.072425、U-Net +11.779160、相同网络仅低FTLE +1.119368、Raw +0.295512、保线身份傅里叶系数 +0.804576 dB；相对bilinear/bicubic为+7.931542/+5.631870 dB。这里仅支持本benchmark上已冻结方案的观测均值，三个优化种子不是独立物理流场，不宣称统计显著或普遍有效。

| 2×流场 | p35 PSNR | 相对ESPCN | 相对仅低FTLE | 相对Raw | 相对bicubic |
|---|---:|---:|---:|---:|---:|
| Cylinder | 52.322428 | +12.098589 | +4.105261 | +0.508296 | +13.208398 |
| Boussinesq | 28.739365 | +0.042741 | +0.052653 | +0.095181 | +1.141452 |
| Pipe | 33.338117 | +0.724265 | +0.319556 | +0.578573 | +8.177630 |
| DoubleGyre | 75.708851 | +7.424103 | 0 | 0 | 约0 |

2×的独立p35增益主要来自Cylinder，Boussinesq相对ESPCN的改善很小。DoubleGyre的四组融合网络、三个种子全部由验证选择step0，保持初始双三次插值；相对原bicubic约2.5e-6 dB来自归一化/浮点舍入，不能解释为编码收益。整个网络相对ESPCN的大幅差值包含双三次残差初始化、节点约束、整场训练和结构贡献，不全部归因于傅里叶编码。五线仍与计算低FTLE共用，不声称额外积分四条线。

### 与此前结论的关系

此前2.1仅有4×/8×：p35相对同结构仅低FTLE为+0.708843/−0.039553 dB；现在新增2×为+1.119368 dB。4×/8×原预测、指标和判断完全不变：8×没有显示优于无几何输入的四场平均PSNR收益，DoubleGyre8×仍低于ESPCN。新增插值行未改变此前学习方法的排名；它明确显示DoubleGyre2×/4×的高分主要来自平滑场的双三次插值。

不同指标的平均不能混用：8×物理单位RMSE均值p35=0.21678，低于仅低FTLE=0.21938；这与PSNR平均的排名不同，原因是逐场对数PSNR和物理误差均值并非同一种聚合。2×SSIM均值p35=0.98097，也略低于仅低FTLE=0.98136。这里“没有8×平均收益”严格指预先采用的四场等权PSNR，不等于所有误差或结构指标都更差。完整指标均保留，不按结果更换主指标。

### 完整性与边界

源数据为1.2原12训练/3验证/3测试，未重采样或积分。完整积分窗与源帧跨集合隔离；test未用于本轮训练停止点/配置选择。它仍是历史benchmark，不称全新确认集。2×仅扩展倍率，保持原dropout=0；1.2的dropout0/0.15及旧FMT/距离＋夹角/FMT v5对照保留历史表，不与本轮p35混用。

独立重算432份新学习预测和144份插值预测，120行合表全部PASS。三倍率的验证/测试真值和mask逐元素一致；逐流场训练PSNR范围跨倍率/方法完全一致；24组插值统计复现1.2冻结结果。24个Slurm进程全部COMPLETED，48条开始/结束事件独立核对通过；无失败/取消、无checkpoint。
...
<a id="task4c-pointnet-plan-20260916"></a>
## 2026-09-16：追加经典PointNet小宽度基线1.1

用户追加经典PointNet。为延续同等参数比较，保留原论文两个T-Net和正交正则、BatchNorm/ReLU及全局最大池化，只缩小通道；方法明确命名pointnet_small，总76,749参数，与FMT76,738相差11。原始宽度二分类实现为3,470,283参数，本次不运行；此前参数量选择的可选问题未收到回复时按已说明的匹配方案推进。官方代码固定2618f72bc1a0fd21b074096e748016960d44ef55，架构及损失逐项核对，MIT许可证保留。

完整复用BottomDensity1.2数据193,000训练/3,000验证/10,000测试，标签、积分、划分和训练预算不改；三种子96611–96613，V100。现有Point-NN等前三基线不受影响。PointNet模型特有的训练损失为CE+0.001×0.5×sum((AA^T-I)^2)，直接遵循官方半平方和；验证选模型和调lr仍使用原固定F1/AP及CE规则。

CPU通过初始化/训练后点顺序不变、推理批次独立、训练与推理padding排除、两个T-Net恒等初始化与可训练性、正则项解析值/梯度、全量参数及有限梯度检查。逐字源比较确认旧训练循环仅修改模型工厂、固定特征标准化开关及添加PointNet正则项。协议docs/Task4C_pointnet_protocol_1.1.md；配置SHA256 `d4fba7fcab4113e911d74bfcd1b1f48688ae145b99e66ce14f4e7a82fa1e1f51`。暂无正式训练或测试指标，不预设性能。


<a id="task4c-pointnet-submitted-20260916"></a>
### PointNet小宽度基线1.1通过真实预检并提交
...
Scientific commit `9d05abc571bc7c45e5da8a8f9f3ee55db367d467`; config `config/Ablation_Task4C_FPSAugmentSearch_1.1.json`, SHA256 `4f5b741fc131a28ee27b7dd6588426d7202579e0a527fd7b8d86d32adda10675`. Remote checkout `/ibex/user/zhanx0o/FMT_Task4C_FPSAugmentSearch_1p1_20260916`. Submitted 2026-09-16 19:12:15–19:12:16 UTC (22:12 Saudi). Jobs51974951preflight,51974952reuse,51974953pilot,51974954[0–263]screen,51974955shortlist,51974956[0–35]refine,51974957selection,51974958[0–5]final and51974959merge submitted (312processes). CPU checks passed, GPU preflight initially pending, later stages dependency-pending. No performance result yet; frozen originals remain unchanged.

At2026-09-16 19:15UTC, V100 preflight51974951 and data reuse51974952 completed. Exact frozen FPS feature and all augmentation/parameter/gradient checks passed; all18 source hashes unchanged,193000/3000/10000 rows. Real-data pilot51974953 waits for GPU; no screening or new F1 yet.
...
用户确认离散相邻时刻刚体最小二乘实现。新方法先消除中心平移和最佳相邻旋转，再由初始物质邻居确定朝向，使c156含方向分量的完整输入可保持客观；拟合用原同步物理时刻，48点逐线弧长重采样只在去相机之后进行。对照fmt_c156和更大首层的坐标Raw网络，不更改冻结的FMT/Task3/5历史结果。三种子180次训练固定，不搜索，先工程检查再完整训练。原验证选择用途保留，原测试为已用benchmark，不声称全新独立确认。
