"""Contracts for preregistered Task3 activation-residual-mix portfolio 94.1."""
...
import Audit_Task3_AuxiliaryActivationResidualMixPortfolio_94_1 as audit_module
...
from Select_Task3_AuxiliaryActivationResidualMixPortfolio_94_1 import (
    _canonical_sha256,
...
SOURCE_92_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryActivationInputShiftPortfolio_92.1.yaml"
)
SOURCE_93_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryActivationResidualMix_93.1.yaml"
)


class AuxiliaryActivationResidualMixPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "auxiliary_activation_residual_mix"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.232
        )
        self.assertEqual(spec["selection"]["target_absolute_fmt_f1"], 0.899)
        self.assertFalse(spec["confirmation_opened"])

    def test_source_config_hashes_are_frozen(self):
        config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
        self.assertEqual(
            _canonical_sha256(SOURCE_92_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_93_CONFIG),
            config["sources"]["auxiliary_activation_residual_mix"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_and_cleanup_contract(self):
        ...
        self.assertNotIn(
            "Select_Task3_AuxiliaryActivationResidualMixPortfolio_94_1",
            source,
        )
        self.assertNotIn(
        ...
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_activation_residual_mix_portfolio_94.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_activation_residual_mix_93.1_cleanup.sh"
        ).read_text(encoding="utf-8")
