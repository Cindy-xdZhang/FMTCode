...
def _validate_screen_seed_contract(spec: dict) -> None:
    """Require every training seed to have a declared frozen Raw backbone.

    Task3 residual training loads ``<dataset>_raw_seed<seed>.pt`` for both the
    FMT and matched Raw-PCA arms.  Declaring the available backbone seeds in
    the experiment config catches an impossible array before any GPU job is
    submitted.
    """
    screen_seeds = [int(value) for value in spec.get("screen_seeds", [])]
    if not screen_seeds or len(screen_seeds) != len(set(screen_seeds)):
        raise ValueError("screen_seeds must be unique and non-empty")
    if "available_raw_checkpoint_seeds" not in spec:
        return
    available = [
        int(value) for value in spec["available_raw_checkpoint_seeds"]
    ]
    if not available or len(available) != len(set(available)):
        raise ValueError(
            "available_raw_checkpoint_seeds must be unique and non-empty"
        )
    missing = sorted(set(screen_seeds).difference(available))
    if missing:
        raise ValueError(
            "screen_seeds lack frozen Raw checkpoints: "
...
def _load_spec(path: str | Path) -> dict:
    ...
    _validate_screen_seed_contract(spec)
...
def _global_candidate(spec: dict, candidate: dict) -> dict:
    """Aggregate one unchanged Task3 FMT recipe over every dataset."""
    group_rows = [
        _candidate_summary(spec, group_name, candidate)
        for group_name in spec["groups"]
    ]
    details = {}
    seed_values = {int(seed): [] for seed in spec["screen_seeds"]}
    for row in group_rows:
        group_details = json.loads(row["datasets_json"])
        overlap = set(details).intersection(group_details)
        if overlap:
            raise RuntimeError(f"datasets repeated across groups: {sorted(overlap)}")
        details.update(group_details)
        group_seed = json.loads(row["seed_gains_json"])
        count = int(row["dataset_count"])
        for seed in seed_values:
            seed_values[seed].extend([float(group_seed[str(seed)])] * count)
    if set(details) != set(spec["datasets"]):
        raise RuntimeError("global Task3 candidate does not cover every dataset")

    def macro(source: str, metric: str) -> float:
        return float(np.mean([
            value[source][metric] for value in details.values()
        ]))

    fmt_f1 = macro("fmt", "f1")
    raw_f1 = macro("raw_pca", "f1")
    strong_f1 = macro("strong_raw", "f1")
    fmt_ap = macro("fmt", "average_precision")
    raw_ap = macro("raw_pca", "average_precision")
    strong_ap = macro("strong_raw", "average_precision")
    seed_macro = {
        str(seed): float(np.mean(values))
        for seed, values in seed_values.items()
    }
    dataset_f1_gains = [
        float(value["gains"]["f1_vs_raw_pca"])
        for value in details.values()
    ]
    dataset_ap_gains = [
        float(value["gains"]["average_precision_vs_raw_pca"])
        for value in details.values()
    ]
    return {
        "scope": "global",
        ...
        "strong_raw_f1_macro": strong_f1,
        "fmt_minus_raw_pca_f1_macro": fmt_f1 - raw_f1,
        "fmt_minus_strong_raw_f1_macro": fmt_f1 - strong_f1,
        "fmt_ap_macro": fmt_ap,
        "raw_pca_ap_macro": raw_ap,
        "strong_raw_ap_macro": strong_ap,
        "fmt_minus_raw_pca_ap_macro": fmt_ap - raw_ap,
        "fmt_minus_strong_raw_ap_macro": fmt_ap - strong_ap,
        "positive_f1_dataset_count": sum(
            value > 0.0 for value in dataset_f1_gains
        ),
        "positive_ap_dataset_count": sum(
            value > 0.0 for value in dataset_ap_gains
        ),
        "worst_dataset_f1_gain": min(dataset_f1_gains),
        "worst_dataset_ap_gain": min(dataset_ap_gains),
        "worst_seed_f1_gain": min(seed_macro.values()),
        "all_seed_f1_gains_positive": min(seed_macro.values()) > 0.0,
        "seed_gains_json": json.dumps(seed_macro, sort_keys=True),
        "datasets_json": json.dumps(details, sort_keys=True),
    }


def _global_selection_key(spec: dict, row: dict) -> tuple[float, ...]:
    primary = str(
        spec.get("selection", {}).get("global_primary", "absolute_fmt_f1")
    )
    if primary == "absolute_fmt_f1":
        return (
            float(row["fmt_f1_macro"]),
            float(row["fmt_minus_raw_pca_f1_macro"]),
    ...
    raise ValueError(
        "Task3 selection.global_primary must be absolute_fmt_f1 or f1_gain"
    )
...
def select(config_path: str) -> Path:
    spec = _load_spec(config_path)
    scope = str(spec.get("selection", {}).get("scope", "family"))
    if scope not in {"family", "global"}:
        raise ValueError("Task3 selection.scope must be 'family' or 'global'")
    if scope == "global":
        rows = [
            _global_candidate(spec, candidate)
            for candidate in spec["candidates"]
        ]
        minimum_gain = float(
            spec.get("selection", {}).get("minimum_macro_f1_gain", 0.0)
        )
        eligible = [
            row for row in rows
            if float(row["fmt_minus_raw_pca_f1_macro"]) >= minimum_gain
        ]
        if not eligible:
            raise RuntimeError(
                "no global Task3 FMT recipe passed the macro F1 gain guard"
            )
        ranked = sorted(
            eligible, key=lambda row: _global_selection_key(spec, row),
            reverse=True,
        )
        ineligible = sorted(
            [row for row in rows if row not in eligible],
            key=lambda row: _global_selection_key(spec, row), reverse=True,
        )
        ordered = [*ranked, *ineligible]
        for rank, row in enumerate(ordered, 1):
            row["rank_global"] = rank
            row["gain_guard_passed"] = bool(row in eligible)
        output = Path(spec["output_root"])
        _write_csv(output / "global_leaderboard.csv", ordered)
        winner = ranked[0]
        top_k = int(spec.get("selection", {}).get("top_k", 3))
        payload = {
            "experiment": spec["experiment"],
            "selection_scope": "one unchanged FMT recipe for all datasets",
            "selection_rule": (
                "among recipes with non-negative development dataset-macro "
                "F1 gain over same-width Raw-PCA, maximize absolute FMT F1; "
                "tie-break by F1 gain, absolute AP, worst-dataset gain, and "
                "worst-seed gain"
                if str(spec.get("selection", {}).get(
                    "global_primary", "absolute_fmt_f1"
                )) == "absolute_fmt_f1" else
                "maximize development dataset-macro F1 gain over same-width "
                "Raw-PCA; tie-break by absolute FMT F1 and AP gain"
            ),
        ...
        target = output / "global_selection.json"
