"""Contracts for residual-head Adam beta2 portfolio 106.1."""
...
import Audit_Task3_ResidualHeadBeta2RefreshPortfolio_106_1 as audit_module
...
from Select_Task3_ResidualHeadBeta2RefreshPortfolio_106_1 import (
    _canonical_sha256,
...
SOURCE_104_CONFIG = Path(
    "config/Verify_Task3_ResidualHeadWeightDecayRefreshPortfolio_104.1.yaml"
)
SOURCE_105_CONFIG = Path(
    "config/Verify_Task3_ResidualHeadBeta2Refresh_105.1.yaml"
)


class ResidualHeadBeta2RefreshPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "residual_head_beta2_refresh"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.244
        )
        self.assertEqual(spec["selection"]["target_absolute_fmt_f1"], 0.905)
        self.assertFalse(spec["confirmation_opened"])

    def test_source_config_hashes_are_frozen(self):
        config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
        self.assertEqual(
            _canonical_sha256(SOURCE_104_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_105_CONFIG),
            config["sources"]["residual_head_beta2_refresh"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_and_cleanup_contract(self):
        ...
        self.assertNotIn(
            "Select_Task3_ResidualHeadBeta2RefreshPortfolio_106_1", source
        )
        self.assertNotIn(
        ...
        evidence = Path(
            "ibex_bash/verify_task3_residual_head_beta2_refresh_portfolio_106.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_residual_head_beta2_refresh_105.1_cleanup.sh"
        ).read_text(encoding="utf-8")
