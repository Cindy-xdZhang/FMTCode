...
def provenance(config):
    return {'commit': git_commit(), 'config_sha256': file_sha256(config), 'timestamp': stamp(),
            ...
            'source_sha256': {p:file_sha256(p) for p in (
                'experiments/FTLE_Upsampling_2D_1_2.py', 'experiments/Audit_FTLE_Upsampling_2D_1_2.py',
                ...
                'FMT_Utils/FTLE_Temporal_Audit_2D.py',
                'experiments/FTLE_Upsampling_2D_1_1.py')}}


class Model(nn.Module):
    def __init__(self, spec, method, scale, width=0, dropout=0.):
        super().__init__()
    ...
    def forward(self, low, geometry=None):
        ...
        if self.method == 'espcn':
            x = self.dropout(self.net.act(self.net.conv1(value)))
            x = self.dropout(self.net.act(self.net.conv2(x)))
            pred = self.net.shuffle(self.net.conv3(x))
        else:
            x1 = self.dropout(self.net.inc(value))
            x2 = self.dropout(self.net.down1(x1))
            x3 = self.dropout(self.net.down2(x2))
            x = self.dropout(self.net.up1(x3, x2))
            x = self.dropout(self.net.up2(x, x1))
            pred = self.net.head(x)
...
def prepare(spec, config, index, device):
    ...
    for split, times in splits.items():
        for slice_index, t0 in enumerate(times):
            ...
            # One common physical evaluation region for every method AND scale.
            common_mask = np.logical_and.reduce([evaluation_mask(valid_high, valid_high[::s,::s], s) for s in spec['scales']])
    ...
    dump(root / 'manifest.json', {'version': spec['version'], 'provenance': provenance(config),
                                 'source': meta, 'time_splits': splits, 'temporal_certificate': certificate, 'records': records})
...
def run_root(spec, flow, scale, seed, method, dropout):
    return Path(spec['output'])/'runs'/f'{flow}_x{scale}_s{seed}'/f'{method}_p{round(dropout*100):03d}'
...
def train(spec, config, index, device):
    ...
    certificate = certify(manifest['source'], manifest['time_splits'], spec['integration']['tau'])
    if certificate != manifest['temporal_certificate']:
        raise ValueError('Temporal certificate changed')
    ...
    for dropout, method in itertools.product(spec['dropout_rates'], spec['methods']):
        ...
        model = Model(spec, method, scale, width, dropout).to(device)
        cfg = spec['training']
        ...
        for epoch in range(cfg['epochs']):
            ...
            if epoch % 10 == 0:
                print(json.dumps({'flow':flow,'scale':scale,'seed':seed,'method':method,'dropout':dropout,'epoch':epoch,'validation_mse':score}), flush=True)
            if epoch-best_epoch >= cfg['patience']:
                break
        model.load_state_dict(state)
        # Immutable selection evidence must exist before any test-data read.
        selection = {'locked_at':stamp(), 'best_epoch':best_epoch, 'best_validation_mse':best,
                     'flow':flow,'scale':scale,'seed':seed,'method':method,'dropout':dropout,
                     'normalization':stats,'config_sha256':file_sha256(config),
                     'data_manifest_sha256':file_sha256(manifest_path),
                     'train_files':[d['file'] for d in train_raw],
                     'validation_files':[d['file'] for d in val_raw],
                     'history_sha256':file_sha256(root/'history.json')}
        dump(root/'selection.json', selection)
        test_access = {'first_read_at':stamp(),'selection_sha256':file_sha256(root/'selection.json'),
                       'data_manifest_sha256':file_sha256(manifest_path)}
        # No optimizer, scheduler, or model selection runs below this point.
        ...
        result = {'version':spec['version'],'flow':flow,'scale':scale,'seed':seed,'method':method,'dropout':dropout,'test_access':test_access,
...
def summarize(spec, config):
    ...
    for flow, scale, seed in matrix(spec):
        for dropout,method in itertools.product(spec['dropout_rates'],spec['methods']):
            result = read(run_root(spec, flow, scale, seed, method, dropout)/'result.json')
            for row in result['metrics']:
                rows.append({**{k:result[k] for k in ('flow','scale','seed','method','dropout','parameters')}, **row})
        ...
        for split in ('validation','test'):
            for d in load_data(spec,flow,scale,split):
                for method,order in [('bilinear',1),('bicubic',3)]:
                    rows.append({'flow':flow,'scale':scale,'seed':-1,'method':method,'dropout':-1.,'parameters':0,
    ...
    aggregate = []
    variants = list(itertools.product(spec['methods'],spec['dropout_rates'])) + [('bilinear',-1.),('bicubic',-1.)]
    for flow,scale,(method,dropout) in itertools.product([d['name'] for d in spec['flows']],spec['scales'],variants):
        group = [r for r in rows if r['flow']==flow and r['scale']==scale and r['method']==method and r['dropout']==dropout and r['split']=='test']
        item = {'flow':flow,'scale':scale,'method':method,'dropout':dropout,'seeds':len(set(r['seed'] for r in group))}
...
def submit(spec, config):
    ...
    def dispatch(phase, count, dependency=None, gpu=False):
        cmd = ['sbatch','--parsable','--cpus-per-task=4','--mem=48G','--time=02:00:00',
               '--job-name=ftle12-'+phase,'--output='+str(root/'slurm'/(phase+'-%A_%a.log'))]
        ...
        cmd += ['ibex_bash/ftle_upsampling_2d_1p2.sh',phase,config]
        submitted = stamp()
        ...
        with open('docs/ibex_run_registry.md','a',encoding='utf-8') as stream:
            stream.write('\n\nFTLE 2D 1.2 submission: `'+json.dumps(record)+'`\n')
...
def main():
    ...
    parser.add_argument('--config',default='config/Other_FTLEUpsampling2D_1.2.json')
    ...
    if args.phase=='runtime': event(spec,args.config,args.runtime_phase,args.state,args.exit_code)
        ...
        from experiments.Audit_FTLE_Upsampling_2D_1_2 import audit_data,audit_results,preflight
