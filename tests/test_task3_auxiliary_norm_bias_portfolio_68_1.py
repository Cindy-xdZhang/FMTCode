"""Contracts for preregistered Task3 auxiliary norm-bias portfolio 68.1."""
...
import Audit_Task3_AuxiliaryNormBiasPortfolio_68_1 as audit_module
...
from Select_Task3_AuxiliaryNormBiasPortfolio_68_1 import (
    _canonical_sha256,
...
CONFIG = Path("config/Verify_Task3_AuxiliaryNormBiasPortfolio_68.1.yaml")
SOURCE_66_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryNormScalePortfolio_66.1.yaml"
)
SOURCE_67_CONFIG = Path("config/Verify_Task3_AuxiliaryNormBias_67.1.yaml")


class AuxiliaryNormBiasPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]), {"current_portfolio", "auxiliary_norm_bias"}
        )
        ...
        self.assertEqual(spec["selection"]["target_dataset_macro_f1_gain"], 0.212)
    ...
    def test_source_config_hashes_are_frozen(self):
        ...
        self.assertEqual(
            _canonical_sha256(SOURCE_66_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_67_CONFIG),
            config["sources"]["auxiliary_norm_bias"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_is_independent_of_68_selector(self):
        source = inspect.getsource(audit_module)
        self.assertNotIn("Select_Task3_AuxiliaryNormBiasPortfolio_68_1", source)
    ...
    def test_evidence_and_cleanup_counts(self):
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_norm_bias_portfolio_68.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_norm_bias_67.1_cleanup.sh"
        ).read_text(encoding="utf-8")
