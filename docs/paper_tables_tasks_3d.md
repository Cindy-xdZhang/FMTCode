...
## Task4-c 4.14：物理积分长度与局部空间块，已提交

科学commit`5c9c9042`，完整重建27,000训练/3,000验证/10,000测试。Channel单向目标长度0.08/0.1/0.12，TBL4/5/6，物理ds均在用户规定范围；相同9种邻距/积分组合用于三个集合。头区内部原生单元分块，评价中心距任意训练中心至少1h、距同头区训练中心不超过4h；共享头区/实例/源场，不是原空间留出评测。两原模型固定正则化配置各三种子，真实小预检和工程训练已通过，正式Ibex作业已提交，尚无科学F1。见[4.14协议](Task4C_physical_length_protocol_4.14.md)。
