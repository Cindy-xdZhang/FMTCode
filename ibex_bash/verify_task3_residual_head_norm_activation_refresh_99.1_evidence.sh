...
set -euo pipefail
cd "${TASK399_REPO_ROOT:-/home/zhanx0o/FMT_Task3_ResidualHeadNormActivationRefresh_99_1}"
mkdir -p slurm_logs
source /home/zhanx0o/anaconda3/etc/profile.d/conda.sh
conda activate deepvortex

output_root="$PWD/outputs/Verify_Task3_ResidualHeadNormActivationRefresh_99.1"
...
if [[ "$per_run_count" -ne 600 ]]; then
  echo "expected 600 per-run CSV files, found $per_run_count" >&2
  exit 1
fi
...
if [[ "$checkpoint_count_before" -ne 600 ]]; then
  echo "expected 600 temporary checkpoints, found $checkpoint_count_before" >&2
  exit 1
fi
...
if [[ "$archived_per_run_count" -ne 600 ]]; then
