...
repo_root="${TASK375_REPO_ROOT:-/home/zhanx0o/FMT_Task3_AuxiliaryPostNorm_75_1}"
portfolio_root="${TASK376_REPO_ROOT:-/home/zhanx0o/FMT_Task3_AuxiliaryPostNormPortfolio_76_1}"
cd "$repo_root"
mkdir -p slurm_logs

output_root="$repo_root/outputs/Verify_Task3_AuxiliaryPostNorm_75.1"
...
portfolio="$portfolio_root/outputs/Verify_Task3_AuxiliaryPostNormPortfolio_76.1/portfolio_selection.json"
portfolio_audit="$portfolio_root/outputs/Verify_Task3_AuxiliaryPostNormPortfolio_76.1/independent_audit.json"
...
expected_output_root="$resolved_outputs/Verify_Task3_AuxiliaryPostNorm_75.1"
...
if [[ "$resolved_output_root" != "$expected_output_root" ]] || \
  ...
  echo "refusing cleanup outside the exact resolved 75.1 output root: $resolved_candidates" >&2
  exit 1
fi
...
if evidence.get("status") != "passed" or int(
    ...
    raise SystemExit("75.1 evidence archive is incomplete")
if hashlib.sha256(archive_path.read_bytes()).hexdigest() != str(
    evidence.get("stable_archive_sha256", "")
):
    raise SystemExit("75.1 archive SHA-256 differs")
if source_audit.get("status") != "passed":
    raise SystemExit("75.1 independent audit did not pass")
if audit.get("status") != "passed" or not audit.get("all_frozen_hashes_verified"):
    raise SystemExit("76.1 independent portfolio audit did not pass")
if int(portfolio.get("frozen_model_count", -1)) != 40 or int(
    portfolio.get("frozen_artifact_file_count", -1)
) != 80:
    raise SystemExit("76.1 frozen portfolio is incomplete")
PY

checkpoint_count_before="$(find "$resolved_candidates" -type f \
  \( -name '*.pt' -o -name '*.pth' -o -name '*.ckpt' \) | wc -l)"
if [[ "$checkpoint_count_before" -ne 240 ]]; then
  echo "expected exactly 240 temporary 75.1 checkpoints, found $checkpoint_count_before" >&2
  exit 1
fi
...
if [[ "$checkpoint_count_after" -ne 0 ]]; then
  echo "75.1 checkpoint cleanup incomplete: $checkpoint_count_after remain" >&2
  exit 1
fi
...
Path(sys.argv[1]).write_text(json.dumps({
    "experiment": "Verify_Task3_AuxiliaryPostNorm_75.1",
    "job_id": os.environ.get("SLURM_JOB_ID", ""),
    "checkpoint_count_before": int(sys.argv[2]),
    "checkpoint_count_after": int(sys.argv[3]),
    "reason": "76.1 frozen portfolio and independent audit complete",
    "status": "passed",
