...
output_root="$PWD/outputs/Verify_Task3_AuxiliaryPostNorm_75.1"
...
if [[ "$per_run_count" -ne 240 ]]; then
  echo "expected 240 per-run CSV files, found $per_run_count" >&2
  exit 1
fi
...
if [[ "$checkpoint_count_before" -ne 240 ]]; then
  echo "expected 240 temporary checkpoints, found $checkpoint_count_before" >&2
  exit 1
fi
...
if [[ "$archived_per_run_count" -ne 240 ]]; then
