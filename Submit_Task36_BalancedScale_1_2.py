...
def submit(mode,config):
    ...
    from experiments.Task36_BalancedScale_1_2 import search_plan,final_plan
    ...
    stages=[('preflight',4,16,'00:30:00',None,False),
        ...
        ('search',4,32,'02:00:00',f'0-{len(search_plan(spec))-1}%18',True),
        ('select',2,4,'00:10:00',None,False),
        ('prepare_test',4,32,'02:00:00',f'0-{nd-1}%9',False),
        ('final',4,32,'02:00:00',f'0-{len(final_plan(spec))-1}%18',True),
        ('audit',4,24,'02:00:00',f'0-{nd*len(spec["train_sizes"])-1}%9',False),
        ('merge',2,4,'00:10:00',None,False)]
    commit=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip()
    previous=None
    for phase,cpu,mem,limit,array,gpu in stages:
        command=['sbatch','--parsable',f'--job-name=t36-balanced12-{phase}','--nodes=1','--ntasks=1',
        ...
        command += ['ibex_bash/task36_balanced_1p2.sh',phase,config]
...
if __name__=='__main__':
    ...
    parser.add_argument('mode',choices=['all','runtime'])
    parser.add_argument('--config',default='config/Verify_Task36_BalancedScale_1.2.json')
