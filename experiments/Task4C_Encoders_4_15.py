"""Pure FMT variants versus Conv3D on the frozen Task4-c 4.14 samples."""
...
from FMT_Utils.Task4C_Encoders_4_15 import ENCODERS, encode_lines, make_model, local_indices

DEFAULT_CONFIG = 'config/Verify_Task4C_Encoders_4.15.json'
...
def load_spec(config):
    ...
    if len(spec['encoders']) != 3 or spec['encoders'][0] != 'fmt_v5' or spec['encoders'][-1] != 'conv3d':
        raise ValueError('Expected the two specified encoders and Conv3D')
    if spec['encoders'][1] not in ('objective_fmt_nTDO_v2', 'fmt_objective_ntod_v2'):
        raise ValueError('Unknown objective encoder')
...
def check_parent(spec, full=False):
    ...
    for flow in spec['flows']:
        ...
        for split in SPLITS:
            ...
            paths += [(parent / key / name, encoded['splits'][key]['files'][name]) for name in ('metadata.npz', 'voxels.npy')]
            if full:
...
def encode(spec, config):
    ...
    for encoder in spec['encoders']:
        ...
        manifests[encoder] = dict(version=spec['version'], encoder=encoder, identity=identity(config),
            ...
            line_width=None if encoder == 'conv3d' else ENCODERS[encoder][1],
            extra_direction_spectrum=False, reused_4_14_samples=True)
    ...
    for flow in spec['flows']:
        for split in SPLITS:
            ...
            for encoder in spec['encoders']:
                ...
                if encoder == 'conv3d':
                    (dest / 'voxels.npy').symlink_to((parent / key / 'voxels.npy').resolve())
                else:
                    files[encoder] = np.lib.format.open_memmap(dest / 'fmt.npy', mode='w+', dtype='float32',
                        shape=(n, 27, ENCODERS[encoder][1] + 1))
            for start in range(0, n, 32):
                ...
                for encoder, target in files.items():
                    target[sl] = encode_lines(geometry, ss, cc, encoder).cpu().numpy()
            for target in files.values():
                target.flush()
            for encoder in spec['encoders']:
                dest = Path(variant_spec(spec, encoder)['output']) / key
                name = 'voxels.npy' if encoder == 'conv3d' else 'fmt.npy'
                h = old['splits'][key]['files']['voxels.npy'] if encoder == 'conv3d' else pipeline.sha(dest / name)
...
def train(spec, config, index):
    ...
    result.update(encoder=encoder, encoder_version=None if encoder == 'conv3d' else ENCODERS[encoder][2],
        feature_blocks=['voxels'] if encoder == 'conv3d' else [encoder],
        test_scope='frozen_4.14_known_heads_local_spatial_blocks_already_used_benchmark',
        parent_commit=spec['parent_commit'], raw_coordinate_or_direction_bypass=False,
...
def merge(spec, config):
    ...
    result = dict(version=spec['version'], identity=identity(config), rows=rows, fixed_threshold=.5,
        historical_4_14_reference=parent['rows'], same_4_14_samples=True, no_test_selection=True,
...
def runtime(spec, config, phase, state, code=None):
    ...
    append_record(root / 'runtime_events.jsonl', row, 'Task4C 4.15 runtime')
    append_record('docs/ibex_run_registry.md', row, 'Task4C 4.15 runtime')
...
def submit(spec, config, phase, dependency=None):
    ...
    command = ['sbatch', '--parsable', '--cpus-per-task=4', '--mem=64G',
        ...
        '--job-name=t4c415-' + phase, '--output=' + str(logs / (phase + '.%A_%a.out')),
    ...
    command += ['ibex_bash/task4c_encoders_4p15.sh', phase, config]
    ...
    append_record(root / 'submissions.jsonl', row, 'Task4C 4.15 submitted')
    append_record('docs/ibex_run_registry.md', row, 'Task4C 4.15 submitted')
    print(json.dumps(row), flush=True)
...
def main():
    ...
    if a.phase == 'train': train(spec, a.config, a.index)
        ...
        from experiments.Audit_Task4C_Encoders_4_15 import preflight, audit
