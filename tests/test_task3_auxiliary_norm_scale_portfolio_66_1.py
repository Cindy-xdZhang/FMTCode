"""Contracts for preregistered Task3 auxiliary norm-scale portfolio 66.1."""
...
import Audit_Task3_AuxiliaryNormScalePortfolio_66_1 as audit_module
...
from Select_Task3_AuxiliaryNormScalePortfolio_66_1 import (
    _canonical_sha256,
...
CONFIG = Path("config/Verify_Task3_AuxiliaryNormScalePortfolio_66.1.yaml")
SOURCE_64_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryEpsilonPortfolio_64.1.yaml"
)
SOURCE_65_CONFIG = Path("config/Verify_Task3_AuxiliaryNormScale_65.1.yaml")


class AuxiliaryNormScalePortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]), {"current_portfolio", "auxiliary_norm_scale"}
        )
        ...
        self.assertEqual(spec["selection"]["target_dataset_macro_f1_gain"], 0.211)
    ...
    def test_source_config_hashes_are_frozen(self):
        ...
        self.assertEqual(
            _canonical_sha256(SOURCE_64_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_65_CONFIG),
            config["sources"]["auxiliary_norm_scale"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_is_independent_of_66_selector(self):
        source = inspect.getsource(audit_module)
        self.assertNotIn("Select_Task3_AuxiliaryNormScalePortfolio_66_1", source)
    ...
    def test_evidence_and_cleanup_counts(self):
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_norm_scale_portfolio_66.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_norm_scale_65.1_cleanup.sh"
        ).read_text(encoding="utf-8")
