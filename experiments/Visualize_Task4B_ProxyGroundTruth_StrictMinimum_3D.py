"""Version 4.3: strict IVD > actual hairpin minimum; ground truth only."""
import argparse
import hashlib
import json
import os
...
def render_volume(labels,low,high,spacing,path,name,threshold,fraction,spec):
    ...
    text(renderer,f'{name.upper()}  |  Proxy ground truth: IVD > a',.027,.951,30)
    for k in range(4):
...
def main(config, render_only=False):
    ...
    assert spec['threshold_multiplier']==1.0
    assert spec['a_policy']=='actual_minimum_original_gt_cells_and_gt_voxel_centers'
    ...
    for name in ['channel','tbl']:
        ...
        previous_a=original['flows'][name]['thresholds']['a']
        assert np.isfinite(gt_ivd).all() and np.isfinite(ivd[ids>0]).all()
        a=min(float(gt_ivd.min()),float(ivd[ids>0].min()))
        threshold=a
        previous_threshold=.98*previous_a
        ...
        # Strict comparison intentionally excludes the minimum-valued samples.
        assert np.all(ivd[ids>0]>=a) and np.all(gt_ivd>=a)
        ...
        np.savez_compressed(out/f'{name}_groundtruth.npz',labels=labels,vortex_ids=ids,domain_min_xyz=low,
            domain_max_xyz=high,resolution_xyz=resolution,a=a,threshold=threshold)
        report=dict(a=a,a_policy=spec['a_policy'],previous_a=previous_a,
            ...
            excluded_grid_hairpin_at_minimum=int(np.sum((ids>0)&(ivd==a))),
            excluded_original_hairpin_cells_at_minimum=int(np.sum(gt_ivd==a)),
        ...
        report['hairpins']=render(points,hairpin_labels,spacing,points.min(0)-spacing,points.max(0)+spacing,
            out/f'{name}_groundtruth_hairpins_3d.png',f'{name.upper()} | Proxy GT hairpins: IVD > a',
...
if __name__=='__main__':
    ...
    p.add_argument('--config',default='config/Other_Task4B_ProxyGTThreshold_4.3.json')
