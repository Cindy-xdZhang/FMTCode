...
- **step1 只筛中心**（λ₂<阈值 ∧ oyf>0；头区样本还要同头区，GT 头部样本还要在实例内且满足头部夹角）。26 个邻居只要求在计算域内。
...
- **实例覆盖与划分。** “覆盖”＝某划分在该 hairpin 实例头部区域（GT 体积内 ∧ |cos(速度,涡量)|²<0.5）有 ≥2 个采样点，构建目标每实例 10 个。实例按流场固定种子分 80% 训练覆盖组 / 20% 未见组（重叠包围盒同组）；训练覆盖每个覆盖组实例，测试覆盖全部实例；未见组实例不进训练/验证（GT 包围盒外扩一格为排除区）；负类头区跟随最近实例分组。
- **距离与顺序。** 评价点（验证/测试）距任意训练中心 ≥1h；覆盖组实例的正类测试点距同实例训练中心 ≤6h（“轻微偏移”，由 1h–6h 环带配对的训练中心保证）；测试距验证 ≥0.5h；(中心,尺度) 同划分唯一。生成顺序 test→validation→train。
...
- **状态。** 实现版本 `mainExp_Task4C_FixedDataset_1.1`：r1/r2 均在 pilot 或 build 阶段失败（见实现记录），r3 已按用户晚间规则（1h、100 次、下限 2/目标 10）改好代码，待用户确认规范文本后部署；冻结哈希建成后填入本节。
