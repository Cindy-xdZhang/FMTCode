...
output_root="$PWD/outputs/Verify_Task3_SafeFactorCombination_44.1"
...
if [[ "$per_run_count" -ne 1680 ]]; then
  echo "expected 1680 per-run CSV files, found $per_run_count" >&2
  exit 1
fi

checkpoint_count_before="$(find "$candidate_root" -type f \
  \( -name '*.pt' -o -name '*.pth' -o -name '*.ckpt' \) | wc -l)"
if [[ "$checkpoint_count_before" -ne 1680 ]]; then
  echo "expected 1680 temporary checkpoints, found $checkpoint_count_before" >&2
  exit 1
fi
...
if [[ "$archived_per_run_count" -ne 1680 ]]; then
