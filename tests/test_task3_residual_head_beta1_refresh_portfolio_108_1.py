"""Contracts for residual-head Adam beta1 portfolio 108.1."""
...
import Audit_Task3_ResidualHeadBeta1RefreshPortfolio_108_1 as audit_module
...
from Select_Task3_ResidualHeadBeta1RefreshPortfolio_108_1 import (
    _canonical_sha256,
...
SOURCE_106_CONFIG = Path(
    "config/Verify_Task3_ResidualHeadBeta2RefreshPortfolio_106.1.yaml"
)
SOURCE_107_CONFIG = Path(
    "config/Verify_Task3_ResidualHeadBeta1Refresh_107.1.yaml"
)


class ResidualHeadBeta1RefreshPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "residual_head_beta1_refresh"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.246
        )
        self.assertEqual(spec["selection"]["target_absolute_fmt_f1"], 0.906)
        self.assertFalse(spec["confirmation_opened"])

    def test_source_config_hashes_are_frozen(self):
        config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
        self.assertEqual(
            _canonical_sha256(SOURCE_106_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_107_CONFIG),
            config["sources"]["residual_head_beta1_refresh"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_and_cleanup_contract(self):
        ...
        self.assertNotIn(
            "Select_Task3_ResidualHeadBeta1RefreshPortfolio_108_1", source
        )
        self.assertNotIn(
        ...
        evidence = Path(
            "ibex_bash/verify_task3_residual_head_beta1_refresh_portfolio_108.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_residual_head_beta1_refresh_107.1_cleanup.sh"
        ).read_text(encoding="utf-8")
