...
def main():
    config=Path('config/Verify_FMTAllV2_1.2.json')
    ...
    _,_,backbone, _=sources(spec,'Task5','channel','development')
    raw,_=_load_raw_model(backbone/'channel_raw_seed40.pt',338,torch.device('cpu'))
    model=PathlineFMTResidualClassifier3D(raw,fmt_dim=338,**residual_model_kwargs(spec['model']))
    count=sum(p.numel() for p in model.parameters())
    print('Task5 matched width338 parameter_count',count,flush=True)
    assert count<148225
    checks=[]
    for task in ('Task1','Task5'):
        ...
        for dataset in spec['datasets']:
            ...
            new=features(rows,spec[task.lower()]['new_feature'],'cpu')
            assert old.shape[1]==(189 if task=='Task1' else 268)
            assert new.shape==(old.shape[0],259 if task=='Task1' else 338)
