"""Task4-c 4.3: fixed physical data, learned per-line pooling and Conv3D capacity controls."""
...
DEFAULT_CONFIG = "config/mainExp_Task4C_LinePooling_4.3.json"
...
def identity(config):
    ...
    for p in ("experiments/Task4C_Multiscale_4_1.py", "FMT_Utils/Task4C_Multiscale_4_1.py",
              "FMT_Utils/Task4C_Regularized_4_2.py",
...
def encode_partition(spec, config, split, report):
    ...
    for flow in spec["flows"]:
        key = flow["name"]+"/"+split
        folder, source = out/key, parent/key
        ...
        for name in ("geometry.npy", "seeds.npy", "metadata.npz"):
            if sha(source/name) != preparation["splits"][split]["files"][name]:
        ...
        (folder/"metadata.npz").write_bytes((source/"metadata.npz").read_bytes())
        with np.load(source/"metadata.npz") as original:
            metadata = {k:original[k] for k in original.files}
        n = len(geometry)
        fmt = np.lib.format.open_memmap(folder/"fmt.npy", mode="w+", dtype="float32", shape=(n,27,234))
        voxels = np.lib.format.open_memmap(folder/"voxels.npy", mode="w+", dtype="float16", shape=(n,4,24,24,24))
        for offset in range(0,n,32):
            ...
            counts = torch.as_tensor(metadata["counts"][sl].astype(np.int64),device=device)
            tokens = line_fmt(g,se,counts)
            if offset == 0:
                from FMT_Utils.Task4C_LinePooling_4_3 import fixed_statistics
                from FMT_Utils.Task4C_Regularized_4_2 import extended_fmt
                recovered, reference = fixed_statistics(tokens), extended_fmt(g,se,counts)
                if not torch.allclose(recovered, reference, atol=1e-5, rtol=1e-6):
                    raise ValueError("Per-line features do not recover frozen 4.2 statistics")
            fmt[sl] = tokens.cpu().numpy()
            voxels[sl] = bundle_voxels(g,counts).cpu().numpy().astype(np.float16)
            if offset%3200 == 0: print("Encoded",key,offset,n,flush=True)
        fmt.flush(); voxels.flush()
        report["splits"][key] = {"samples":n,"class_counts":np.bincount(metadata["labels"],minlength=2).tolist(),
...
def load_data(spec, split, method, manifest, candidate):
    ...
    if method == "fmt_mlp" and candidate["architecture"] == "reference_4.2":
        import torch
        from FMT_Utils.Task4C_LinePooling_4_3 import fixed_statistics
        values = data["features"]
        data["features"] = np.concatenate([fixed_statistics(torch.from_numpy(values[i:i+256])).numpy()
...
def train(spec, config, index, phase):
    ...
    from FMT_Utils.Task4C_LinePooling_4_3 import make_model, classifier_loss, classifier_probability
    ...
    def standardize(data, fit=False):
        ...
        if method != "fmt_mlp":
            return
        tokens = data["features"].ndim == 3
        mask = data["features"][..., -1:] if tokens else None
        x = transform_fmt(data["features"][..., :-1] if tokens else data["features"], candidate["signed_log_fmt"])
        if fit:
            valid = x[mask[...,0]>.5] if tokens else x
        ...
        data["features"] = np.concatenate((x*mask,mask),-1) if tokens else x
...
def submit_job(spec, config, phase, array=None, previous=None):
    ...
    command = ["sbatch", "--parsable", "--nodes=1", "--ntasks=1", "--cpus-per-task=4",
               ...
               f"--job-name=t4c43-{phase}", f"--output={out}/logs/{phase}_%A_%a.out", f"--error={out}/logs/{phase}_%A_%a.err"]
    if previous:
    ...
    command += ["ibex_bash/task4c_line_pooling_4p3.sh", phase, config]
    ...
    append_locked("docs/ibex_run_registry.md", "\n- Task4-c 4.3 SUBMITTED "+json.dumps(row)+"\n")
