...
（考证过程的原始版本更密集，本版为可读性重写；2026-08-16。Task3/Task4 任务映射与当前状态于 2026-08-23 修订。）
...
| L6 | Hairpin 涡分割 | 2026-07 起 | 3D 发卡涡的提取与分割 | 移去 `optimal-connection` 仓库（C++） | = Task4 多涡型分类的语义背景 |
...
**留给我们什么**：primitive 的定义和 5 线顺序约定；Vatistas 合成数据管线（`experiments/FittingVatistasParam.py` + `experiments/VatistasFlowDatasetGenerator.py`，已复制，是 Task1 定量评测的标签来源）；一个可对照的"有参数"编码器。
...
已复制到本仓库：`experiments/FittingVatistasParam.py`、`experiments/VatistasFlowDatasetGenerator.py`、`docs/from_pyflowvis/vatistas_profile.md`。
...
**留给我们什么**：`experiments/FMT_Clustering.py`（Task1 入口，已修复导入并清理）+ `config/PathlineFMTclustering.yaml`。这条管线冻结后复跑、补上数字，就是 mainExp_1.1。

关键文件（已复制）：`experiments/FMT_Clustering.py`、`config/PathlineFMTclustering.yaml`、`FMT_Utils/FlowlinePostProcessing.py`（降采样与局部化）、`pnn/libs/flows.py`（画图）。
...
**L6 · Hairpin 涡分割**：2026-07-15 起在另一个仓库 `optimal-connection`（C++）做 3D 发卡涡。PyflowVis 里留下一份规划文档 `small-label-morphology-naming.md`（已复制到 `docs/from_pyflowvis/`）。它只属于 Task4 的语义背景：小标签预算下可比较冻结特征+简单分类头与端到端微调；streamwise、spanwise、hairpin 等多涡型分类只存在于 3D。它不再属于 Task3。

---

## 4. 四个任务现在各自站在哪（2026-08-23 修订）

| 任务 | 现状 | 下一步（见问题分析 P9） |
|---|---|---|
| **Task1（2D/3D）** 无监督聚类分涡 | 3D 旧 161D FMT 在固定时间留出上 7/8 数据条目优于 Raw，F-22 是反例；2D 有旧定性结果 | 建立独立 Task1 主实验，补 F-22、ARI/NMI、validation 映射和客观性验证 |
| **Task2（2D/3D）** FMT 作为 VAE 输入 | 已从零实现；受控 Re640/Re6400 上 `FMT+VAE > Raw+VAE`，但只覆盖一个 flow family | 用冻结受控协议扩展到全部 flow family；结论不得写成“VAE提高FMT” |
| **Task3（2D/3D）** IVD 监督二分类 | 3D `mainExp_Task3Universality_2.2` 在当前 8 条目三-seed method mean 上达到 F1/AP 明显增益 | 做不含 baseline-gain 约束的论文级去偏 confirmation；当前不做2D |
| **Task4（仅3D）** streamwise/spanwise/hairpin 多分类 | 尚未实现；L6 只提供标签和任务语义背景 | 当前不启动，先冻结多分类标签协议 |
...
| commit | 修改 | 原因 |
|---|---|---|
| a3fd97d | `experiments/FMT_Clustering.py` 导入名 `generate_FLowMap_SLICE` → `generate_Flowmap_SLICE`（2 行） | 上游改名导致 Task1 入口 ImportError |
