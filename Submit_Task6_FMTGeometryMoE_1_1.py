...
def runtime(config,phase,state,exit_code=None):
    ...
    device='CPU'
    if phase in ('fit_check','screen','refine','final'):
        import torch
...
def submit(mode,config):
    ...
    if mode=='search':
        ...
        stages=[('preflight',4,16,'00:20:00',None,False),
            ...
            ('fit_check',4,32,'01:00:00',f'0-{nd-1}%9',True),
            ('screen',4,32,'01:30:00',f'0-{len(spec["screen_datasets"])*len(spec["candidates"])-1}%18',True),
            ('promote',2,4,'00:10:00',None,False),
            ('refine',4,32,'04:00:00',f'0-{nd*len(spec["families"])*spec["promote_per_family"]-1}%18',True),
            ('select',2,4,'00:10:00',None,False),
            ('advance',2,4,'00:10:00',None,False)]
    else:
        from experiments.Task6_FMTGeometryMoE_1_1 import frozen_selection
        ...
        stages=[('final',4,32,'04:00:00',f'0-{nd*(len(spec["families"])+1)*len(spec["seeds"])-1}%18',True),
    ...
    for phase,cpu,mem,limit,array,gpu in stages:
        command=['sbatch','--parsable',f'--job-name=t6-moe11-{phase}','--nodes=1','--ntasks=1',
        ...
        command += ['ibex_bash/task6_fmt_geometry_moe_1p1.sh',phase,config]
