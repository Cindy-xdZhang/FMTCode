...
class _ActivationInputScale(nn.Module):
    """Apply one fixed scalar immediately before an existing activation."""

    def __init__(self, activation, scale):
        super().__init__()
        self.activation = activation
        self.scale = float(scale)

    def forward(self, values):
        return self.activation(values * self.scale)


def _scale_auxiliary_projection_activation_inputs(module, scale=1.0):
    """Scale every existing projection activation input without new parameters."""
    scale = float(scale)
    if not math.isfinite(scale) or scale <= 0.0:
        raise ValueError(
            "auxiliary_projection_activation_input_scale must be finite and "
            "positive"
        )
    if scale == 1.0:
        return 0
    ...
    def wrap(parent):
        ...
        for name, child in tuple(parent.named_children()):
            if isinstance(child, activation_types):
                setattr(parent, name, _ActivationInputScale(child, scale))
                wrapped += 1
            else:
                wrapped += wrap(child)
        return wrapped
...
class _ActivationInputShift(nn.Module):
    """Add one fixed scalar immediately before an existing activation."""
    ...
    def forward(self, values):
        return self.activation(values + self.shift)


def _shift_auxiliary_projection_activation_inputs(module, shift=0.0):
    """Shift every existing projection activation input without parameters."""
    shift = float(shift)
    if not math.isfinite(shift):
        raise ValueError(
            "auxiliary_projection_activation_input_shift must be finite"
        )
    if shift == 0.0:
        return 0
    activation_types = (
    ...
    def wrap(parent):
        ...
        for name, child in tuple(parent.named_children()):
            if isinstance(child, activation_types):
                setattr(parent, name, _ActivationInputShift(child, shift))
                wrapped += 1
            else:
                wrapped += wrap(child)
        return wrapped
...
class _ActivationResidualGain(nn.Module):
    """Add one fixed linear bypass to an existing activation response."""
    ...
    def forward(self, values):
        return self.activation(values) + self.gain * values


def _add_auxiliary_projection_activation_residuals(module, gain=0.0):
    """Add a fixed activation residual without parameters or random draws."""
    gain = float(gain)
    if not math.isfinite(gain) or gain < 0.0:
        raise ValueError(
            "auxiliary_projection_activation_residual_gain must be finite "
            "and non-negative"
        )
    if gain == 0.0:
        return 0
    # A residual-mix wrapper is treated as one activation response so this
    # operation is composed after the complete 93.1 transform. Input scale
    # and shift wrappers remain outside and therefore still act first.
    activation_types = (
    ...
    def wrap(parent):
        ...
        for name, child in tuple(parent.named_children()):
            if isinstance(child, activation_types):
                setattr(parent, name, _ActivationResidualGain(child, gain))
                wrapped += 1
            else:
                wrapped += wrap(child)
        return wrapped
...
def residual_model_kwargs(model_spec):
    ...
    return {
        ...
        "auxiliary_projection_activation_input_scale": float(model_spec.get(
            "auxiliary_projection_activation_input_scale", 1.0
        )),
        ...
        "auxiliary_projection_activation_residual_gain": float(model_spec.get(
            "auxiliary_projection_activation_residual_gain", 0.0
        )),
