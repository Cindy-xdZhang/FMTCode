...
def task1_inputs(args):
    ...
    times=t0+indices*dt
    identity={'experiment':'Other_FMTObjectivityTranslation_1.2','dataset':args.dataset,
...
def main():
    ...
    for medium in args.media:
        image=render(paths,labels,ids,audit,args.output_dir,
                     'Cylinder Re160 3D | Mean-flow translating observer' if args.dataset=='cylinder3d' and args.global_mean else f'{args.dataset} | Translation objectivity check',medium)
        print(image)
