"""Contracts for preregistered Task3 activation-input-scale portfolio 90.1."""
...
import Audit_Task3_AuxiliaryActivationInputScalePortfolio_90_1 as audit_module
...
from Select_Task3_AuxiliaryActivationInputScalePortfolio_90_1 import (
    _canonical_sha256,
...
SOURCE_88_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryProjectionActivationPortfolio_88.1.yaml"
)
SOURCE_89_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryActivationInputScale_89.1.yaml"
)


class AuxiliaryActivationInputScalePortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "auxiliary_activation_input_scale"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.228
        )
        self.assertEqual(spec["selection"]["target_absolute_fmt_f1"], 0.897)
        self.assertFalse(spec["confirmation_opened"])

    def test_source_config_hashes_are_frozen(self):
        config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
        self.assertEqual(
            _canonical_sha256(SOURCE_88_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_89_CONFIG),
            config["sources"]["auxiliary_activation_input_scale"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_and_cleanup_contract(self):
        ...
        self.assertNotIn(
            "Select_Task3_AuxiliaryActivationInputScalePortfolio_90_1",
            source,
        )
        self.assertNotIn(
        ...
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_activation_input_scale_portfolio_90.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_activation_input_scale_89.1_cleanup.sh"
        ).read_text(encoding="utf-8")
