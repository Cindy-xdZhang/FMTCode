...
2026-09-14最新推进`Ablation_Task4C_CenterDiversity_4.9`，见`docs/Task4C_center_diversity_protocol_4.9.md`：保持27,000训练视图与原head/scale/标签/实例计数，训练中心4,460→13,875；实际预处理及独立落盘审计通过，固定网络/优化设置做新旧数据三种子验证，原测试不变。4.8 SAM消融已完成，选定验证FMT0.395441、Conv3D0.429179，目标未达成；全部判断及原始证据位置见experiment_log。
