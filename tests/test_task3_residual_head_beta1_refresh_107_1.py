"""Contracts for residual-head Adam beta1 refresh 107.1."""
...
CONFIG = "config/Verify_Task3_ResidualHeadBeta1Refresh_107.1.yaml"
...
class ResidualHeadBeta1RefreshTests(unittest.TestCase):
    def test_registered_grid_targets_and_job_count(self):
        ...
        self.assertEqual(
            ...
            ["training.residual_head_optimizer_beta1"],
        )
        selection = spec["optimization_selection"]
        self.assertEqual(selection["target_dataset_macro_f1_gain"], 0.246)
        self.assertEqual(selection["target_absolute_fmt_f1"], 0.906)
    ...
    def test_exact_source_control_and_registered_grid(self):
        ...
        for index, row in enumerate(spec["optimization_candidates"]):
            ...
            if index == 0:
                ...
                self.assertEqual(
                    set(row["training"]), {"residual_head_optimizer_beta1"}
                )
                observed.append(float(
                    row["training"]["residual_head_optimizer_beta1"]
                ))
        self.assertEqual(observed, EXPECTED)

    def test_source_recipe_is_preserved_except_registered_beta1(self):
        spec = _load_optimization_spec(CONFIG)
        with tempfile.TemporaryDirectory() as directory:
            ...
            source_path.write_text(json.dumps({
                "experiment": (
                    "Verify_Task3_ResidualHeadBeta2RefreshPortfolio_106.1"
                ),
        ...
        for rows in resolved.values():
            self.assertNotIn("residual_head_optimizer_beta1", rows[0]["training"])
            self.assertEqual(
                rows[1]["training"]["residual_head_optimizer_beta1"], 0.0
            )
            self.assertEqual(
                rows[1]["training"]["residual_head_optimizer_beta2"], 0.5
            )
            self.assertEqual(
                rows[1]["training"]["auxiliary_optimizer_beta1"], 0.7
            )
            self.assertEqual(
    ...
    def test_head_beta1_changes_only_downstream_group(self):
        model = _model()
        training = {
            "learning_rate": 0.001,
            "weight_decay": 0.0001,
        ...
        self.assertEqual(optimizer.param_groups[0]["betas"], (0.5, 0.8))
    ...
    def test_validation_evidence_and_record_contract(self):
        self.assertEqual(
            _residual_head_optimizer_betas({
                "optimizer_beta1": 0.8,
                "optimizer_beta2": 0.95,
                "residual_head_optimizer_beta1": 0.0,
            }),
            ((0.0, 0.95), True),
        )
        for invalid in (-1.0, 1.0, float("inf"), float("nan")):
            with self.assertRaisesRegex(
                ValueError, "residual_head_optimizer_beta1"
            ):
                _residual_head_optimizer_betas({
                    "residual_head_optimizer_beta1": invalid
                })
        source = inspect.getsource(_train_one)
        self.assertIn("training_residual_head_optimizer_beta1", source)
        self.assertIn("residual_head_optimizer_betas", source)
        spec = _load_optimization_spec(CONFIG)
        self.assertEqual(
            spec["optimization_selection"]["absolute_fmt_guard"],
            {
                "control_optimization_id": "c00_control_source",
                "f1_tolerance": 0.0,
                "average_precision_tolerance": 0.0,
            },
        )
        gpu = Path(
            "ibex_bash/verify_task3_residual_head_beta1_refresh_107.1_gpu.sh"
        ).read_text(encoding="utf-8")
        evidence = Path(
            "ibex_bash/verify_task3_residual_head_beta1_refresh_107.1_evidence.sh"
        ).read_text(encoding="utf-8")
