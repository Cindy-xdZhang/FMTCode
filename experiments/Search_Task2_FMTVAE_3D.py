...
def _global_candidate(spec: dict, feature: dict, architecture: dict) -> dict:
    """Aggregate one unchanged FMT/VAE recipe over every dataset.

    The aggregation is dataset-macro, not family-macro: a physical family
    containing several Reynolds numbers contributes one independent row per
    registered dataset.  This is the uniform-configuration protocol used by
    the paper table; the legacy family-specific selector remains available.
    """
    group_rows = [
    ...
    details = {}
    seed_values = {
        int(seed): [] for seed in spec["screen_seeds"]
    }
    for row in group_rows:
        overlap = set(details).intersection(json.loads(row["datasets_json"]))
        if overlap:
            raise RuntimeError(f"datasets repeated across groups: {sorted(overlap)}")
        details.update(json.loads(row["datasets_json"]))
        group_seed_gains = json.loads(row["seed_gains_json"])
        group_dataset_count = int(row["dataset_count"])
        for seed in seed_values:
            seed_values[seed].extend(
                [float(group_seed_gains[str(seed)])] * group_dataset_count
            )
    if set(details) != set(spec["datasets"]):
        raise RuntimeError("global Task2 candidate does not cover every dataset")
    raw = float(np.mean([value["raw_f1"] for value in details.values()]))
    fmt = float(np.mean([value["fmt_f1"] for value in details.values()]))
    seed_macro = {
        str(seed): float(np.mean(values))
        for seed, values in seed_values.items()
    }
    dataset_gains = [
        float(value["fmt_minus_raw_f1"]) for value in details.values()
    ]
    return {
        "scope": "global",
        "group": "all_datasets",
        ...
        "raw_f1_macro": raw,
        "fmt_f1_macro": fmt,
        "fmt_minus_raw_f1_macro": fmt - raw,
        "worst_dataset_f1_gain": min(dataset_gains),
        "positive_dataset_count": sum(value > 0.0 for value in dataset_gains),
...
def select(config_path: str) -> Path:
    ...
    if scope not in {"family", "global"}:
        raise ValueError("Task2 selection.scope must be 'family' or 'global'")
    if scope == "global":
        rows = [
            _global_candidate(spec, feature, architecture)
        ...
        _write_csv(output / "global_leaderboard.csv", ranked)
        top_k = int(spec.get("selection", {}).get("top_k", 3))
        winner = ranked[0]
        payload = {
            "experiment": spec["experiment"],
            "selection_scope": "one unchanged FMT/VAE recipe for all datasets",
            "selection_rule": (
                "maximize paired same-VAE development dataset-macro F1 gain; "
                "tie-break by worst-seed gain and absolute FMT F1"
            ),
            ...
            "winner": winner,
            "top_k": ranked[:top_k],
            "development_dataset_macro_f1_gain": float(
                winner["fmt_minus_raw_f1_macro"]
            ),
            "development_dataset_macro_fmt_f1": float(winner["fmt_f1_macro"]),
            "development_dataset_macro_raw_f1": float(winner["raw_f1_macro"]),
            "positive_dataset_count": int(winner["positive_dataset_count"]),
            "worst_dataset_f1_gain": float(winner["worst_dataset_f1_gain"]),
