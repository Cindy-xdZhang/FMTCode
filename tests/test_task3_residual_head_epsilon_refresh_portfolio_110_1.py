"""Contracts for residual-head Adam epsilon portfolio 110.1."""
...
import Audit_Task3_ResidualHeadEpsilonRefreshPortfolio_110_1 as audit_module
...
from Select_Task3_ResidualHeadEpsilonRefreshPortfolio_110_1 import (
    _canonical_sha256,
...
SOURCE_108_CONFIG = Path(
    "config/Verify_Task3_ResidualHeadBeta1RefreshPortfolio_108.1.yaml"
)
SOURCE_109_CONFIG = Path(
    "config/Verify_Task3_ResidualHeadEpsilonRefresh_109.1.yaml"
)


class ResidualHeadEpsilonRefreshPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "residual_head_epsilon_refresh"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.248
        )
        self.assertEqual(spec["selection"]["target_absolute_fmt_f1"], 0.907)
        self.assertFalse(spec["confirmation_opened"])

    def test_source_config_hashes_are_frozen(self):
        config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
        self.assertEqual(
            _canonical_sha256(SOURCE_108_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_109_CONFIG),
            config["sources"]["residual_head_epsilon_refresh"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_and_cleanup_contract(self):
        ...
        self.assertNotIn(
            "Select_Task3_ResidualHeadEpsilonRefreshPortfolio_110_1", source
        )
        self.assertNotIn(
        ...
        evidence = Path(
            "ibex_bash/verify_task3_residual_head_epsilon_refresh_portfolio_110.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_residual_head_epsilon_refresh_109.1_cleanup.sh"
        ).read_text(encoding="utf-8")
