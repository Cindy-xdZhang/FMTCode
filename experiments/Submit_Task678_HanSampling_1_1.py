"""Versioned dense-query Ibex dependency graph and durable process registry."""
import argparse
import datetime
import json
import os
...
from experiments.Run_Task678_HanSampling_1_1 import matrix
...
def submit(spec, config, dry_run=False):
    ...
    phases=[('smoke',[],['--time=00:20:00','--cpus-per-task=4','--mem=16G']),
        ('build',['smoke'],[f'--array=0-{d-1}%3','--time=03:00:00','--cpus-per-task=8','--mem=48G']),
        ('data_audit',['build'],['--time=01:00:00','--cpus-per-task=4','--mem=48G']),
        ('train',['data_audit'],[f'--array=0-{n-1}%9','--time=02:00:00','--cpus-per-task=4','--mem=48G',
             '--gres=gpu:1','--constraint=a100|v100','--exclude=gpu203-02-r']),
        ('affine',['data_audit'],[f'--array=0-{d-1}%3','--time=01:00:00','--cpus-per-task=4','--mem=48G']),
        ('audit',['train','affine'],['--time=03:00:00','--cpus-per-task=4','--mem=48G'])]
    for phase,deps,extra in phases:
        command=['sbatch','--parsable','--nodes=1','--ntasks=1',f'--job-name=FMThan_{phase}',
        ...
        command+=['ibex_bash/verify_task678_hansampling_1p1.sh',phase,config,commit]
        if dry_run:
