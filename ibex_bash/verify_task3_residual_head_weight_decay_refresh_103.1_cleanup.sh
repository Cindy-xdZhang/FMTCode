...
repo_root="${TASK3103_REPO_ROOT:-/home/zhanx0o/FMT_Task3_ResidualHeadWeightDecayRefresh_103_1}"
portfolio_root="${TASK3104_REPO_ROOT:-/home/zhanx0o/FMT_Task3_ResidualHeadWeightDecayRefreshPortfolio_104_1}"
cd "$repo_root"
mkdir -p slurm_logs

output_root="$repo_root/outputs/Verify_Task3_ResidualHeadWeightDecayRefresh_103.1"
...
portfolio="$portfolio_root/outputs/Verify_Task3_ResidualHeadWeightDecayRefreshPortfolio_104.1/portfolio_selection.json"
portfolio_audit="$portfolio_root/outputs/Verify_Task3_ResidualHeadWeightDecayRefreshPortfolio_104.1/independent_audit.json"
...
expected_output_root="$resolved_outputs/Verify_Task3_ResidualHeadWeightDecayRefresh_103.1"
...
if [[ "$resolved_output_root" != "$expected_output_root" ]] || \
  ...
  echo "refusing cleanup outside exact resolved 103.1 root: $resolved_candidates" >&2
  exit 1
fi
...
if [[ "$checkpoint_count_before" -ne 660 ]]; then
  echo "expected exactly 660 temporary 103.1 checkpoints, found $checkpoint_count_before" >&2
  exit 1
fi
...
if [[ "$checkpoint_count_after" -ne 0 ]]; then
  echo "103.1 checkpoint cleanup incomplete: $checkpoint_count_after remain" >&2
  exit 1
fi
...
Path(sys.argv[1]).write_text(json.dumps({
    "experiment": "Verify_Task3_ResidualHeadWeightDecayRefresh_103.1",
    "job_id": os.environ.get("SLURM_JOB_ID", ""),
    "checkpoint_count_before": int(sys.argv[2]),
    "checkpoint_count_after": int(sys.argv[3]),
    "reason": "104.1 frozen portfolio and independent audit complete",
    "status": "passed",
