...
| （已实现，待真实数据）mainExp_3DFMT_1.1 | 2026-08-17 | Task1-3D | 每个种子生成 7 条 pathline（center、x±、y±、z±）；中心位移与邻居相对位移先做时间差分；三维实序列逐坐标做 Fourier 变换，每个频率取 Gram 不变量并附加旋向三重积；邻居特征排序池化；StandardScaler + KMeans(k=2)；输出 3D/正交投影/pathline 图 | `FMT_Clustering_3D.py`, `FMT_Utils/DFT_FMT_3D.py`, `FMT_Utils/FMT_3D_pipeline.py` | `config/PathlineFMTclustering3D.yaml` | 合成端到端 smoke：180/180 valid，feature `[180,77]`，cluster `[60,120]`；真实场尚无指标 | 流程与数学不变性测试通过；是否区分涡区必须由真实 3D 场可视化判断。只保证常数旋转/常数平移不变，不保证时变观察者下的完整客观性。 |
