"""Contracts for residual-head norm/activation portfolio 100.1."""
...
import Audit_Task3_ResidualHeadNormActivationRefreshPortfolio_100_1 as audit_module
...
from Select_Task3_ResidualHeadNormActivationRefreshPortfolio_100_1 import (
    _canonical_sha256,
...
CONFIG = Path(
    "config/Verify_Task3_ResidualHeadNormActivationRefreshPortfolio_100.1.yaml"
)
SOURCE_98_CONFIG = Path(
    "config/Verify_Task3_ResidualHeadCapacityRefreshPortfolio_98.1.yaml"
)
SOURCE_99_CONFIG = Path(
    "config/Verify_Task3_ResidualHeadNormActivationRefresh_99.1.yaml"
)


class ResidualHeadNormActivationRefreshPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]),
            {"current_portfolio", "residual_head_norm_activation_refresh"},
        )
        ...
        self.assertEqual(
            spec["selection"]["target_dataset_macro_f1_gain"], 0.238
        )
        self.assertEqual(spec["selection"]["target_absolute_fmt_f1"], 0.902)
        self.assertFalse(spec["confirmation_opened"])

    def test_source_config_hashes_are_frozen(self):
        config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
        self.assertEqual(
            _canonical_sha256(SOURCE_98_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_99_CONFIG),
            config["sources"]["residual_head_norm_activation_refresh"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_and_cleanup_contract(self):
        ...
        self.assertNotIn(
            "Select_Task3_ResidualHeadNormActivationRefreshPortfolio_100_1",
            source,
        )
        self.assertNotIn(
        ...
        evidence = Path(
            "ibex_bash/verify_task3_residual_head_norm_activation_refresh_portfolio_100.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_residual_head_norm_activation_refresh_99.1_cleanup.sh"
        ).read_text(encoding="utf-8")
        self.assertIn('result_count" -ne 40', evidence)
        self.assertIn('total_count" -ne 80', evidence)
        self.assertIn('checkpoint_count_before" -ne 600', cleanup)
