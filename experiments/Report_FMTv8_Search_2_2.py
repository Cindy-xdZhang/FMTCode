...
def report(root, aborted):
    ...
    result = dict(selected_pool=pools[selected[0]], selected_profile=selected[1],
                  ...
                  costs=cost, per_dataset=flow, task4_per_flow=subgroup,
    ...
    lines += ['', '| 方法 | 任务 | 特征维数 | 总参数 | 可训练参数 | 每次训练秒 | 分类/残差训练秒 |',
              '|---|---|---:|---:|---:|---:|---:|']
    for r in summary['rows']:
        lines.append(f"| {r['pool']}/{r['profile']} | {r['task']} | {r['input_dimensions']} | {r['parameters']:,} | {r['trainable_parameters']:,} | {r['mean_training_seconds']:.3f} | {r['mean_residual_or_classifier_seconds']:.3f} |")
    lines += ['', '特征维数仅指有效几何特征：Task4-c另加一个有效线标记；Task3/5辅助输入补零到433槽并保留原Raw骨干。', '',
              '单个方法训练时间计入一次Raw骨干及本方法残差；共同骨干在搜索中只计算一次。不同表示和分支投影可能有不同参数量，不能都称同参数比较。', '',
              ...
              '完整59候选验证排名见`candidate_ranking.csv`；原50执行配置含两对等价映射，即48种不同表示。', '',
