"""Contracts for preregistered Task3 auxiliary post-norm portfolio 76.1."""
...
import Audit_Task3_AuxiliaryPostNormPortfolio_76_1 as audit_module
...
from Select_Task3_AuxiliaryPostNormPortfolio_76_1 import (
    _canonical_sha256,
...
CONFIG = Path("config/Verify_Task3_AuxiliaryPostNormPortfolio_76.1.yaml")
SOURCE_74_CONFIG = Path(
    "config/Verify_Task3_AuxiliaryFeatureScalePortfolio_74.1.yaml"
)
SOURCE_75_CONFIG = Path("config/Verify_Task3_AuxiliaryPostNorm_75.1.yaml")


class AuxiliaryPostNormPortfolioTests(unittest.TestCase):
    def test_registered_contract(self):
        spec = _load_spec(CONFIG)
        self.assertEqual(
            set(spec["sources"]), {"current_portfolio", "auxiliary_post_norm"}
        )
        ...
        self.assertEqual(spec["selection"]["target_dataset_macro_f1_gain"], 0.216)
    ...
    def test_source_config_hashes_are_frozen(self):
        ...
        self.assertEqual(
            _canonical_sha256(SOURCE_74_CONFIG),
            config["sources"]["current_portfolio"][
                "expected_config_canonical_sha256"
            ],
        )
        self.assertEqual(
            _canonical_sha256(SOURCE_75_CONFIG),
            config["sources"]["auxiliary_post_norm"][
                "expected_config_canonical_sha256"
            ],
        )
    ...
    def test_auditor_is_independent_of_76_selector(self):
        source = inspect.getsource(audit_module)
        self.assertNotIn("Select_Task3_AuxiliaryPostNormPortfolio_76_1", source)
    ...
    def test_evidence_and_cleanup_counts(self):
        evidence = Path(
            "ibex_bash/verify_task3_auxiliary_post_norm_portfolio_76.1_evidence.sh"
        ).read_text(encoding="utf-8")
        cleanup = Path(
            "ibex_bash/verify_task3_auxiliary_post_norm_75.1_cleanup.sh"
        ).read_text(encoding="utf-8")
        self.assertIn('result_count" -ne 40', evidence)
        self.assertIn('total_count" -ne 80', evidence)
        self.assertIn('checkpoint_count_before" -ne 240', cleanup)
