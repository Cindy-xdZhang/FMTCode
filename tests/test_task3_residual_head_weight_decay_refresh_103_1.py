"""Contracts for residual-head weight-decay refresh 103.1."""
...
class ResidualHeadWeightDecayRefreshTests(unittest.TestCase):
    def test_registered_grid_targets_and_job_count(self):
        ...
        self.assertEqual(
            ...
            ["training.residual_head_weight_decay_multiplier"],
        )
        selection = spec["optimization_selection"]
        self.assertEqual(selection["target_dataset_macro_f1_gain"], 0.242)
        self.assertEqual(selection["target_absolute_fmt_f1"], 0.904)
        ...
        for index, row in enumerate(spec["optimization_candidates"]):
            ...
            if index == 0:
                ...
                observed.append(float(
                    row["training"]["residual_head_weight_decay_multiplier"]
                ))
    ...
    def test_source_recipe_is_preserved_except_registered_multiplier(self):
        ...
        for rows in resolved.values():
            self.assertNotIn(
                "residual_head_weight_decay_multiplier", rows[0]["training"]
            )
            self.assertEqual(
            ...
            self.assertEqual(
                rows[1]["training"]["auxiliary_weight_decay_multiplier"], 4.0
            )
            self.assertEqual(rows[1]["training"]["weight_decay"], 0.0002)
    ...
    def test_default_is_exact_flat_optimizer_control(self):
        ...
        self.assertEqual(_residual_head_weight_decay_multiplier(training), 1.0)
    ...
    def test_head_multiplier_changes_only_downstream_weight_decay(self):
        model = _model()
        training = {
            "learning_rate": 0.001,
            "weight_decay": 0.0001,
            "residual_head_weight_decay_multiplier": 0.0,
        }
    ...
    def test_head_and_auxiliary_regularization_coexist(self):
        model = _model()
        training = {
            "learning_rate": 0.001,
            "weight_decay": 0.0001,
            "residual_head_learning_rate_multiplier": 0.25,
            "residual_head_weight_decay_multiplier": 10.0,
            "auxiliary_weight_decay_multiplier": 0.05,
        }
        parameters, _, auxiliary_group = _optimizer_parameter_spec(
            model, training
        )
        ...
        self.assertEqual(optimizer.param_groups[1]["weight_decay"], 0.000005)
    ...
    def test_validation_evidence_and_record_contract(self):
        ...
        for invalid in (-1.0, float("inf"), float("nan")):
            with self.assertRaisesRegex(
                ValueError, "residual_head_weight_decay_multiplier"
            ):
                _residual_head_weight_decay_multiplier({
                    "residual_head_weight_decay_multiplier": invalid
                })
        source = inspect.getsource(_train_one)
        self.assertIn("training_residual_head_weight_decay_multiplier", source)
        self.assertIn("residual_head_weight_decay", source)
        spec = _load_optimization_spec(CONFIG)
        self.assertEqual(
            spec["optimization_selection"]["absolute_fmt_guard"],
            {
                "control_optimization_id": "w00_control_source",
                "f1_tolerance": 0.0,
                "average_precision_tolerance": 0.0,
            },
        )
        gpu = Path(
            "ibex_bash/verify_task3_residual_head_weight_decay_refresh_103.1_gpu.sh"
        ).read_text(encoding="utf-8")
        evidence = Path(
            "ibex_bash/verify_task3_residual_head_weight_decay_refresh_103.1_evidence.sh"
        ).read_text(encoding="utf-8")
