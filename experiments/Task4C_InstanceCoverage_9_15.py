"""Task4-c v9.15: instance coverage, two-set training, and sparse voxel caches."""
...
CONFIG = 'config/mainExp_Task4C_InstanceCoverage_9.15.json'
...
def identity(config):
    paths = [__file__, 'FMT_Utils/Task4C_InstanceCoverage_9_15.py',
        ...
        'ibex_bash/task4c_instance_9p15.sh']
...
def load_spec(config):
    ...
    for flow, limits, target in [('channel', (.001, .005), .1), ('tbl', (.01, .05), 5.)]:
        for step in spec['integration'][flow]:
            assert limits[0] <= step['ds'] <= limits[1]
            assert .8*target-1e-12 <= step['ds']*step['maxiteration'] <= 1.2*target+1e-12
    return spec
...
def preflight(spec, config, device):
    ...
    report = dict(parameters=parameters, resolutions={}, device=device, identity=identity(config))
...
def export_viewer_package(spec, config, coverage):
    """Twenty bundles per instance, including all three mandatory head examples."""
    ...
    for fi, flow in enumerate(spec['flows']):
        for role in ('train', 'test'):
            ...
            for item in coverage:
                if item['flow'] != flow['name'] or item['role'] != role:
                    continue
                source = root/'physical'/flow['name']/str(item['instance'])
                with np.load(source/'metadata.npz') as z: m = {k: z[k] for k in z.files}
                mandatory = np.flatnonzero(m['mandatory_head'])
                positive = np.flatnonzero((m['labels'] == 1) & ~m['mandatory_head'])
                negative = np.flatnonzero(m['labels'] == 0)
                take = np.r_[mandatory,
                    positive[diverse_centers(m['center'][positive], 7, spec['sampling']['seed']+item['instance'])],
                    negative[diverse_centers(m['center'][negative], 10, spec['sampling']['seed']+item['instance']+1)]]
                assert len(take) == 20 and len(np.unique(take)) == 20
                g = np.load(source/'geometry.npy', mmap_mode='r')[take].astype(np.float64)
                world = g*m['radius'][take, None, None, None]+m['centroid'][take, None, None, :]
                world[np.arange(27)[None] >= m['counts'][take, None]] = 0
                pack = {k: m[k][take] for k in ('counts', 'labels', 'center', 'head_component', 'instance',
                    'owner_instance', 'mandatory_head', 'scale_id', 'radius', 'neighbor_distance')}
                pack.update(geometry=world.astype(np.float32), row_ids=take+population)
                for method in spec['methods']:
                    pack['p_'+method] = np.array([predictions[method, role][fi, item['instance'], int(i)] for i in take])
                population += len(m['labels']); packs.append(pack)
            ...
            entries[flow['name']+'/'+role] = dict(file=filename, sha256=sha(output/filename),
                selected=len(packed['labels']), population=population, selection_uses_prediction_scores=False,
                per_instance_selected=20, per_instance_mandatory_head=3)
    write(output/'manifest.json', dict(version=spec['version'], identity=identity(config),
        config=dict(display_bundles=200, splits=['train', 'test']), flows=spec['flows'], models=models, pending=[], splits=entries,
        evidence_note='F1使用完整流场集合，阈值0.5。每实例展示20束（10束Hairpin、10束Non-hairpin），包含3束必选head样本；按真实束标签与空间位置抽取，不依据预测选图。束标签由有效播种点的GT归属确定。坐标与比例保持物理尺度，z向上。'))
...
def runtime(spec, config, phase, state, code=None):
    row = dict(phase=phase, state=state, exit_code=code, time_utc=datetime.now(timezone.utc).isoformat(),
...
def submit(spec, config):
    if shutil.disk_usage(Path.cwd()).free < 40*2**30:
        raise RuntimeError('Need at least 40 GiB free for geometry and sparse caches; no old results are deleted')
    ...
    phases = [('preflight', None, True, '00:20:00'), ('pilot', '0-15%8', False, '02:00:00'),
              ('prepare', '0-15%8', False, '08:00:00'), ('encode', '0-1%2', True, '03:00:00'),
              ('train', '0-11%6', True, '23:00:00'), ('merge', None, False, '00:30:00')]
    for phase, array, gpu, limit in phases:
        cmd = ['sbatch', '--parsable', '--nodes=1', '--ntasks=1', '--cpus-per-task=4', '--mem=32G',
            '--time='+limit, '--job-name=t4c915-'+phase, f'--output={root}/logs/{phase}_%A_%a.out',
        ...
        cmd += ['ibex_bash/task4c_instance_9p15.sh', phase, config]
